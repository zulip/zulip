...
IGNORED_PHRASES = [
    ...
    r"(exclude )?topic <z-topic-name></z-topic-name>",
    r"exclude {operand}",
    r"search for {operand}",
    r"(exclude )?channel {operand}",
    r"(exclude )?channels public",
    r"(exclude )?messages around {operand}",
    r"(exclude )?messages with links",
    r"(exclude )?messages with images",
    r"(exclude )?messages with attachments",
    r"(exclude )?messages with reactions",
    r"(exclude )?messages with {operand}",
    r"(exclude )?message ID {operand}",
    r"(exclude )?sent by",
    r"(exclude )?direct messages with",
    r"(exclude )?direct messages including",
