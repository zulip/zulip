...
def build_custom_checkers(by_lang):
    ...
    html_rules = whitespace_rules + prose_style_rules + [
        ...
        {'pattern': "aria-label='[^{]",
         ...
         'good_lines': ['<button class="button small btn-danger deactivate btn-danger" aria-label="{{ \'Deactivate stream\' }}">'],
        ...
        {'pattern': 'script src="http',
         ...
         'good_lines': ['<script src="{{ render_bundle("translations") }}"></script>'],
         'bad_lines': ['<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>']},
        {'pattern': "title='[^{]",
         'description': "`title` value should be translatable.",
         'good_lines': ['<link rel="next" title="{{ next.title|striptags|e }}" href="{{ next.link|e }}"/>'],
        ...
        {'pattern': '\Walt=["\'][^{"\']',
         ...
         'exclude': set(['static/templates/settings/display-settings.handlebars',
                         ...
                         'templates/zerver/markdown_help.html']),
         'good_lines': ['<img src="{{source_url}}" alt="{{name}}" />', '<img alg="" />'],
         'bad_lines': ['<img alt="Foo Image" />']},
        {'pattern': '\Walt=["\']{{ ?["\']',
         'description': "alt argument should be enclosed by _().",
         'good_lines': ['<img src="{{source_url}}" alt="{{name}}" />'],
         'bad_lines': ['<img alt="{{ " />']}, 
        {'pattern': r'\bon\w+ ?=',
