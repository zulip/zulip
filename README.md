...
2. Add your API key:
   ...
   ```
   _(Note: `zproject/dev-secrets.conf` is gitignored and will never be committed)._
...
2. Post a new message discussing a completely different topic (e.g., _"Should we switch our database cache to Redis? What TTL should we use?"_).
...
4. If drift is detected, a **Topic Drift Warning Banner** appears above the compose box:
   > ⚠️ _Topic discussion seems to have drifted. Suggested new title: "Redis Database Caching"_
...
- **`implementation.md`** (located at the root of this repository)
