...
Organizations can configure whether resolving a topic requires a message.

* If a message is required, you will need to send a resolution message when
  marking the topic as resolved.
* If a message is optional, you can either send a resolution message or use
  **Resolve without message**.
* Resolution messages must contain at least 5 visible characters after
  formatting is rendered. For example, markdown formatting syntax by itself
