...
* `message`: A dictionary containing information on the Zulip message
  that the bot is reacting to. This is the message object for an
  existing message that has already been sent (provided by
  `handle_message`).
* `emoji_name`: A string with the name of the emoji reaction to add

### Example implementation

From `zulip_bots/bots/helloworld/helloworld.py`:
The helloworld bot is a standard implementation of replying and
reacting to a message.

```python
def handle_message(self, message, bot_handler):
  bot_handler.send_reply(message, 'beep boop')
