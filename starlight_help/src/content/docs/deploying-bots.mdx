...
Everything is set up! Test your Botserver like you would
test a normal bot.
...
<Steps>
  ...
  1. Download the `botserverrc` file from the **Bots** tab of the
     **Personal settings** menu, by clicking the **download**
     (<DownloadIcon />) icon above the **Bots** table.
...
[supervisord](https://supervisord.org/) is a popular tool for running
...
* Your Botserver needs to be accessible from your Zulip server over
  ...
  recommend using [zulip-run-bot](/help/running-bots) instead for
