...
* **Message history**, excluding messages in private channels
...
When planning to migrate from Microsoft Teams to Zulip, make sure your Microsoft
...
and Zulip's import tool for Microsoft Teams requires an active Microsoft Entra
...
[update their Zulip email addresses](/help/change-your-email-address).
...
<Steps>
  ...
  1. Take note of the application's `tenant_id`, `client_id`, and
     `client_secret`.
...
| [get hosted content](https://learn.microsoft.com/en-us/graph/api/chatmessagehostedcontent-get?view=graph-rest-1.0\&tabs=http)       | Used to download hosted content attachments.                  | `ChannelMessage.Read.All` and `Chat.Read.All` |
| [list chat members](https://learn.microsoft.com/en-us/graph/api/chat-list-members?view=graph-rest-1.0)                              | Used to list members of chats for converting direct messages. | `ChatMember.Read.All`                         |
...
* Microsoft Teams allows multiple group chats with an identical set of
  members, but Zulip doesn't. When several group chats share the same
  set of members, only one of them is imported; messages in the others
  are not imported.
...
<Steps>
  ...
  1. **Make sure all users [change their Zulip email
     ...
     tools](https://zulip.readthedocs.io/en/latest/production/management-commands.html)
     also allow self-hosted server administrators to change user emails directly;
     Zulip Cloud users can [contact support](mailto:support@zulip.com) for
     assistance.
