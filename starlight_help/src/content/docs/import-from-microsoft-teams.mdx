...
* **Name**
* **Message history**, excluding direct messages
...
tool](https://learn.microsoft.com/en-us/microsoftteams/migration-from-teams) supports
...
API](https://learn.microsoft.com/en-us/microsoftteams/migration-from-teams#steps-to-access-the-teams-data-export-tool)
...
<Steps>
  ...
  1. File a ticket using the predefined title and in the description, provide an
...
tool will call several Microsoft Graph API endpoints to collect these data:
...
The following instructions will guide you through creating a Microsoft Entra ID application
...
<Steps>
  1. Log in to the [Microsoft Entra Teams admin panel](https://entra.microsoft.com/) as
  ...
  1. Search and add these permissions:
     ...
     * `RoleManagement.Read.Directory`
     ...
     details about these tokens and the endpoints that we use.
...
tool when converting the exported data:

* The [list directory
  role](https://learn.microsoft.com/en-us/graph/api/directoryrole-list?view=graph-rest-1.0\&tabs=http)
  endpoint is used to find the ID for the global administrator role. The least-privileged scope
  for this endpoint is the `RoleManagement.Read.Directory` scope.
* The [directory role list
  member](https://learn.microsoft.com/en-us/graph/api/directoryrole-list-members?view=graph-rest-1.0\&tabs=http)
  endpoint is used to find users who are global administrator. The least-privileged scope
  for this endpoint is the `User.Read.All` scope.
* The [list
  user](https://learn.microsoft.com/en-us/graph/api/user-list?view=graph-rest-1.0\&tabs=http)
  endpoint is used to find the guest accounts. The least-privileged scope for this endpoint
  is the `User.Read.All` scope.
...
* Microsoft Teams's user roles are mapped to Zulip's [user
  ...
  | Member               | Member     |
  | Guest                | Guest      |
* Message thread replies will combined to the main topic.
