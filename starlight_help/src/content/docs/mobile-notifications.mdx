...
Zulip Server 12.0+ and Zulip Cloud support end-to-end encryption for mobile
push notifications. With E2EE, message content and metadata (including the
sender's name, and the channel and topic where the message was sent) are
encrypted before leaving your Zulip server, and cannot be read by the
Zulip Mobile Push Notification Service, Apple, or Google.

With E2EE:

* **Apple and Google cannot read** the contents of your Zulip notifications.
  They can see that a notification arrived and how large it was, but not
  what it says.
* **The Zulip Mobile Push Notification Service cannot read** the contents
  of your notifications. It forwards the encrypted payload to Apple or
  Google without decrypting it.
* **Your Zulip server** is the source of each notification and creates the
  encrypted payload before sending.

If the Zulip app is not running when a notification arrives, you may
briefly see a generic alert ("New notification") before the message
contents appear. If you uninstall the Zulip app or stop using a device,
that device will stop receiving notifications until you log in again.

Support for E2EE is [coming soon][e2ee-flutter-issue] to the Zulip mobile
app. Once your app is updated, push notifications sent from an up-to-date
Zulip server will be end-to-end encrypted.
...
Organization administrators can require end-to-end encryption for message
content in mobile push notifications. When this setting is enabled, message
content is omitted when sending notifications to an app that doesn't yet
support E2EE; users will see "New message" in place of the message text.
See the [technical
