...
* The **email** is used for [HTTP Basic authentication](/api/http-headers).
...
An **outgoing webhook** bot responds based on its configured
[triggers](/help/bots-overview#outgoing-webhook-bot-triggers). Depending on the
bot's triggers, it can read direct messages where the bot is a participant, and
channel messages where the bot is [mentioned](/help/mention-a-user-or-group).
When triggered, it POSTs the message content to a URL of your choice. The POST
request format can be in a Zulip format or a Slack-compatible format. This is
the preferred bot type for interactive bots built on top of Zulip Botserver.
...
## Outgoing webhook bot triggers

These determine what events will trigger an outgoing webhook bot's POST action.

| Trigger                                                     | Description                                                                                                                                                                                                                         |
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mentions of the bot, whether or not it received the message | Any message directly [mentioning](/help/mention-a-user-or-group) the bot, regardless of whether or not the bot actually receives it. Topic, group and @-all mentions are not direct mentions, and therefore do not trigger the bot. |
| Direct messages the bot receives                            | Both one-on-one or group direct messages.                                                                                                                                                                                           |
