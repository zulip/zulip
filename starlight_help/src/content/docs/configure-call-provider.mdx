...
as its default call provider. Administrators can also use
their own self-hosted instance of Jitsi Meet for their organization,
as described below.
...
<ZulipNote>
  **Warning: This applies only to self-hosted Zulip servers, not to
  Zulip Cloud**:
  Self-hosted instances of Zulip could be configured to use
  [Jitsi with JWT Token Authentication](https://zulip.readthedocs.io/en/stable/production/video-calls.html#jwt-authentication).
  Choosing **Custom URL** falls back to unauthenticated calls against
  your custom server.
</ZulipNote>
