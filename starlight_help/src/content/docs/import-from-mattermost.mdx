...
<Tabs>
  <TabItem label="Default installation">
    <Steps>
      1. These instructions assume your Mattermost version is `8.0.0` or higher.
      1. Log into your Mattermost production server.
         ```bash "yourdomain"
         mmctl auth login `<server_ip>`
         ```
         You’ll be prompted for a username (use your admin user), password, and for a
      ...
      1. Create a full export of the server, including attached files.
         ...
         ```
         Since version >= 7.3.0, mmctl exports attachments by default:
         ```bash
         mmctl export create
         ```
      ...
      1. Download the export file to the server, replacing `latest_export`
         ...
         ```
      1. Finally, copy the exported zip file from the server to your local
  ...
  <TabItem label="Docker">
    <Steps>
      ...
      1. Create a full export of the server, including attached files.
         On version \< 7.3.0:
         ```bash
         docker exec -it mattermost-docker_app_1 mmctl export create --attachments
         ```
         Since version >= 7.3.0, mmctl exports attachments by default:
         ```bash
      ...
      1. Download the exported file from the server to your local
         computer.
    </Steps>
  </TabItem>
  ...
  <TabItem label="GitLab Omnibus">
    <Steps>
      1. Make sure you have [mmctl](https://github.com/mattermost/mmctl) installed - these
         instructions assume your version is `7.5.1` or higher.
      1. Log into your Mattermost GitLab instance using a non-SSO GitLab
    ...
    If remote connection isn't possible, see
    [GitLab's documentation](https://docs.gitlab.com/integration/mattermost/#use-mmctl-through-a-local-connection)
    on using `mmctl` through a local connection.
  </TabItem>
</Tabs>
