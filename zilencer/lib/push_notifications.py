...
from zerver.lib.devices import b64decode_token_id, b64encode_token_id
    ...
    for idx, response in enumerate(batch_response.responses):
        ...
        if response.success:
            ...
            if isinstance(error, FCMUnregisteredError):
                remote_push_device.expired_time = timezone_now()
                remote_push_device.save(update_fields=["expired_time"])
                delete_token_ids.append(b64encode_token_id(remote_push_device.token_id))
...
def send_e2ee_push_notifications(
    ...
    *,
    ...
    for push_request in push_requests:
        token_ids.add(push_request.token_id)
        token_ids_int.add(b64decode_token_id(push_request.token_id))
    ...
    token_id_to_remote_push_device = {
        b64encode_token_id(remote_push_device.token_id): remote_push_device
