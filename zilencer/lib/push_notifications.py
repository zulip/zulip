...
def send_e2ee_push_notification_apple(
    ...
    for remote_push_device, result in results:
        ...
        result_info_dict = get_info_from_apns_result(
            result,
            remote_push_device,
            log_context,
        )
        ...
        elif result_info_dict.get("delete_device_id") is not None:
            remote_push_device.expired_time = timezone_now()
            remote_push_device.save(update_fields=["expired_time"])
            delete_device_ids.append(result_info_dict["delete_device_id"])

    print(successfully_sent_count, "yohooo")
    return successfully_sent_count
