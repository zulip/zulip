...
def loadConfig():
    ...
    infile = open("config.generate_data.json", "r")
...
def parseFile(config, gens, corpus_file):
    ...
    with open(corpus_file, "r") as infile:
        ...
        paragraphs = addFlair(paragraphs)
...
def addFlair(paragraphs):
    ...
    for i in range(len(paragraphs)):
        ...
        if key == "None":
            ...
            txt = addMD("list", paragraphs[i][1])
        elif key == "emoji":
            txt = addEmoji(paragraphs[i][1])
        elif key == "link":
            txt = addLink(paragraphs[i][1])
        elif key == "picture":
            txt = txt      # todo: implement pictures

        results.append((paragraphs[i][0], txt))
...
def addEmoji(text):
    ...
    vals[start] = vals[start] + " " + next(gens["emojis"]) + " "
    return " ".join(vals)
...
def getDialog(line):
    ...
    if '.' in line:
        ...
        if strpos > 0:
            ...
            vals = actor.split()
            if len(vals) < 2:
                return(actor, removeActions(line[strpos + 1:].strip()))
            else:
...
if __name__ == "__main__":
    ...
    writeFile(paragraphs, "../new_test_data.txt")

    # todo: these should be rolled into tests eventually

    # checkForDupes(gens)

    # txt = "There's only one way to rock!"

    # for i in range(100):
    #    print (addMD ("*", txt))
    #    print (addMD ("**", txt))

    # print(getFlairGen(100))

    # move this to a test file
    # print (removeActions("this text. [This is my action!] And more text."))
    # print (removeActions("this is my text. [This is my action!]"))
