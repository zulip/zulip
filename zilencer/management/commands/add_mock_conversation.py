...
* Remove the blue box (find the CSS for .selected_message, and uncheck it)
...
"""
    ...
    def add_message_formatting_conversation(self) -> None:
        ...
        stream = ensure_stream(realm, "general", acting_user=None)

        UserProfile.objects.filter(delivery_email__contains="stage").delete()
        dal = do_create_user("1@stage.example.com", "password", realm, "Dal Kim", acting_user=None)
        self.set_avatar(dal, "static/images/characters/Dal-Kim.jpg")
        bo = do_create_user("2@stage.example.com", "password", realm, "Bo Lin", acting_user=None)
        self.set_avatar(bo, "static/images/characters/Bo-Lin.jpg")
        maxy = do_create_user(
            "3@stage.example.com", "password", realm, "Maxy Stert", acting_user=None
        )
        self.set_avatar(maxy, "static/images/characters/Maxy-Stert.jpg")
        ...
        staged_messages: List[Dict[str, Any]] = [
            {
                "sender": dal,
                "content": "Hey @**Bo Lin**, check out Zulip's Markdown formatting! "
            ...
            {
                "sender": maxy,
                "content": "I think you forgot your base case there, Bo :laughing:\n"
                "```quote\n```python\ndef fib(n: int) -> int:\n    # returns the n-th Fibonacci number\n"
                "    return fib(n-1) + fib(n-2)\n```\n```",
            },
            {
        ...
        do_add_reaction(maxy, preview_message, "whale", emoji_code, reaction_type)
        ...
        do_add_reaction(dal, preview_message, "thumbs_up", emoji_code, reaction_type)
