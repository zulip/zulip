...
class MissingBouncerPublicKeyError(Exception):
    pass


class PushRegistrationLifespanExceededError(Exception):
    pass
...
def action_function(
    ...
    *,
    ...
    push_registration = json.loads(push_registration_bytes.decode("utf-8"))
    ...
    if timezone_now() - timestamp > PUSH_REGISTRATION_LIVENESS_TIMEOUT:
    ...
    if remote:
        realm = None
        remote_realm = RemoteRealm.objects.filter(uuid=realm_uuid).first()
    else:
        realm = Realm.objects.filter(uuid=realm_uuid).first()
        remote_realm = None

    assert realm is not None and remote_realm is not None
...
@typed_endpoint
def register_remote_push_device_new(
    request: HttpRequest,
    server: RemoteZulipServer,
    *,
    realm_uuid: Json[str],
    ...
    try:
        ...
        raise InvalidBouncerPublicKeyError(_("TODO"))
    except PushRegistrationLifespanExceededError:
        raise PushRegistrationLivenessTimedOutError(_("TODO"))
