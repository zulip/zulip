...
1.  In the same directory, start the Zulip bot with the command:
    ...
    `python /usr/local/share/zulip/integrations/google/google-calendar --user <email for reminders>`

    Use the [optional flags](#configuration-options) to configure the bot.

1.  You will only get reminders as long as the terminal session with the bot
    remains open.

    !!! tip ""

        Consider using `screen` to run the bot in the background.
