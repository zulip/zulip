...
* `social_backends`: list of dictionaries describing the available
  social authentication backends (such as google/github/SAML). Each
  dictionary specifies the name and icon that should be displayed on
  the login buttons (`display_name` and `display_icon`, where
  `display_icon` can be the empty string, if no icon is to be
  displayed), the URLs that should be accessed to initiate
  login/signup with the backend (`login_url` and `signup_url`) and
  `name`, which is a unique, stable, machine-readable name for the
  authentication backend.  The list is sorted in the order in which
  these authentication methods should be displayed.
