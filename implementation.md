...
- **`zerver/actions/message_recap.py`**: Implements the core business logic `do_generate_recap()`:
  ...
  - Retrieves full message contents and metadata via `messages_for_ids()`.
  - Groups messages by conversation (Stream + Topic or DM participants) in `format_messages_for_recap_prompt()`.
  - Calls the configured OpenAI-compatible LLM endpoint (Groq / Gemini / OpenAI) using `client.chat.completions.create()`.
  - Tracks token usage and AI inference runtime using Zulip's `ai_stats_start()` / `ai_stats_finish()` and increments `COUNT_STATS["ai_credit_usage::day"]`.
  - Converts the returned CommonMark text to safe rendered HTML via `markdown_convert()`.
- **`zproject/urls.py`**: Registers the route `messages/recap` for `GET` requests (mapped to both `/api/v1/messages/recap` and `/json/messages/recap`).
...
- **`web/templates/left_sidebar.hbs`**: "Message recap" navigation item with magic-wand icon in the left sidebar under Views.
- **`web/templates/inbox_view/inbox_view.hbs`**: "Recap" button added directly into the Inbox search/filter header bar.
...
- **`zproject/urls.py`**: Registers the `topics/check_drift` route.
- **`zerver/tests/test_topic_drift.py`**: Automated test suite covering drift detection, on-topic negative cases, short topic skipping, permission denial, and credit limits.
...
3. **Bounded Context & Compact Output (Token Optimization)**:
   - We bound input history to the `MAX_MESSAGES_FOR_DRIFT = 15` most recent messages and constrain the LLM completion to `max_tokens = 1000` with JSON output mode. By requesting only `{"has_drift": bool, "suggested_title": str}`, we eliminate verbose explanatory text, reducing output token consumption by ~60-70% per call while maintaining low inference latency.
...
- **`web/src/compose.ts`**: Triggers `check_topic_drift_for_sent_message()` immediately in `send_message_success` when a user posts a message to a stream topic.
- **`web/src/compose_banner.ts`**: Added `topic_drift_suggestion` class to banner manager.
- **`web/templates/popovers/left_sidebar/left_sidebar_topic_actions_popover.hbs`**: Added **"Improve topic title (AI)"** option in the topic actions popover menu for manual on-demand triggers.
- **`web/src/topic_popover.ts`**: Click handler for manual topic title improvement from the sidebar menu.
