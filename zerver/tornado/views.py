...
from zerver.tornado.event_queue import fetch_auth_longpoll, fetch_events, \
...
# TODO: Add URLs.py plumbing
# TODO: Add all the nginx (etc.) plumbing for this new endpoint.
@has_request_variables
def get_auth_longpoll(request: HttpRequest,
                      desktop_flow_id: str=REQ(validator=check_string)) -> HttpResponse:
    # TODO: Validate format of desktop_flow_id as a long string.

    # Extract the Tornado handler from the request
    handler = request._tornado_handler  # type: AsyncDjangoHandler
    handler._request = request
    return fetch_auth_longpoll(desktop_flow_id, handler)
