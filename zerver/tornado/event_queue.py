...
def process_notification(notice: Mapping[str, Any]) -> None:
    ...
    elif event["type"] == "realm_user" and event["op"] == "api_key_changed":
        # This event is generated by do_regenerate_api_key and is
        # intended for the Tornado server, not clients; as such, we
        # return.
        flush_tornado_user_profile_cache(user_id=event["person"]["user_id"])
        return
