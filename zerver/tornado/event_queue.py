...
def process_user_topic_event(event: Mapping[str, Any], users: Iterable[int]) -> None:
    for user_profile_id in users:
        for client in get_client_descriptors_for_user(user_profile_id):
            ...
            if event.get("topic_name") != "" or client.empty_topic_name:
                client.add_event(event)
                continue

            event_copy = dict(event)
            event_copy["topic_name"] = Message.EMPTY_TOPIC_FALLBACK_NAME
...
def process_stream_typing_notification_event(
    ...
    for user_profile_id in users:
        for client in get_client_descriptors_for_user(user_profile_id):
            ...
            if event.get("topic") != "" or client.empty_topic_name:
                client.add_event(event)
                continue

            event_copy = dict(event)
            event_copy["topic"] = Message.EMPTY_TOPIC_FALLBACK_NAME
            client.add_event(event_copy)
        ...
        for client in get_client_descriptors_for_user(user_profile_id):
            ...
            for message_id, message_detail in event_copy["message_details"].items():
                if message_detail["type"] == "stream" and message_detail.get("topic") == "":
                    event_copy["message_details"][message_id]["topic"] = (
