...
from zerver.lib.typed_endpoint_validators import check_url
...
def update_link_previews_backend(
    ...
    *,
    message_id: PathOnly[NonNegativeInt],
    op: Literal["remove"],
    # A message never renders more previews than this, so a request that
    # names more URLs cannot be removing previews that exist.
    urls: Json[
        Annotated[
            list[Annotated[str, AfterValidator(check_url)]],
            Len(max_length=InlineInterestingLinkProcessor.INLINE_PREVIEW_LIMIT_PER_MESSAGE),
        ]
    ],
) -> HttpResponse:
    ...
    check_remove_link_previews(user_profile, message_id, urls)
