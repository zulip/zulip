...
def json_change_settings(
    ...
    *,
    ...
    request_settings = {k: v for k, v in locals().items() if k in user_profile.property_types and v is not None}

    if target_users is not None:
        if not user_profile.is_realm_owner:
            raise JsonableError(_("Only organisation admins can change this setting for other users."))
        
        user_ids = target_users.get("users", [])
        user_group_ids = target_users.get("user_groups", [])
        print("user_ids", user_ids)
        users = UserProfile.objects.filter(id__in=user_ids, realm=user_profile.realm, is_active=True)
        ...
        for k, v in request_settings.items():
            ...
            do_change_user_setting(total_users, k, v, acting_user=user_profile, skip_if_already_edited=skip_if_already_edited)

        if timezone is not None:
            do_change_user_setting(total_users, "timezone", timezone, acting_user=user_profile, skip_if_already_edited=skip_if_already_edited)
