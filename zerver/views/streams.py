...
from zerver.lib.users import bulk_access_user_by_email, bulk_access_user_by_id
...
def bulk_principals_to_user_profiles(
    ...
    if isinstance(principals[0], str):
        return bulk_access_user_by_email(
            users_emails=principals,  # type: ignore[arg-type] # principals guaranteed to be list[str] only.
    ...
    # principals are user ids.
        ...
        return bulk_access_user_by_id(
            users_ids=principals,  # type: ignore[arg-type] # principals guaranteed to be list[int] only.
            acting_user=acting_user,
            allow_deactivated=False,
            allow_bots=True,
            for_admin=False,
