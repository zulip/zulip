...
class ConstructorGroupsService:
    ...
    def get_or_create_default_room(
        ...
        """Get existing default room or create new one if not exists.

        Uses single API call to /room/default endpoint.
        Returns 200 if room already exists, 201 if newly created.
        """
        data = {
...
def make_constructor_groups_video_call(
    ...
    room_data = service.get_or_create_default_room(
        ...
        name=f"{user_profile.full_name}'s Zulip Videoconferencing Room",
        fallback_name=f"{user_profile.full_name}-{user_profile.id}'s Zulip Videoconferencing Room",
    )
