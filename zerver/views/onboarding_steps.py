...
def mark_onboarding_step_as_read(
    ...
    *,
    onboarding_step: str,
    schedule_onboarding_video_reminder_delay: Json[int] | None = None,
) -> HttpResponse:
    ...
    if schedule_onboarding_video_reminder_delay is not None:
        assert onboarding_step == "onboarding_video"
        ...
        message_content = _("""
Is this a good time to watch the [Welcome to Zulip video]({onboarding_video_url})?
""").format(onboarding_video_url=settings.ONBOARDING_VIDEO_URL)
        deliver_at = timezone_now() + timedelta(seconds=schedule_onboarding_video_reminder_delay)
