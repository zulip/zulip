from django.core.serializers.json import DjangoJSONEncoder
from django.db import models
from django.db.models.functions import Now
from django.utils.timezone import now as timezone_now

from zerver.models.realms import Realm
from zerver.models.users import UserProfile


class IdempotentRequest(models.Model):
    """Ensure idempotency for non-idempotent HTTP requests (e.g., POST).

    Currently, only used for message sending, but in future it should be applied to
    any non-idempotent request having the possibility of being wrongly replayed.
    """

    realm = models.ForeignKey(Realm, on_delete=models.CASCADE)
    user = models.ForeignKey(UserProfile, on_delete=models.CASCADE)

    # Only client-provided.
    idempotency_key = models.UUIDField()

    # We use both Now() and timezone_now to ensure timestamp is auto created
    # when inserting via raw SQL and Django, respectively.
    timestamp = models.DateTimeField(db_default=Now(), default=timezone_now, db_index=True)

    # The cache of successful response.
    # TODO: we may need to cache json error in the future.
    cached_result = models.JSONField(default=None, null=True, encoder=DjangoJSONEncoder)

    # None if the work has not been attempted.
    # In the future we will probably need to cache errors as well,
    # so we need this bool to differentiate between succeeded
    # and failed work.
    succeeded = models.BooleanField(default=None, null=True)

    # The retention period, in hours, after which rows are
    # considered expired and eligible for deletion by the cron job.
    RETENTION_DURATION_IN_HRS = 24

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["realm", "user", "idempotency_key"],
                name="unique_realm_user_idempotency_key",
            ),
        ]
