...
def get_display_recipient(recipient):
    # type: (Recipient) -> List[Dict[str, Any]]
...
def get_display_recipient_remote_cache(recipient_id, recipient_type, recipient_type_id):
    # type: (int, int, int) -> Union[str, List[Dict[str, Any]]]
    """
...
def get_active_streams(realm):
    # type: (Realm) -> QuerySet
    # TODO: Return_Type_Is_QuerySet_Or_List[stream] because in others objects.filter it
    # seems to be a list but here it is not as we are using it in line 680
    """
...
def get_stream(stream_name, realm):
    # type: (str, Union[int, Realm]) -> Optional[Stream]
    try:
...
def bulk_get_streams(realm, stream_names):
    # type: (Realm, STREAM_NAMES) -> Dict[int, Any]
    # TODO: RETURN_TYPE_CAN_BE_STR_OR_INT zerver/views/streams.py:66
    if isinstance(realm, Realm):
        realm_id = realm.id
    else:
        realm_id = realm
...
class Message(models.Model):
    ...
    def content_has_attachment(content):
        # type: (str) -> Match
        # TODO: RETURN_TYPE_IS_INSTANCE_OF_RE.MATCHOBJECT
...
class UserPresence(models.Model):
    ...
    def to_presence_dict(client_name=None, status=None, timestamp=None, push_enabled=None,
        ...
        # type: (str, int, int, bool, bool, bool) -> Dict[str, Any]
