...
# NODL MODIFICATION START - Lazy load emoji data
# The emoji-datasource file is generated by Zulip's npm build process and
# may not exist in Railway deployments. Since Slack import is not used by nodl,
# we make this lazy to avoid FileNotFoundError at module import time.
# Date: 2024-12-01
...
def _get_emoji_data() -> list[dict[str, Any]]:
    global _emoji_data
    if _emoji_data is None:
        with open(emoji_data_file_path, "rb") as emoji_data_file:
            _emoji_data = orjson.loads(emoji_data_file.read())
    return _emoji_data
...
# NODL MODIFICATION START - Lazy build the translation dict
# Build the translation dict from Slack emoji name to codepoint lazily
_slack_emoji_name_to_codepoint: dict[str, str] | None = None


def _get_slack_emoji_name_to_codepoint() -> dict[str, str]:
    global _slack_emoji_name_to_codepoint
    if _slack_emoji_name_to_codepoint is None:
        _slack_emoji_name_to_codepoint = {}
        for emoji_dict in _get_emoji_data():
            short_name = emoji_dict["short_name"]
            emoji_code = get_emoji_code(emoji_dict)
            _slack_emoji_name_to_codepoint[short_name] = emoji_code
            for sn in emoji_dict["short_names"]:
                if sn != short_name:
                    _slack_emoji_name_to_codepoint[sn] = emoji_code
    return _slack_emoji_name_to_codepoint
# NODL MODIFICATION END
...
def build_reactions(
    ...
    # NODL MODIFICATION: Use lazy getter for emoji name to codepoint mapping
    slack_emoji_name_to_codepoint = _get_slack_emoji_name_to_codepoint()
    for slack_reaction in reactions:
