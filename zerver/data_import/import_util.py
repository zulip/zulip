...
def get_attachment_path_and_content(
    ...
    path_id = upload_backend.generate_message_upload_path(str(realm_id), sanitize_name(filename))
    attachment_url = f"/user_uploads/{path_id}"
    markdown_link = get_markdown_link_for_url(link_name, attachment_url)

    return AttachmentLinkResult(url=path_id, markdown_link=markdown_link)
