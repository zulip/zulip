...
def mattermost_data_file_to_dict(
    ...
    with open(mattermost_data_file, "rb") as fp:
        for line in fp:
            ...
            if data_type == "post":
                if combine_into_one_realm:
                    row["post"]["team"] = DEFAULT_SINGLE_TEAM_OBJECT["display_name"]
                ...
                # part of (modulo combine_into_one_realm=True). This data will be
                # backfilled later.
                bot_data["teams"] = (
                    []
                    if not combine_into_one_realm
                        ...
                        {
                            "name": DEFAULT_SINGLE_TEAM_OBJECT["display_name"],
                            "roles": "team_user",
                            "channels": [],
                        }
                    ]
                ...
                mattermost_data[data_type].append(row[data_type])
            elif data_type == "post" and combine_into_one_realm:
                row[data_type]["channel"] = COMPILED_CHANNEL_ID_FORMAT.format(
                    id=row[data_type]["name"], team=row[data_type]["team"]
                )
                row[data_type]["team"] = DEFAULT_SINGLE_TEAM_OBJECT["name"]
                mattermost_data[data_type].append(row[data_type])
                ...
                if (
                    ...
                    if isinstance(row[data_type]["team"], str):
                        row[data_type]["team"] = DEFAULT_SINGLE_TEAM_OBJECT["name"]
                    else:
                        raise AssertionError(
                            "Failed to compile unknown object into the combined realm. %s",
                            row[data_type],
                        )
                mattermost_data[data_type].append(row[data_type])
