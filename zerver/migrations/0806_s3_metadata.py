...
# A copy of zerver.lib.mime_types.INLINE_MIME_TYPES (a migration can't
# import app code that changes); identical to it as of this migration.
INLINE_MIME_TYPES = [
    ...
    with ExitStack() as stack:
        if settings.PRODUCTION:
            log_file: TextIO = stack.enter_context(
                open("/var/log/zulip/migrations_0806_s3_metadata.log", "w")
            )
        else:
            log_file = sys.stderr
            print(file=log_file)
        ...
        for i, attachment in enumerate(all_attachments.iterator()):
            ...
            if not attachment.content_type:
                # Old uploads may have no content-type (null or empty).
                ...
                if not changed:
                    print(f">> {attachment.path_id}:")
                print(f"  content_type: {attachment.content_type!r}")
            ...
            # ImageAttachment.content_type is denormalized from
            # Attachment.content_type (migration 0660); keep still-empty
            # rows in sync now that we have a real value.
            ImageAttachment.objects.filter(path_id=attachment.path_id).filter(
                Q(content_type__isnull=True) | Q(content_type="")
            ).update(content_type=attachment.content_type)
            ...
            content_disposition = (
                content_disposition_header(is_attachment, attachment.file_name) or "inline"
            )

            # STANDARD objects have no storage-class header, so boto3 returns
            # None (the stub says str); treat as "STANDARD" so we skip them.
            # See https://github.com/boto/boto3/issues/437.
            s3_storage_class = cast("str | None", s3_object.storage_class) or "STANDARD"
            ...
            if s3_object.metadata != metadata:
                ...
                pass
            elif s3_storage_class != settings.S3_UPLOADS_STORAGE_CLASS:
                pass
            else:
                continue
...
class Migration(migrations.Migration):
    # One slow S3 request per attachment, so don't hold a transaction
    # open for the whole run; it's idempotent and safe to re-run.
    atomic = False
