...
def do_bulk_backfill_extra_data(
    ...
    for audit_log_entry in python_valued_audit_log_entries:
        ...
        new_value = ast.literal_eval(audit_log_entry.extra_data)  # type: ignore[attr-defined] # The migration cannot depend on zerver.models, which contains the real type of the RealmAuditLog model, so it cannot be properly typed.
        if old_value != {} and old_value != new_value:
            inconsistent_extra_data_json.append((audit_log_entry.id, old_value, new_value))  # type: ignore[attr-defined] # The migration cannot depend on zerver.models, which contains the real type of the RealmAuditLog model, so it cannot be properly typed.
        audit_log_entry.extra_data_json = new_value  # type: ignore[attr-defined] # The migration cannot depend on zerver.models, which contains the real type of the RealmAuditLog model, so it cannot be properly typed.
