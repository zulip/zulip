...
def set_stream_subscribe_count(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    # Here we compute per-stream subscriber_count in the sub-query which returns a stream_subscribers_table,
    ...
    query = sql.SQL(
        """UPDATE {stream_table} AS stream
            SET subscriber_count = stream_subscribers_table.n_subscribers
            FROM (
                SELECT recipient.type_id AS stream_id,
                COUNT(subscription.user_profile_id) AS n_subscribers
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("zerver", "0703_stream_subscriber_count"),
    ]
