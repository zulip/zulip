...
class JitsiVideoCallTest(ZulipTestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.user = self.example_user("hamlet")
        # Realm uses the server default (settings.JITSI_SERVER_URL =
        # https://jitsi.example.com in the test settings), which is a
        # prerequisite for JWT to be minted.
    ...
    def test_get_effective_jitsi_server_url(self) -> None:
        from zerver.lib.video_calls import get_effective_jitsi_server_url

        # Realm override wins and is stripped of trailing slash.
        self.user.realm.jitsi_server_url = "https://custom.jitsi.example.com/"
        self.user.realm.save()
        self.assertEqual(
            get_effective_jitsi_server_url(self.user.realm),
            "https://custom.jitsi.example.com",
        )

        # No realm override -> fall back to the server default (stripped).
        self.user.realm.jitsi_server_url = None
        self.user.realm.save()
        ...
        # No realm override and no server default -> None.
        with self.settings(JITSI_SERVER_URL=None):
            self.assertIsNone(get_effective_jitsi_server_url(self.user.realm))
    ...
    def test_create_jitsi_url_custom_realm_url_disables_jwt(self) -> None:
        # A realm that overrides jitsi_server_url points at a Jitsi
        # server the operator's secrets don't belong to, so JWT must
        # not be minted for it even when the server-side secrets are
        # set.
        self.user.realm.jitsi_server_url = "https://custom.jitsi.example.com"
        self.user.realm.save()
        response = self.client_get(
    ...
    def test_join_jitsi_custom_realm_url_disables_jwt(self) -> None:
        # A realm that overrides jitsi_server_url points at a Jitsi
        # server the operator's secrets don't belong to, so JWT must
        # not be minted for it even when the server-side secrets are
        # set.
    ...
    def test_server_jitsi_jwt_configured_state(self) -> None:
        # Server default URL + secrets → server_jitsi_jwt_configured is true.
        state = fetch_initial_state_data(self.user, realm=self.user.realm, event_types=["realm"])
        self.assertTrue(state["server_jitsi_jwt_configured"])

        # Missing either secret → false.
        with self.settings(JITSI_SERVER_APP_ID=None):
            state = fetch_initial_state_data(
                self.user, realm=self.user.realm, event_types=["realm"]
            )
            self.assertFalse(state["server_jitsi_jwt_configured"])
