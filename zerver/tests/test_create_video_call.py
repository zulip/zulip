...
class JitsiVideoCallTest(ZulipTestCase):
    ...
    def test_jitsi_jwt_enabled_flag_tracks_realm_url_override(self) -> None:
        # Server default URL + secrets → JWT enabled.
        state = fetch_initial_state_data(self.user, realm=self.user.realm, event_types=["realm"])
        self.assertTrue(state["jitsi_jwt_enabled"])

        # Realm overrides URL → JWT disabled (the operator's secrets do
        # not belong to the realm's custom Jitsi server).
        ...
        state = fetch_initial_state_data(self.user, realm=self.user.realm, event_types=["realm"])
        self.assertFalse(state["jitsi_jwt_enabled"])
