...
class ConstructorGroupsVideoCallTest(ZulipTestCase):
    ...
    def test_get_existing_room(self) -> None:
        """Test that existing room is returned with 200 status"""
        responses.add(
            responses.POST,
    ...
    def test_create_new_room(self) -> None:
        """Test that new room is created with 201 status"""
        responses.add(
            responses.POST,
            f"{self.base_api_url}/room/default",
            json=self.mock_valid_room,
    ...
    def test_request_contains_correct_data(self) -> None:
        """Test that request contains correct room name and fallback name"""
        responses.add(
            responses.POST,
            f"{self.base_api_url}/room/default",
            json=self.mock_valid_room,
        ...
        self.assertEqual(
            request_body["name"], f"{self.user_profile.full_name}'s Zulip Videoconferencing Room"
        )
        self.assertEqual(
            request_body["fallback_name"],
            f"{self.user_profile.full_name}-{self.user_profile.id}'s Zulip Videoconferencing Room",
        )

    def test_unsupported_http_method(self) -> None:
        """Test that unsupported HTTP methods raise an error"""
