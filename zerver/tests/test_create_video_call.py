...
class GaleneVideoCallTest(ZulipTestCase):
    ...
    @responses.activate
    def test_create_galene_subgroup_video_call_success(self) -> None:
        responses.add(
            responses.PUT,
        ...
        with mock.patch("zerver.views.video_calls.uuid.uuid4", return_value=call_id):
            response = self.client_post(
                ...
                {"group_name": "devel", "subgroup_name": "team-check-in"},
            )
    ...
    @responses.activate
    def test_create_galene_no_subgroup_video_call_success(self) -> None:
        responses.add(
            responses.PUT,
            urljoin(self.galene_api_url, ".groups/devel"),
            status=201,
        ...
        with mock.patch("zerver.views.video_calls.uuid.uuid4", return_value=call_id):
            response = self.client_post(
                "/json/calls/galene/create",
                {"group_name": "devel", "subgroup_name": ""},
            )
    ...
    @responses.activate
    def test_create_galene_subgroup_with_url_reserved_chars(self) -> None:
        # `#`, `?`, `/`, and whitespace in the subgroup_name must be
        # percent-encoded before being spliced into a Galène URL path —
        # otherwise they turn the path into a fragment / query string /
        # extra path segments.
        subgroup_name = "design #1 / rev?"
        responses.add(
            responses.PUT,
            urljoin(self.galene_api_url, ".groups/devel"),
            status=201,
        ...
        call_id = uuid.UUID("00000000-0000-0000-0000-000000000003")
        with mock.patch("zerver.views.video_calls.uuid.uuid4", return_value=call_id):
            response = self.client_post(
                "/json/calls/galene/create",
                {"group_name": "devel", "subgroup_name": subgroup_name},
            )
        ...
        signed = self.signer.sign_object(
            {
                "name": f"devel/{quote(subgroup_name, safe=',')}-{call_id}",
                "is_dm": False,
            }
        )
        self.assertEqual(url, "/calls/galene/join?galene_call_join_details=" + signed)
    ...
    @responses.activate
    def test_create_galene_group_with_url_reserved_chars(self) -> None:
        # Same concern for the group name: `/` would otherwise inject an
        # unintended Galène subgroup level.
        group_name = "dev/ops"
        responses.add(
            responses.PUT,
            urljoin(self.galene_api_url, f".groups/{quote(group_name, safe='')}"),
            status=201,
        )

        call_id = uuid.UUID("00000000-0000-0000-0000-000000000004")
        with mock.patch("zerver.views.video_calls.uuid.uuid4", return_value=call_id):
            response = self.client_post(
                "/json/calls/galene/create",
                {"group_name": group_name, "subgroup_name": "standup"},
            )
        ...
        signed = self.signer.sign_object(
            {
                "name": f"{quote(group_name, safe='')}/standup-{call_id}",
                "is_dm": False,
            }
        )
        self.assertEqual(url, "/calls/galene/join?galene_call_join_details=" + signed)
    ...
    def test_create_galene_dm_video_call_success(self) -> None:
        # For a DM call the client sends group_name="dm" and
        # subgroup_name=<sorted comma-joined participant ids> (produced
        # by web/src/people.ts::concat_direct_message_group); the server
        # splices those into the URL as-is. is_dm=True in the signed
        # blob grants op permissions to every joiner (see
        # test_join_galene_dm_video_call_success).
        ...
        participant_ids = sorted([self.user.id, othello.id, cordelia.id])
        joined = ",".join(str(uid) for uid in participant_ids)

        call_id = uuid.UUID("00000000-0000-0000-0000-000000000010")
        with mock.patch("zerver.views.video_calls.uuid.uuid4", return_value=call_id):
            response = self.client_post(
                "/json/calls/galene/create",
                {"group_name": "dm", "subgroup_name": joined, "is_dm": "true"},
            )

        json = self.assert_json_success(response)
        signed = self.signer.sign_object(
            {
                "name": f"dm/{joined}-{call_id}",
                "is_dm": True,
            }
        )
        self.assertEqual(json["url"], "/calls/galene/join?galene_call_join_details=" + signed)

    def test_create_galene_rejects_missing_group(self) -> None:
        response = self.client_post("/json/calls/galene/create", {})
        self.assert_json_error(response, "Missing 'group_name' argument")

    @responses.activate
    def test_join_galene_subgroup_with_url_reserved_chars(self) -> None:
        # A group_name from create_galene_call is already percent-encoded, so
        # join_galene_call can splice it into token_url and call_url as-is.
        subgroup_name = "design #1 / rev?"
        encoded_group = f"devel/{quote(subgroup_name, safe='')}-uuid"
        responses.add(
            responses.POST,
            urljoin(self.galene_api_url, f".groups/{encoded_group}/.tokens/"),
            headers={
        ...
        signed = self.signer.sign_object({"name": encoded_group, "is_dm": False})
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            f"https://galene.example.org/group/{encoded_group}/?token=nsdofj7hsdf8437",
            response["Location"],
        )

    @responses.activate
    def test_join_galene_subgroup_video_call_success(self) -> None:
        responses.add(
            responses.POST,
    ...
    @responses.activate
    def test_join_galene_channel_call_ignores_dm_shaped_group_name(self) -> None:
        # Regression test: even if the signed group_name starts with "dm/",
        # a non-moderator must not be granted op permissions unless the
        # signed is_dm bit says so. This is what the old
        # `group_name.startswith("dm/")` check let a member spoof.
        responses.add(
            responses.POST,
            urljoin(self.galene_api_url, ".groups/dm/1,2,3/.tokens/"),
            headers={"location": "tok"},
            status=200,
        )

        signed = self.signer.sign_object({"name": "dm/1,2,3", "is_dm": False})
        url = append_url_query_string("/calls/galene/join", "galene_call_join_details=" + signed)
        ...
        assert responses.calls[-1].request.body is not None
        payload = orjson.loads(responses.calls[-1].request.body)
        self.assertEqual(payload["permissions"], ["present", "message"])
    ...
    @responses.activate
    def test_join_galene_moderator_gets_op_on_channel_call(self) -> None:
        # Zulip moderators get op regardless of is_dm.
        ...
        signed = self.signer.sign_object({"name": "devel/team-check-in", "is_dm": False})
        url = append_url_query_string("/calls/galene/join", "galene_call_join_details=" + signed)
        ...
        assert responses.calls[-1].request.body is not None
        payload = orjson.loads(responses.calls[-1].request.body)
        self.assertEqual(payload["permissions"], ["op", "present", "message", "caption", "token"])
