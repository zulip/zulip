...
from zerver.lib.types import Invitee, UserGroupMembersData
...
class ActivateTest(ZulipTestCase):
    ...
    def test_updating_group_permissions_when_deactivating(self) -> None:
        ...
        # Settings of deactivated channels and groups are also updated, so that
        # the deactivated user is not left as a member of any anonymous group.
        archived_stream = self.subscribe(hamlet, "archived_stream")
        setting_group_member_dict = UserGroupMembersData(
            direct_members=[hamlet.id], direct_subgroups=[]
        )
        do_change_stream_group_based_setting(
        ...
        do_deactivate_stream(archived_stream, acting_user=iago)

        deactivated_group = check_add_user_group(realm, "deactivated_group", [], acting_user=iago)
        setting_group = self.create_or_update_anonymous_group_for_setting(
            [hamlet], [moderators_group]
        )
        do_change_user_group_permission_setting(
            deactivated_group, "can_add_members_group", setting_group, acting_user=iago
        )
        do_deactivate_user_group(deactivated_group, acting_user=iago)
        ...
        archived_stream.refresh_from_db()
        self.assertEqual(archived_stream.can_administer_channel_group_id, nobody_group.id)

        deactivated_group.refresh_from_db()
        self.assertEqual(deactivated_group.can_add_members_group_id, moderators_group.id)
...
class RecipientInfoTest(ZulipTestCase):
    def test_stream_recipient_info(self) -> None:
        ...
        # Add a message-triggered bot.
        message_triggered_bot = do_create_user(
            email="message-triggered-bot@zulip.com",
            password="",
            realm=realm,
            full_name="",
        ...
        info = get_recipient_info(
            ...
            sender_id=hamlet.id,
            stream_topic=stream_topic,
            possibly_mentioned_user_ids={message_triggered_bot.id},
        )
        self.assertEqual(
            info.message_triggered_bot_tuples,
            [
                (message_triggered_bot.id, UserProfile.EMBEDDED_BOT),
            ],
        )
        ...
        info = get_recipient_info(
            ...
            sender_id=hamlet.id,
            stream_topic=stream_topic,
            possibly_mentioned_user_ids={message_triggered_bot.id, normal_bot.id},
        )
        self.assertEqual(info.default_bot_user_ids, {normal_bot.id})
        self.assertEqual(info.all_bot_user_ids, {normal_bot.id, message_triggered_bot.id})
