...
class GetProfileTest(ZulipTestCase):
    ...
    def test_get_users_involved_in_dms_with_dm_group_recipients(self) -> None:
        ...
        users_involved = get_users_involved_in_dms_with_target_users(
            [hamlet, othello], realm, include_deactivated_users=False
        )
        # Since both target_users share a DMG, each one should be a partner of the other.
        self.assertIn(othello.id, users_involved[hamlet.id])
        self.assertIn(hamlet.id, users_involved[othello.id])
