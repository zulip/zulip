...
from zerver.lib.mcp.tools import DEFAULT_NUM_BEFORE
...
class MCPTransportTest(MCPEndpointTestCase):
    def test_get_method_not_allowed(self) -> None:
        # MCP clients open a GET to request an SSE stream, which this
        # stateless server does not offer; they get a JSON 405, not the
        # HTML error page an ordinary non-API route would return.
        with self.assertLogs(level="WARNING") as mock_warning:
            result = self.client_get("/mcp")
        self.assert_json_error(result, "Method Not Allowed", status_code=405)
        self.assertEqual(mock_warning.output, ["WARNING:root:Method Not Allowed (GET): /mcp"])

    def test_trailing_slash_is_the_same_endpoint(self) -> None:
        hamlet = self.example_user("hamlet")
        # The URL is typed by hand into an agent's configuration, so a
        # trailing slash has to work rather than return a 404 page.
        ...
        self.assertEqual(orjson.loads(result.content), {"jsonrpc": "2.0", "id": 1, "result": {}})
    ...
    def test_realm_setting_disabled(self) -> None:
        hamlet = self.example_user("hamlet")
        do_set_realm_property(hamlet.realm, "enable_mcp_read_access", False, acting_user=None)
        result = self.mcp_request(hamlet, {"jsonrpc": "2.0", "id": 1, "method": "ping"})
    ...
    def test_bearer_scheme_is_case_insensitive(self) -> None:
        ...
        for scheme in ["Bearer", "bearer", "BEARER"]:
            result = self.client_post(
                ...
                orjson.dumps({"jsonrpc": "2.0", "id": 1, "method": "ping"}).decode(),
                content_type="application/json",
                HTTP_AUTHORIZATION=f"{scheme} {hamlet.api_key}",
            )
            self.assertEqual(result.status_code, 200, scheme)

    def test_authorization_header_without_a_key(self) -> None:
        hamlet = self.example_user("hamlet")
        for header in ["Bearer", "Bearer ", "Token abc", hamlet.api_key]:
            result = self.client_post(
                "/mcp",
                orjson.dumps({"jsonrpc": "2.0", "id": 1, "method": "ping"}).decode(),
                content_type="application/json",
    ...
    def test_client_recorded(self) -> None:
        hamlet = self.example_user("hamlet")
        # An MCP client sends a user agent of its own, which must not
        # displace ZulipMCP in the activity we record.
        with mock.patch("zerver.decorator.queue_json_publish_rollback_unsafe") as mock_publish:
            self.client_post(
                "/mcp",
                orjson.dumps({"jsonrpc": "2.0", "id": 1, "method": "ping"}).decode(),
                content_type="application/json",
        ...
        (queue_name, event) = mock_publish.call_args.args[:2]
        self.assertEqual(queue_name, "user_activity")
        self.assertEqual(event["user_profile_id"], hamlet.id)
        self.assertEqual(event["client_id"], Client.objects.get(name="ZulipMCP").id)

    def test_tool_name_recorded_in_request_log(self) -> None:
        hamlet = self.example_user("hamlet")
        with self.assertLogs("zulip.requests", level="INFO") as mock_info:
            self.call_tool(hamlet, "list_channels", {})
            self.call_method(hamlet, "tools/call", {"name": "no_such_tool", "arguments": {}})
            self.call_method(hamlet, "ping")
        self.assert_length(mock_info.output, 3)
        self.assertIn(" [list_channels] ", mock_info.output[0])
        self.assertIn(" [unknown tool] ", mock_info.output[1])
        # Requests that are not tool calls say nothing extra.
        self.assertNotIn(" [", mock_info.output[2].split(" /mcp ")[1])
...
class MCPProtocolTest(MCPEndpointTestCase):
    def test_invalid_json_rpc_request(self) -> None:
        ...
        for payload in [
            ...
            {"jsonrpc": "2.0", "id": 5},
            # JSON-RPC requires a string method, so this is a malformed
            # request rather than a method we do not have.
            {"jsonrpc": "2.0", "id": 5, "method": {"not": "a string"}},
        ]:
    ...
    def test_tools_call_name_is_not_a_string(self) -> None:
        hamlet = self.example_user("hamlet")
        # A list is not hashable, so anything looking this up in the tool
        # registry without checking the type first raises rather than
        # answering.
        for name in [None, 5, ["list_channels"], {"name": "list_channels"}]:
            result = self.call_method(hamlet, "tools/call", {"name": name, "arguments": {}})
            self.assertEqual(result["error"]["code"], -32602)
            self.assertEqual(result["error"]["message"], "Tool name is missing or not a string.")

    def test_tools_call_missing_params(self) -> None:
        hamlet = self.example_user("hamlet")
        result = self.call_method(hamlet, "tools/call")
        self.assertEqual(result["error"]["code"], -32602)
...
class MCPMessageRangeTest(MCPEndpointTestCase):
    ...
    def test_anchor_by_date(self) -> None:
        hamlet = self.example_user("hamlet")
        self.subscribe(hamlet, "mcp-dates")
        narrow = [{"operator": "channel", "operand": "mcp-dates"}]
        sent_on = [
            datetime(2025, 4, 17, 9, 0, tzinfo=timezone.utc),
            datetime(2025, 4, 18, 9, 0, tzinfo=timezone.utc),
            datetime(2025, 4, 19, 9, 0, tzinfo=timezone.utc),
        ]
        message_ids = []
        for date_sent in sent_on:
            with time_machine.travel(date_sent, tick=False):
                message_ids.append(
                    self.send_stream_message(hamlet, "mcp-dates", content=date_sent.isoformat())
                )

        # A date alone means midnight, so the anchor is that day's message.
        payload = self.call_get_messages(
            hamlet, narrow, anchor="date", anchor_date="2025-04-18", num_before=0, num_after=10
        )
        self.assertEqual([message["id"] for message in payload["messages"]], message_ids[1:])
        self.assertEqual(payload["anchor"], message_ids[1])

        # A datetime past that day's message anchors on the following one.
        payload = self.call_get_messages(
            hamlet,
            narrow,
            anchor="date",
            anchor_date="2025-04-18T12:00:00Z",
            num_before=0,
            num_after=10,
        )
        self.assertEqual([message["id"] for message in payload["messages"]], message_ids[2:])

        # And reading backwards from a date is how an agent catches up on
        # what it missed, which is the point of having this at all.
        payload = self.call_get_messages(
            hamlet, narrow, anchor="date", anchor_date="2025-04-18", num_before=10, num_after=0
        )
        self.assertEqual([message["id"] for message in payload["messages"]], message_ids[:2])

    def test_anchor_by_date_errors(self) -> None:
        hamlet = self.example_user("hamlet")
        error_text = self.call_tool_expecting_error(hamlet, "get_messages", {"anchor": "date"})
        self.assertEqual(error_text, "Missing 'anchor_date' argument.")

        error_text = self.call_tool_expecting_error(
            hamlet, "get_messages", {"anchor": "date", "anchor_date": "last Tuesday"}
        )
        self.assertEqual(error_text, "anchor_date is not an ISO 8601 datetime string")

        # Like GET /messages, an anchor_date without anchor="date" is
        # ignored rather than being an error.
        payload = self.call_get_messages(hamlet, [], anchor_date="2025-04-18", num_before=1)
        self.assertTrue(payload["found_newest"])
...
class MCPGuestAccessTest(MCPEndpointTestCase):
    """Guests see a strict subset of the organization, and each tool has to
    respect that on its own; none of them take a "who is asking" shortcut.
    """

    def test_guest_sees_only_subscribed_channels(self) -> None:
        polonius = self.example_user("polonius")
        self.assertTrue(polonius.is_guest)
        self.make_stream("mcp-guest-public")
        self.subscribe(polonius, "mcp-guest-subscribed")

        payload = self.call_tool_expecting_success(polonius, "list_channels", {})
        names = {channel["name"] for channel in payload["channels"]}
        self.assertIn("mcp-guest-subscribed", names)
        # A public channel is visible to members, but not to a guest who is
        # not subscribed to it.
        self.assertNotIn("mcp-guest-public", names)

    def test_guest_cannot_read_an_unsubscribed_public_channel(self) -> None:
        polonius = self.example_user("polonius")
        hamlet = self.example_user("hamlet")
        self.subscribe(hamlet, "mcp-guest-hidden")
        self.send_stream_message(hamlet, "mcp-guest-hidden", content="members only, in effect")

        error_text = self.call_tool_expecting_error(
            polonius, "list_topics", {"channel": "mcp-guest-hidden"}
        )
        self.assertEqual(error_text, "Invalid channel name 'mcp-guest-hidden'")

        payload = self.call_get_messages(
            polonius, [{"operator": "channel", "operand": "mcp-guest-hidden"}]
        )
        self.assertEqual(payload["messages"], [])

        # The same narrow works for a member, so the empty result above is
        # the guest restriction rather than a broken query.
        payload = self.call_get_messages(
            hamlet, [{"operator": "channel", "operand": "mcp-guest-hidden"}]
        )
        self.assert_length(payload["messages"], 1)


class MCPRestParityTest(MCPEndpointTestCase):
    """get_messages is documented as mirroring GET /messages, which is what
    lets it inherit that endpoint's access control and compatibility
    commitments rather than growing its own. Asserting the two agree keeps
    that from quietly becoming false.
    """

    def assert_matches_rest(self, user_profile: UserProfile, **arguments: Any) -> None:
        tool_result = self.call_tool(user_profile, "get_messages", arguments)

        rest_parameters: dict[str, Any] = {
            # The tool's defaults, which GET /messages requires explicitly.
            "anchor": "newest",
            "num_before": DEFAULT_NUM_BEFORE,
            "num_after": 0,
            **arguments,
        }
        for name in ("narrow", "include_anchor"):
            if name in rest_parameters:
                rest_parameters[name] = orjson.dumps(rest_parameters[name]).decode()
        rest_response = orjson.loads(
            self.api_get(user_profile, "/api/v1/messages", rest_parameters).content
        )

        if tool_result["isError"]:
            self.assertEqual(rest_response["result"], "error")
            self.assertEqual(tool_result["content"][0]["text"], rest_response["msg"])
            return

        self.assertEqual(rest_response["result"], "success")
        payload = tool_result["structuredContent"]
        self.assertEqual(
            [message["id"] for message in payload["messages"]],
            [message["id"] for message in rest_response["messages"]],
        )
        for field in ("anchor", "found_anchor", "found_oldest", "found_newest", "history_limited"):
            self.assertEqual(payload[field], rest_response[field], field)

    def test_get_messages_agrees_with_the_rest_endpoint(self) -> None:
        hamlet = self.example_user("hamlet")
        cordelia = self.example_user("cordelia")
        self.subscribe(hamlet, "mcp-parity")
        self.subscribe(cordelia, "mcp-parity")
        message_ids = [
            self.send_stream_message(
                hamlet if index % 2 else cordelia,
                "mcp-parity",
                content=f"message {index}",
                topic_name=f"topic {index % 3}",
            )
            for index in range(12)
        ]
        self.make_stream("mcp-parity-private", invite_only=True)
        self.subscribe(cordelia, "mcp-parity-private")
        self.send_stream_message(cordelia, "mcp-parity-private", content="not for hamlet")
        self.send_personal_message(cordelia, hamlet, content="a direct message")

        channel: list[dict[str, Any]] = [{"operator": "channel", "operand": "mcp-parity"}]
        middle = message_ids[6]
        cases: list[dict[str, Any]] = [
            # The narrow vocabulary, including a term the user cannot use.
            {},
            {"narrow": channel},
            {"narrow": [*channel, {"operator": "topic", "operand": "topic 1"}]},
            {"narrow": [*channel, {"operator": "topic", "operand": "topic 1", "negated": True}]},
            {"narrow": [*channel, {"operator": "sender", "operand": cordelia.id}]},
            {"narrow": [{"operator": "is", "operand": "dm"}]},
            {"narrow": [{"operator": "channel", "operand": "mcp-parity-private"}]},
            # Every way of positioning the range against the anchor.
            {"narrow": channel, "anchor": middle, "num_before": 2, "num_after": 2},
            {"narrow": channel, "anchor": "oldest", "num_before": 0, "num_after": 3},
            {"narrow": channel, "anchor": "first_unread", "num_before": 0, "num_after": 5},
            {"narrow": channel, "anchor": middle, "include_anchor": False, "num_before": 3},
            {"narrow": channel, "anchor": middle, "num_before": 0, "num_after": 0},
            {
                "narrow": channel,
                "anchor": "date",
            ...
            },
            {
                "narrow": channel,
                "anchor": "date",
                "anchor_date": "2100-01-01T12:34:56Z",
                "num_before": 5,
                "num_after": 0,
            },
            # And the errors, which an agent sees as tool errors.
            {"narrow": [{"operator": "channel", "operand": "mcp-parity-nonexistent"}]},
            {"narrow": [{"operator": "bogus_operator", "operand": "x"}]},
            {"num_before": MAX_MESSAGES_PER_FETCH, "num_after": 1},
            {"num_before": 1, "num_after": 1, "include_anchor": False},
            {"anchor": "date"},
            {"anchor": "date", "anchor_date": "not a date"},
        ]
        for arguments in cases:
            with self.subTest(arguments=arguments):
                self.assert_matches_rest(hamlet, **arguments)
...
class MCPDirectoryToolsTest(MCPEndpointTestCase):
    ...
    def test_list_channels_filtered_and_bounded(self) -> None:
        hamlet = self.example_user("hamlet")
        for index in range(3):
            self.subscribe(hamlet, f"mcp-bounded-{index}")

        payload = self.call_tool_expecting_success(
            hamlet, "list_channels", {"query": "mcp-bounded"}
        )
        self.assert_length(payload["channels"], 3)
        self.assertEqual(payload["total_count"], 3)
        self.assertTrue(payload["found_all"])

        # The query matches descriptions too, so an agent can find a
        # channel it only knows the purpose of.
        denmark = get_stream("Denmark", hamlet.realm)
        do_change_stream_description(denmark, "Hamlet's homeland deliberations", acting_user=hamlet)
        payload = self.call_tool_expecting_success(
            hamlet, "list_channels", {"query": "DELIBERATIONS"}
        )
        self.assertEqual([channel["name"] for channel in payload["channels"]], ["Denmark"])

        payload = self.call_tool_expecting_success(
            hamlet, "list_channels", {"query": "mcp-bounded", "limit": 2}
        )
        self.assert_length(payload["channels"], 2)
        # A truncated result still counts every match, so an agent knows
        # what raising the limit would cost before it spends a round trip.
        self.assertEqual(payload["total_count"], 3)
        self.assertFalse(payload["found_all"])

    def test_list_topics_bounded(self) -> None:
        hamlet = self.example_user("hamlet")
        self.subscribe(hamlet, "mcp-many-topics")
        for index in range(3):
            self.send_stream_message(
                hamlet, "mcp-many-topics", content="hi", topic_name=f"topic {index}"
            )

        payload = self.call_tool_expecting_success(
            hamlet, "list_topics", {"channel": "mcp-many-topics"}
        )
        self.assert_length(payload["topics"], 3)
        self.assertEqual(payload["total_count"], 3)
        self.assertTrue(payload["found_all"])

        # Truncation keeps the newest topics, which is what "recent" means.
        payload = self.call_tool_expecting_success(
            hamlet, "list_topics", {"channel": "mcp-many-topics", "limit": 2}
        )
        self.assertEqual([topic["name"] for topic in payload["topics"]], ["topic 2", "topic 1"])
        self.assertEqual(payload["total_count"], 3)
        self.assertFalse(payload["found_all"])
    ...
    def test_get_users_reports_an_incomplete_list(self) -> None:
        hamlet = self.example_user("hamlet")
        payload = self.call_tool_expecting_success(hamlet, "get_users", {"limit": 1})
        self.assert_length(payload["users"], 1)
        self.assertFalse(payload["found_all"])

        complete = self.call_tool_expecting_success(hamlet, "get_users", {"limit": 200})
        self.assertTrue(complete["found_all"])
        # The reported total is what an agent would pass as the limit to
        # fetch the rest, so it has to count every match rather than the
        # page that was returned.
        self.assertEqual(payload["total_count"], len(complete["users"]))

        # A limit exactly matching the number of matches is complete, not
        # merely full; counting the matches is what settles that boundary.
        matches = self.call_tool_expecting_success(hamlet, "get_users", {"query": "cordelia"})
        self.assertTrue(matches["found_all"])
        exact = self.call_tool_expecting_success(
            hamlet, "get_users", {"query": "cordelia", "limit": len(matches["users"])}
        )
        self.assertTrue(exact["found_all"])
    ...
    def test_get_users_unrestricted_for_members(self) -> None:
        ...
        hamlet = self.example_user("hamlet")
        polonius = self.example_user("polonius")
        # A shared subscription would make this user accessible to
        # everyone, which is the case this test needs to avoid.
    ...
    def test_result_size_does_not_change_the_query_count(self) -> None:
        """These tools return a row per user, channel or topic, so a query
        per row would make them cost more as an organization grows. What
        this pins is that the counts do not move with the data, not what
        the counts are.
        """
        hamlet = self.example_user("hamlet")
        realm = get_realm("zulip")
        ...
        def query_counts() -> list[int]:
            ...
            for name, arguments in (
                ("get_users", {"limit": 500}),
                ("list_channels", {"limit": 500}),
                ("list_topics", {"channel": "Denmark", "limit": 500}),
            ):
                with queries_captured() as queries:
                    self.call_tool_expecting_success(hamlet, name, arguments)
                counts.append(len(queries))
            return counts

        # The first MCP request creates the ZulipMCP client row; measuring
        # after that keeps these counts about the tools themselves.
        self.call_tool_expecting_success(hamlet, "get_users", {})

        counts_before = query_counts()
        for index in range(5):
            user = do_create_user(
                f"agent{index}@zulip.com",
                "password",
                realm,
                f"Agent {index}",
                acting_user=None,
            )
            self.make_stream(f"channel {index}")
            self.subscribe(user, "Denmark")
            self.send_stream_message(user, "Denmark", topic_name=f"topic {index}")
        self.assertEqual(query_counts(), counts_before)
