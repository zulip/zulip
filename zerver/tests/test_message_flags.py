...
class UnstarStreamMessagesTest(ZulipTestCase):
    def test_do_unstar_inaccessible_stream_messages(self) -> None:
        """
        Test that do_unstar_inaccessible_stream_messages removes the starred flag
        ...
        Subscription.objects.filter(user_profile=hamlet, recipient_id=stream.recipient_id).update(
            active=False
        )
        with (
            mock.patch("zerver.actions.message_flags.UNSTAR_MESSAGES_BATCH_SIZE", 1),
            self.capture_send_event_calls(expected_num_events=2) as events,
        ):
            count = do_unstar_inaccessible_stream_messages(hamlet, [stream.recipient_id])

        self.assertEqual(count, 2)

        event_message_ids: list[int] = []
        for event in events:
            self.assertEqual(event["event"]["type"], "update_message_flags")
            self.assertEqual(event["event"]["op"], "remove")
            self.assertEqual(event["event"]["operation"], "remove")
            self.assertEqual(event["event"]["flag"], "starred")
            self.assertFalse(event["event"]["all"])
            self.assertEqual(event["users"], [hamlet.id])
            event_message_ids.extend(event["event"]["messages"])
        self.assertCountEqual(event_message_ids, [message_id1, message_id2])
    ...
    def test_do_unstar_inaccessible_stream_messages_no_starred(self) -> None:
        """
        Test that do_unstar_inaccessible_stream_messages is a no-op when there are
    ...
    def test_unsubscribe_private_stream_unstars_messages(self) -> None:
        ...
        um.refresh_from_db()
        self.assertFalse(um.flags.starred)
        ...
        self.assertEqual(events[0]["event"]["messages"], [message_id])
    ...
    def test_unsubscribe_retaining_group_content_access_does_not_queue_unstar(self) -> None:
        hamlet = self.example_user("hamlet")
        iago = self.example_user("iago")
        stream_name = "private_stream"
        stream = self.make_stream(stream_name, invite_only=True)
        ...
        self.assert_length(unstar_events, 0)
