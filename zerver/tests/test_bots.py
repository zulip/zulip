...
class BotTest(ZulipTestCase, UploadSerializeMixin):
    ...
    def test_patch_bot_type(self) -> None:
        ...
        result = self.client_patch(f"/json/bots/{bot_profile.id}", req)
        self.assert_json_error(result, "Missing embedded_bot_service_name.")

        req_embed = dict(
            bot_type=UserProfile.EMBEDDED_BOT,
            embedded_bot_service_name=orjson.dumps("not_existing_service").decode(),
        )
        result = self.client_patch(f"/json/bots/{bot_profile.id}", req_embed)
        self.assert_json_error(result, "Invalid embedded_bot_service_name.")

        req_embed = dict(
            bot_type=UserProfile.EMBEDDED_BOT,
            embedded_bot_service_name=orjson.dumps("converter").decode(),
        )
        result = self.client_patch(f"/json/bots/{bot_profile.id}", req_embed)
        self.assert_json_success(result)
        bot_profile = self.get_bot_user(email)
    ...
    def test_patch_embedded_bot_service_name(self) -> None:
        ...
        req = dict(
            bot_type=UserProfile.EMBEDDED_BOT,
            embedded_bot_service_name=orjson.dumps("encrypt").decode(),
        )
        result = self.client_patch(f"/json/bots/{bot_profile.id}", req)
        self.assert_json_success(result)
        [service] = get_bot_services(bot_profile.id)
        self.assertEqual(service.name, "encrypt")
    ...
    def test_patch_bot_config_data(self, mock_validate_config: MagicMock) -> None:
        ...
        bot_info = {
            "bot_type": orjson.dumps(UserProfile.EMBEDDED_BOT).decode(),
            "config_data": orjson.dumps({"key": "87654321"}).decode(),
        }
