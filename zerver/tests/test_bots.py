...
from zerver.models.bots import get_bot_services, get_default_bot_triggers
...
class BotTest(ZulipTestCase, UploadSerializeMixin):
    ...
    def test_patch_outgoing_webhook_bot(self) -> None:
        ...
        bot_info = {
            ...
            "service_triggers": orjson.dumps(get_default_bot_triggers()).decode(),
        }
        result = self.client_post("/json/bots", bot_info)
        self.assert_json_success(result)
        bot_info = {
    ...
    def test_patch_bot_service_triggers(self) -> None:
        self.login("hamlet")
        original_configured_triggers = get_default_bot_triggers()
        bot_info = {
            "full_name": "The Bot of Hamlet",
            "short_name": "hambot",
            "bot_type": UserProfile.OUTGOING_WEBHOOK_BOT,
            "payload_url": orjson.dumps("http://foo.bar.com").decode(),
            "interface_type": Service.GENERIC,
            "service_triggers": orjson.dumps(original_configured_triggers).decode(),
        }
        result = self.client_post("/json/bots", bot_info)
        self.assert_json_success(result)
        bot_user_id = orjson.loads(result.content)["user_id"]
        service = get_bot_services(bot_user_id)[0]
        self.assertEqual(sorted(service.triggers), sorted(original_configured_triggers))
        ...
        result = self.client_patch(f"/json/bots/{self.get_bot_user(email).id}", bot_info)
        self.assert_json_error(result, "The bot triggers are not specified.")
        service = get_bot_services(bot_user_id)[0]
        self.assertEqual(sorted(service.triggers), sorted(original_configured_triggers))

        new_triggers_with_unknown_value = [
            "bogus_trigger",
            "invalid_trigger",
            "invalid_trigger",
        ...
        bot_info = {
            "service_triggers": orjson.dumps(new_triggers_with_unknown_value).decode(),
        }
        result = self.client_patch(f"/json/bots/{self.get_bot_user(email).id}", bot_info)
        self.assert_json_error(
            result, "Invalid bot service trigger(s): bogus_trigger, invalid_trigger"
        )
        service = get_bot_services(bot_user_id)[0]
        # Triggers should be unchanged because the patch failed.
        self.assertNotEqual(sorted(service.triggers), sorted(new_triggers_with_unknown_value))
        self.assertEqual(sorted(service.triggers), sorted(original_configured_triggers))

        # all caps, extra white space, and duplicate trigger. This passes because
        # `validate_bot_triggers` also cleans the input.
        bot_info = {
        ...
        self.assert_json_success(result)
        service = get_bot_services(bot_user_id)[0]
        self.assertEqual(sorted(service.triggers), sorted(get_default_bot_triggers()))
    ...
    def test_create_outgoing_webhook_bot(self, **extras: Any) -> None:
        ...
        result = self.client_post("/json/bots", bot_info)
        self.assert_json_error(result, "Invalid bot service trigger(s): unknown_trigger")
    ...
    def test_create_incoming_webhook_bot_with_service_name_and_with_keys(self) -> None:
        ...
        config_data = get_bot_config(UserProfile.objects.get(id=result["user_id"]))
        self.assertEqual(
    ...
    def test_create_incoming_webhook_bot_with_service_name_and_no_config_options(self) -> None:
        self.login("hamlet")
        result = self.create_bot(
            bot_type=UserProfile.INCOMING_WEBHOOK_BOT, service_name="helloworld"
        )
        config_data = get_bot_config(UserProfile.objects.get(id=result["user_id"]))
    ...
    def test_create_incoming_webhook_bot_without_service_name(self) -> None:
        self.login("hamlet")
        result = self.create_bot(bot_type=UserProfile.INCOMING_WEBHOOK_BOT)
        new_bot = UserProfile.objects.get(id=result["user_id"])
