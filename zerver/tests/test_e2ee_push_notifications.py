...
from zerver.lib.devices import b64encode_token_id
...
class SendPushNotificationTest(E2EEPushNotificationTestCase):
    ...
    def test_invalid_or_expired_token(self) -> None:
        ...
        with (
            ...
            self.assertEqual(
                "INFO:zerver.lib.push_notifications:"
                f"Clearing `push_token_id` for Device rows with the following token IDs based on response from bouncer: ['{b64encode_token_id(registered_device_apple.token_id)}', '{b64encode_token_id(registered_device_android.token_id)}']",
                zerver_logger.output[3],
            )
            self.assertEqual(
                "INFO:zerver.lib.push_notifications:"
