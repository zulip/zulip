...
from zerver.lib.webhooks.common import \
    ...
    INVALID_JSON_MESSAGE, get_http_headers, parse_headers_dict
...
class WebhooksCommonTestCase(ZulipTestCase):
    ...
    def test_get_http_headers_for_success(self, import_module_mock: MagicMock) -> None:

        def fixture_to_headers(fixture_name: str) -> Dict[str, str]:
            # A sample function which would normally perform some extra operations before returning
            # a dictionary corresponding to the fixture name passed. For the sake of an example, we
            # directly return a dictionary regardless of the fixture name.
            return {"key": "value"}

        fake_module = SimpleNamespace(fixture_to_headers=fixture_to_headers)
        import_module_mock.return_value = fake_module

        headers = get_http_headers("some_integration", "complex_fixture")
        self.assertEqual(headers, {"key": "value"})

    def test_get_http_headers_for_non_existant_integration(self) -> None:
        headers = get_http_headers("some_random_nonexistant_integration", "fixture_name")
        self.assertEqual(headers, {})

    @patch("zerver.lib.webhooks.common.importlib.import_module")
    def test_get_http_headers_for_error_handling(self, import_module_mock: MagicMock) -> None:

        fake_module = SimpleNamespace()
        import_module_mock.return_value = fake_module

        with self.assertRaises(AttributeError):
            get_http_headers("some_integration", "simple_fixture")
    ...
    def test_parse_headers_dict_empty_dict(self) -> None:
        self.assertEqual(parse_headers_dict({}), {})

    def test_parse_headers_dict_for_headers_with_spaces(self) -> None:
        with self.assertRaises(ValueError):
            parse_headers_dict({"Header With Spaces": "val"})
