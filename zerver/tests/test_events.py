...
class NormalActionsTest(BaseAction):
    ...
    def test_navigation_views_events(self) -> None:
        with self.verify_action() as events:
            do_add_navigation_view(self.user_profile, fragment="inbox", is_pinned=True, name=None)
        check_navigation_view_add("events[0]", events[0])

        do_add_navigation_view(self.user_profile, fragment="recent", is_pinned=True, name=None)
        with self.verify_action() as events:
            do_update_navigation_view(self.user_profile, fragment="recent", is_pinned=False)
        check_navigation_view_update("events[0]", events[0])

        with self.verify_action() as events:
            do_remove_navigation_view(self.user_profile, "recent")
