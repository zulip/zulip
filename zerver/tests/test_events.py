...
property_schema_checker = check_events_dict([
    ('type', equals('realm')),
    ('op', equals('update')),
...
class RealmPropertyActionTest(BaseAction):
    def do_set_realm_property_test(self, name: str) -> None:
        ...
        for val in vals[1:]:
            ...
            property_schema_checker('events[0]', events[0])
...
class BotActionTest(BaseAction):
    ...
    def check_update_fields(self,
                            ...
                            i: int) -> None:
        event = events[i]
        realm_bot_update_schema(f'events[{i}]', event)
        self.assertEqual(set(event['bot'].keys()), {'user_id', field_name})
    ...
    def test_change_bot_default_all_public_streams(self) -> None:
        ...
        events = self.verify_action(action)
        self.check_update_fields('default_all_public_streams', events, 0)
    ...
    def test_change_bot_default_sending_stream(self) -> None:
        ...
        events = self.verify_action(action)
        self.check_update_fields('default_sending_stream', events, 0)
        self.assertEqual(events[0]['bot']['default_sending_stream'], 'Rome')

        action = lambda: do_change_default_sending_stream(bot, None)
        events = self.verify_action(action)
        self.assertEqual(events[0]['bot']['default_sending_stream'], None)
    ...
    def test_change_bot_default_events_register_stream(self) -> None:
        ...
        events = self.verify_action(action)
        self.check_update_fields('default_events_register_stream', events, 0)
        self.assertEqual(events[0]['bot']['default_events_register_stream'], 'Rome')
...
'''
The following schema is a pretty weak check that the
event is well-typed.  In the actual code, as well
as the tests below, we use the following dict to
perform further type checks:

    user_profile.notification_setting_types

We can improve this, but my main goal here is to
extract out the basic schema.
'''
update_global_notifications_schema_checker = check_events_dict([
...
class NotificationActionTest(BaseAction):
    def check_event(self,
                    ...
                    events: List[Dict[str, Any]],
                    i: int,
                    setting: Union[bool, int, str]) -> None:
        event = events[i]
        update_global_notifications_schema_checker(f'events[{i}]', event)
        self.assertEqual(notification_name, event['notification_name'])
        self.assertEqual(setting, event['setting'])
            ...
            for setting_value in [True, False]:
                events = self.verify_action(
                    lambda: do_change_notification_settings(
                        ...
                        setting_value,
                        log=False))
                self.check_event(notification_setting, events, 0, setting_value)
    ...
    def test_change_notification_sound(self) -> None:
        ...
        events = self.verify_action(
            lambda: do_change_notification_settings(
                ...
                'ding',
                log=False))
        self.check_event(notification_setting, events, 0, 'ding')
        ...
        for val in [2, 3, 1]:
            events = self.verify_action(
                lambda: do_change_notification_settings(
                    ...
                    val,
                    log=False))
            self.check_event(notification_setting, events, 0, val)
