...
class NormalActionsTest(BaseAction):
    ...
    def test_do_update_bot_service(self) -> None:
        ...
        # Embedded bot: the Service row updates, but no event is sent
        # since the frontend does not consume base_url/interface for
        # embedded bots.
        embedded_bot = self.create_test_bot(
            "embedded",
            self.user_profile,
            full_name="Embedded Bot",
            bot_type=UserProfile.EMBEDDED_BOT,
        ...
        with self.verify_action(num_events=0, state_change_expected=False):
            do_update_bot_service(
                ...
                base_url="http://embedded.example.com",
                acting_user=self.user_profile,
            )
        embedded_service = get_bot_services(embedded_bot.id)[0]
        self.assertEqual(embedded_service.interface, Service.SLACK)
        self.assertEqual(embedded_service.base_url, "http://embedded.example.com")
