...
class AccessStreamTest(ZulipTestCase):
    ...
    def test_get_content_accessible_streams_queryset(self) -> None:
        ...
        # private channels they are not subscribed to, covering both settings
        # that grant content access. The guest should not gain access from
        # either, but the non-guest should — which also confirms the channels
        # are set up such that group access works.
        access_group = check_add_user_group(
            realm, "channel_access_group", [guest, hamlet], acting_user=guest
        )
        for channel_name, setting_name in [
            ("private-history-public", "can_add_subscribers_group"),
            ("private-protected", "can_subscribe_group"),
        ]:
            do_change_stream_group_based_setting(
