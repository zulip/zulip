...
class SoftReactivationReturnTests(ZulipTestCase):
    ...
    def test_get_days_since_last_visit(self) -> None:
        ...
        self.make_activity(self.example_user("othello"), days_ago=2)
        self.assertEqual(get_days_since_last_visit(user), 30)

        # The visit that is loading this page does not shorten the away-time.
        UserActivity.objects.create(
            user_profile=user,
            client=make_client("website"),
            query="/",
        ...
        # A visit from earlier today is not an away-time worth reporting.
        UserActivity.objects.filter(user_profile=user).update(
            last_visit=timezone_now() - timedelta(hours=6)
        )
        self.assertIsNone(get_days_since_last_visit(user))
    ...
    def test_capable_client_gets_reactivating_response(self) -> None:
        ...
        self.make_activity(user, days_ago=45)
        ...
        self.assertEqual(
            result,
            {
                ...
                "long_term_idle": True,
                "long_term_idle_days": 45,
                "last_event_id": -1,
            },
        )
        self.assertEqual(mock_request_queue.call_args.kwargs["event_types"], ["long_term_idle"])
        mock_enqueue.assert_called_once_with(user.id, notify_client=True)
        user.refresh_from_db()
        self.assertTrue(user.long_term_idle)

    def test_register_endpoint_matches_documented_reactivating_response(self) -> None:
        # Responses from the real endpoint are validated against the OpenAPI
        # documentation by the test client.
        user = self.example_user("hamlet")
        self.soft_deactivate(user)
        self.make_activity(user, days_ago=45)
        ...
        with (
            ...
            mock.patch("zerver.lib.events.queue_soft_reactivation"),
        ):
            result = self.client_post(
                "/json/register",
                {
                    "client_capabilities": orjson.dumps(
                        {
                            "notification_settings_null": False,
                            "long_term_idle_reactivation": True,
                        }
                    ).decode()
                },
            )
        response = self.assert_json_success(result)
        self.assertTrue(response["long_term_idle"])
        self.assertEqual(response["long_term_idle_days"], 45)
        self.assertEqual(response["queue_id"], "test-queue-id")
        self.assertEqual(response["last_event_id"], -1)

        # A user whose last visit we cannot date gets a null away-time.
        UserActivity.objects.filter(user_profile=user).delete()
        with (
            mock.patch("zerver.lib.events.request_event_queue", return_value=queue_data),
            mock.patch("zerver.lib.events.queue_soft_reactivation"),
        ...
        response = self.assert_json_success(result)
        self.assertIsNone(response["long_term_idle_days"])
    ...
    def test_client_without_capability_reactivates_synchronously(self) -> None:
        ...
        self.assertFalse(result["long_term_idle"])
        self.assertNotIn("long_term_idle_days", result)
        user.refresh_from_db()
        self.assertFalse(user.long_term_idle)
    ...
    def test_dedicated_worker_sends_ready_event(self) -> None:
        ...
        self.assertEqual(events[0]["event"], {"type": "long_term_idle", "value": False})
        self.assertEqual(events[0]["users"], [user.id])
        user.refresh_from_db()
        self.assertFalse(user.long_term_idle)

    def test_default_deferred_work_worker_sends_ready_event(self) -> None:
        # The backfill runs on deferred_work unless the dedicated queue is on.
        user = self.example_user("hamlet")
        self.soft_deactivate(user)
    ...
    def test_background_reactivation_sends_event(self) -> None:
        user = self.example_user("hamlet")
        self.soft_deactivate(user)
    ...
    def test_no_event_when_nothing_changed_and_nobody_waiting(self) -> None:
        user = self.example_user("hamlet")
        self.assertFalse(user.long_term_idle)
    ...
    def test_notification_reactivation_has_no_waiting_client(self) -> None:
        # Notifications reactivate a user who has no client waiting for them.
        user = self.example_user("hamlet")
        self.soft_deactivate(user)
    ...
    def test_ready_event_sent_even_if_already_reactivated(self) -> None:
        # Another path can reactivate the user before this job runs.
        user = self.example_user("hamlet")
        self.assertFalse(user.long_term_idle)
    ...
    def test_apply_event_updates_long_term_idle(self) -> None:
        # A second tab registering mid-reactivation buffers this event in a
        # queue that fetches the whole state; apply_event mirrors
        # fetch_initial_state_data, which pairs these two fields.
        user = self.example_user("hamlet")
        state = {"long_term_idle": True, "long_term_idle_days": 45}
        apply_event(
            user,
            state=state,
            event={"type": "long_term_idle", "value": False},
        ...
        self.assertEqual(state, {"long_term_idle": False})

    def test_long_term_idle_event_matches_documentation(self) -> None:
        # A worker emits this event, so verify_action cannot capture it.
        event = {"id": 0, "type": "long_term_idle", "value": False}
