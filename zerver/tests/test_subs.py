...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_realm_admin_remove_others_from_subbed_private_stream(self) -> None:
        ...
        are on.
        """
        # query_count is higher than the public-stream equivalent because removing a
        # user from a private stream sends a channel events notification message,
        # which requires additional queries to create and deliver the message.
        result = self.attempt_unsubscribe_of_principal(
            query_count=38,
            target_users=[self.example_user("cordelia")],
            is_realm_admin=True,
...
class PrivateStreamEventTest(ZulipTestCase):
    def test_private_stream_join_leave_notifications(self) -> None:
        ...
        Iago is subscribed throughout as the "witness" user, so that UserMessage
        rows exist for all notifications and get_topic_messages can retrieve them.
        """
        hamlet = self.example_user("hamlet")
        iago = self.example_user("iago")
        realm = hamlet.realm
        ...
        stream_name = "secret"
        stream = self.make_stream(stream_name, realm=realm, invite_only=True)

        # Iago subscribes first so he can witness all subsequent notifications.
        self.subscribe(iago, stream_name)
        messages = get_topic_messages(iago, stream, "channel events")
        self.assert_length(messages, 1)
        self.assertEqual(
            messages[0].content,
            f"@_**{iago.full_name}|{iago.id}** subscribed to this channel.",
        )
        ...
        # Hamlet unsubscribes himself; Iago witnesses the notification.
        ...
        # Iago removes Hamlet; Iago witnesses the notification.
        result = self.client_patch(
            "/json/users/me/subscriptions",
            {
    ...
    def test_private_stream_bulk_notifications(self) -> None:
        """
        A bulk subscription change to a private channel collapses into a
        single message: a few users are named inline, while larger changes
        show a count with the full roster in an expandable spoiler block.
        The names are listed alphabetically regardless of input order.
        """
        iago = self.example_user("iago")
        cordelia = self.example_user("cordelia")
        hamlet = self.example_user("hamlet")
        ...
        # Iago is subscribed throughout as the witness who receives the messages.
        self.subscribe(iago, stream.name)

        def mention(user: UserProfile) -> str:
            return f"@_**{user.full_name}|{user.id}**"

        def latest_content() -> str:
            return get_topic_messages(iago, stream, "channel events")[-1].content

        # Two users: named inline as "A and B" (sorted by name, not input order).
        bulk_add_subscriptions(realm, [stream], [hamlet, cordelia], acting_user=iago)
        self.assertEqual(
            latest_content(),
            f"{mention(iago)} subscribed {mention(cordelia)} and {mention(hamlet)} "
            "to this channel.",
        )
        bulk_remove_subscriptions(realm, [hamlet, cordelia], [stream], acting_user=iago)

        # Three users: named inline as "A, B, and C".
        bulk_add_subscriptions(realm, [stream], [othello, cordelia, hamlet], acting_user=iago)
        self.assertEqual(
            latest_content(),
            f"{mention(iago)} subscribed {mention(cordelia)}, {mention(hamlet)}, "
            f"and {mention(othello)} to this channel.",
        )
        bulk_remove_subscriptions(realm, [othello, cordelia, hamlet], [stream], acting_user=iago)
        ...
        bulk_add_subscriptions(realm, [stream], members, acting_user=iago)
        self.assertEqual(
            latest_content(),
            f"```spoiler {mention(iago)} subscribed 4 users to this channel.\n{roster}\n```",
        )
        bulk_remove_subscriptions(realm, members, [stream], acting_user=iago)
        self.assertEqual(
            latest_content(),
            f"```spoiler {mention(iago)} unsubscribed 4 users from this channel.\n{roster}\n```",
        )

        # With no acting user (e.g. an automated bulk change), no actor is named.
        bulk_add_subscriptions(realm, [stream], members, acting_user=None)
        self.assertEqual(
            latest_content(),
            f"```spoiler 4 users subscribed to this channel.\n{roster}\n```",
        )
        bulk_remove_subscriptions(realm, members, [stream], acting_user=None)
        self.assertEqual(
            latest_content(),
            f"```spoiler 4 users unsubscribed from this channel.\n{roster}\n```",
        )

    def test_public_stream_no_join_leave_notifications(self) -> None:
        """
        Verify that public streams do not receive subscription notifications,
        since these are only sent for private (invite-only) streams.
        """
        hamlet = self.example_user("hamlet")
        realm = hamlet.realm
        ...
        self.subscribe(hamlet, "public-stream")
        self.assert_length(get_topic_messages(hamlet, stream, "channel events"), 0)
