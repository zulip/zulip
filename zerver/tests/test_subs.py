...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_rename_private_stream_requires_admin(self) -> None:
        ...
        do_change_is_admin(user_profile, True)
        result = self.client_patch('/json/streams/%d' % (stream_id,),
                                   {'new_name': ujson.dumps('stream_name_new')})
        self.assert_json_success(result)

        # Not every subscribed user can rename.
        do_change_is_admin(user_profile, False)
        stream = self.subscribe(user_profile, 'stream_name_new')
        result = self.client_patch('/json/streams/%d' % (stream_id,),
                                   {'new_name': ujson.dumps('stream_name2')})
        self.assert_json_error(result, 'Must be an organization administrator')

        # Stream admin can rename. (don't have to be org admin)
        sub = get_subscription('stream_name_new', user_profile)
        do_change_subscription_property(user_profile, sub, stream, 'is_stream_admin', True)
        result = self.client_patch('/json/streams/%d' % (stream_id,),
                                   {'new_name': ujson.dumps('stream_name2')})
        self.assert_json_success(result)
...
class SubscriptionRestApiTest(ZulipTestCase):
    ...
    def test_api_invalid_stream_id(self) -> None:
        """
        Trying to set an Must be an organization administrator returns a JSON error.
        """
        test_email = self.example_email("hamlet")
        self.login(test_email)
