...
class TestCreateStreams(ZulipTestCase):
    ...
    def test_can_set_topics_policy_group(self) -> None:
        ...
        self.assert_json_error(result, "You do not have permission to change empty topics policy.")
...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_change_topics_policy(self) -> None:
        ...
        messages = get_topic_messages(user_profile, stream, "channel events")
        expected_notification = f"@_**{user_profile.full_name}|{user_profile.id}** changed the empty topic policy from **Automatic** to **Empty topic allowed**."
        self.assertEqual(messages[-1].content, expected_notification)
        ...
        stream = get_stream("stream_name1", realm)
        self.assertFalse(is_user_in_group(stream.can_administer_channel_group, hamlet))
        result = self.client_patch(
            f"/json/streams/{stream.id}",
        ...
        self.assertTrue(is_user_in_group(stream.can_administer_channel_group, hamlet))
        result = self.client_patch(
