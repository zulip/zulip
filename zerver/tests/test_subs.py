...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_change_protected_history_for_can_create_topic_group(self) -> None:
        ...
        # For a private stream with protected history, "can_create_topic_group" must be an "everyone_system_group".
        ...
        check_stream_property_update("can_create_topic_group", anonymous_group)
        check_stream_property_update("history_public_to_subscribers", False, expect_fail=True)
        ...
        # Test when both "history_public_to_subscribers", "can_create_topic_group",
        ...
        # Testing an unparsed everyone system group.
        unparsed_everyone_group = UserGroupMembersData(
            direct_members=[],
            direct_subgroups=[everyone_system_group.id],
        )
        params = {
            "history_public_to_subscribers": orjson.dumps(False).decode(),
            "can_create_topic_group": orjson.dumps({"new": unparsed_everyone_group}).decode(),
        }
        result = self.client_patch(f"/json/streams/{stream.id}", params)
        self.assert_json_success(result)
        stream.refresh_from_db()
