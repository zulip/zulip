...
from zerver.lib.actions import (
    ...
    do_get_streams, do_change_subscription_property,
...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_deactivate_stream_backend(self) -> None:
        user_profile = self.example_user('hamlet')
        self.login_user(user_profile)
        stream = self.make_stream('new_stream1')
        ...
        # Stream admin can also deacivate the stream.
        stream = self.subscribe(user_profile, 'new_stream2')
        do_change_user_role(user_profile, UserProfile.ROLE_MEMBER)
        result = self.client_delete('/json/streams/%d' % (stream.id,))
        self.assert_json_success(result)
        subscription_exists = get_active_subscriptions_for_stream_id(stream.id).filter(
    ...
    def test_rename_private_stream_requires_admin(self) -> None:
        ...
        # Unsubscribed org admin can rename.
        do_change_user_role(user_profile, UserProfile.ROLE_REALM_ADMINISTRATOR)
        result = self.client_patch('/json/streams/%d' % (stream_id,),
                                   {'new_name': ujson.dumps('stream_name2')})
        self.assert_json_success(result)
    ...
    def test_cant_remove_others_from_stream(self) -> None:
        ...
        result = self.attempt_unsubscribe_of_principal(
            query_count=6, target_users=[self.example_user('cordelia')], is_realm_admin=False,
    ...
    def test_stream_admin_remove_others_from_public_stream(self) -> None:
        ...
        """
        result = self.attempt_unsubscribe_of_principal(
            query_count=23, target_users=[self.example_user('cordelia')], is_stream_admin=True,
    ...
    def test_realm_admin_remove_others_from_unsubbed_public_stream(self) -> None:
        ...
        """
        result = self.attempt_unsubscribe_of_principal(
            query_count=23, target_users=[self.example_user('cordelia')], is_realm_admin=True,
            is_subbed=True, invite_only=False, target_users_subbed=True)
        json = self.assert_json_success(result)
        self.assertEqual(len(json["removed"]), 1)
        self.assertEqual(len(json["not_removed"]), 0)
    ...
    def test_realm_admin_remove_multiple_users_from_stream(self) -> None:
        ...
        """
        result = self.attempt_unsubscribe_of_principal(
            query_count=32, target_users=[self.example_user('cordelia'), self.example_user('prospero')],
    ...
    def test_realm_admin_remove_others_from_unsubbed_private_stream(self) -> None:
        ...
        result = self.attempt_unsubscribe_of_principal(
            ...
            is_subbed=False, invite_only=True, target_users_subbed=True,
...
class SubscriptionAPITest(ZulipTestCase):
    ...
    def test_set_stream_admin_on_stream_creation(self) -> None:
        ...
        result = self.common_subscribe_to_streams(hamlet, [stream_public, stream_private],
        ...
        self.common_subscribe_to_streams(hamlet, [stream_old, stream_new],
                                         {"principals": ujson.dumps([hamlet.email,
                                                                     cordelia.email])})
