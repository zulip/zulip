...
class TestCreateStreams(ZulipTestCase):
    ...
    def do_test_permission_setting_on_stream_creation(self, setting_name: str) -> None:
        ...
        # Delete the created stream, so we can create a new one with
        # same name for testing another setting.
        stream.delete()
        ...
        if permission_config.allow_everyone_group:
            self.assert_json_success(result)
            stream = get_stream("new_stream7", realm)
        ...
        subscriptions = [{"name": "new_stream8", "description": "Eighth new stream"}]
