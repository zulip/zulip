...
class RealmTest(ZulipTestCase):
    ...
    def test_do_deactivate_realm_clears_scheduled_jobs(self) -> None:
        ...
        do_deactivate_realm(user.realm, acting_user=None, email_owners=False)
    ...
    def test_do_send_realm_deactivation_email_no_acting_user(self) -> None:
        realm = get_realm("zulip")
        do_deactivate_realm(realm, acting_user=None, email_owners=True)
    ...
    def test_do_send_realm_deactivation_email_by_support(self) -> None:
        ...
        iago = self.example_user("iago")
        do_deactivate_realm(realm, acting_user=iago, email_owners=True)
        self.assertEqual(realm.deactivated, True)
        self.assert_length(mail.outbox, 1)
        self.assertIn(
    ...
    def test_do_send_realm_deactivation_email_by_owner(self) -> None:
        ...
        self.assertIn(
            "You have deactivated your Zulip organization, Zulip Dev, on", mail.outbox[0].body
        )
        self.assertIn(
            "Your Zulip organization Zulip Dev has been deactivated", mail.outbox[1].subject
        )
        self.assertIn(
            "Your Zulip organization, Zulip Dev, was deactivated by Iago on", mail.outbox[1].body
        )
