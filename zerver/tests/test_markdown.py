...
class MarkdownMiscTest(ZulipTestCase):
    ...
    def test_get_user_group_name(self) -> None:
        realm = get_realm("zulip")
        content = "@*hamletcharacters*"
        group = NamedUserGroup.objects.get(realm_for_sharding=realm, name="hamletcharacters")
        mention_backend = MentionBackend(realm.id)
        mention_data = MentionData(mention_backend, content, message_sender=None)
        self.assertEqual(mention_data.get_user_group_name(group.id), "hamletcharacters")

        with self.assertRaisesRegex(JsonableError, "Group ID '0' does not exist"):
            mention_data.get_user_group_name(0)
    ...
    def test_fetch_group_membership_for_bulk_mention_groups(self) -> None:
        ...
        # Bulk mention groups with the default mention permission SystemGroups.EVERYONE.
        ...
        # This way we can test that our algorithm correctly and efficiently
        # does the permission check against different mention permissions
        ...
        # It's one extra query than the previous and is triggered
        # when any mentioned group needs permission check
        # (i.e., does NOT allow mention by everyone). Again,
        # we make sure it's query count is constant.
        with self.assert_database_query_count(3):
            mention_data = MentionData(mention_backend, content, message_sender=zoe)

        # We should fetch group membership ONLY for groups the sender
        # zoe is allowed to mention.
        self.assertEqual(mention_data.get_group_members(hamlet_group.id), hamlet_members_ids)
        ...
        content = ""
        for group in mentioned_groups:
            content += f"@_*{group_name}*, "

        # Group silent mention is always allowed, so for a message with only silent mentions,
        # we do not compute permission OR fetch group memberships.
        with self.assert_database_query_count(1):
            mention_data = MentionData(mention_backend, content, message_sender=zoe)

        for group in mentioned_groups:
            self.assertEqual(mention_data.get_group_members(group.id), set())
