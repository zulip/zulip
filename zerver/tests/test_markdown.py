...
class MarkdownMiscTest(ZulipTestCase):
    ...
    def test_fetch_group_membership_when_mention_group_permission_changes(self) -> None:
        ...
        # This also tests an edge case where a sender's permission changes (they can't mention that group)
        ...
        # Mention hamlet_group by 3 senders that are direct/indirect members of the same can_mention_group.
    ...
    def test_fetch_group_membership_for_bulk_mention_groups(self) -> None:
        ...
        # ensuring we do the permission checks in a constant time.
        realm = get_realm("zulip")
        ...
        sub_group = check_add_user_group(realm, "sub_group_1", [zoe], acting_user=iago)
        ...
        for i in range(6):
            ...
            mentioned_groups_ids.append(group.id)
            content += f"@*{group_name}*, "

            # We allow some groups to be mentioned by the sender zoe through can_mention_group,
            ...
            # checks against different mention permissions for different groups.
            if i % 2 == 0:
                do_change_user_group_permission_setting(
                    group,
                    "can_mention_group",
        ...
        # Ensure we are doing the mention permission checks against multiple
        # groups in a constant time.
        # This is one extra query than test_bulk_user_group_mentions,
        # that extra query is only triggered when any mentioned group
        # needs permission check (e.g. not SystemGroups.EVERYONE).
        with self.assert_database_query_count(3):
            mention_data = MentionData(mention_backend, content, message_sender=zoe)

        # We should fetch group membership, for ONLY groups the sender (zoe) is allowed
        # to mention.
        self.assertEqual(mention_data.get_group_members(hamlet_group.id), hamlet_members_ids)
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[0]), {othello.id})
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[1]), set())
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[2]), {othello.id})
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[3]), set())
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[4]), {othello.id})
        self.assertEqual(mention_data.get_group_members(mentioned_groups_ids[5]), set())
