...
class PreviewTestCase(ZulipTestCase):
    ...
    @responses.activate
    @override_settings(INLINE_URL_EMBED_PREVIEW=True)
    def test_default_code_block_language_in_messages_with_embedded_links(self) -> None:
        ...
        content = f"```\nprint('Hello, World!')\n``` \n\n {url}"
        # Check stream's default code block language is used in messages with embedded
        # links.
        with mock_queue_publish("zerver.actions.message_send.queue_event_on_commit") as patched:
            msg_id = self.send_stream_message(
                user,
        ...
        # Check that message uses the stream's default code block language
        # even when message is re-rendered after processing of embedded links.
        msg = Message.objects.get(id=msg_id)
        assert msg.rendered_content is not None
        self.assertIn('data-code-language="Python"', msg.rendered_content)

        # Check stream's default code block language is used in messages with embedded
        # links after editing them.
        edited_content = f"```\nprint('Hello, Zulip!')\n``` \n\nPR:  {url}"
        with mock_queue_publish("zerver.actions.message_edit.queue_event_on_commit") as patched:
            result = self.client_patch(
                "/json/messages/" + str(msg_id),
        ...
        # Check realm's default code block language is used for DMs.
        with mock_queue_publish("zerver.actions.message_send.queue_event_on_commit") as patched:
            msg_id = self.send_personal_message(user, self.example_user("hamlet"), content=content)

        patched.assert_called_once()
        queue = patched.call_args[0][0]
        self.assertEqual(queue, "embed_links")
        event = patched.call_args[0][1]
        self.assertEqual(event["default_code_block_language"], "javascript")
        ...
        # Check realm's default code block language is used when editing DMs.
        edited_content = f"```\nprint('Hello, Zulip!')\n``` \n\nPR:  {url}"
        with mock_queue_publish("zerver.actions.message_edit.queue_event_on_commit") as patched:
            result = self.client_patch(
                "/json/messages/" + str(msg_id),
