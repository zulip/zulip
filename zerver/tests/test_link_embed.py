...
class PreviewTestCase(ZulipTestCase):
    ...
    def test_default_code_block_language_in_messages_with_embedded_links(self) -> None:
        ...
        # Check that message uses the stream's default code block
        # language after processing of embedded links.
        msg = Message.objects.get(id=msg_id)
        assert msg.rendered_content is not None
        ...
        # Test for DMs realm's default code block language is used.
        with mock_queue_publish("zerver.actions.message_send.queue_event_on_commit") as patched:
        ...
        self.assertEqual(queue, "embed_links")
        event = patched.call_args[0][1]
        self.assertEqual(event["default_code_block_language"], "")
