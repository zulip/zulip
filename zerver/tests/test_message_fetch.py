...
class NarrowBuilderTest(ZulipTestCase):
    ...
    def test_add_term_using_dm_including_operator_with_personal_recipient(self) -> None:
        # Creating personal recipients for Othello and Hamlet
        othello = self.example_user("othello")
        othello.recipient = Recipient.objects.create(type=Recipient.PERSONAL, type_id=othello.id)
        othello.save()

        hamlet = self.example_user("hamlet")
        hamlet.recipient = Recipient.objects.create(type=Recipient.PERSONAL, type_id=hamlet.id)
        hamlet.save()

        # Reinitialize the NarrowBuilder with the updated user profile
...
class GetOldMessagesTest(ZulipTestCase):
    ...
    def get_query_ids(self) -> dict[str, int | str]:
        ...
        query_ids["hamlet_recipient"] = self.get_dm_recipient(hamlet_user).id
        query_ids["hamlet_and_othello_recipient"] = self.get_dm_recipient(
...
"""
    ...
    def test_dm_recipient_id(self) -> None:
        ...
        recipient = self.get_dm_recipient(hamlet, othello)
        ...
        self.assertEqual(result["messages"][0]["recipient_id"], recipient.id)
        ...
        self.assertEqual(result["messages"][1]["recipient_id"], recipient.id)
