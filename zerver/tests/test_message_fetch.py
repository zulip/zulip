...
class NarrowBuilderTest(ZulipTestCase):
    ...
    def test_add_term_using_channels_operator_and_all_operand(self) -> None:
        ...
        # Subscribing to a channel with protected history adds an "id IN
        # (...)" clause restricting results to messages the user can see.
        self.subscribe(hamlet, "private-channel")
        self.send_stream_message(hamlet, "private-channel")
        self._do_add_term_test(
            term,
            'WHERE ("zerver_message"."recipient_id" IN (%s, %s, %s, %s, %s, %s, %s, %s, %s) '
            'OR "zerver_message"."id" IN (%s))',
        )
    ...
    def test_add_term_using_channels_operator_and_subscribed_operand(self) -> None:
        ...
        self._do_add_term_test(
            term,
            'WHERE "zerver_message"."recipient_id" IN (%s, %s, %s, %s, %s)',
        )
        realm = get_realm("zulip")
        hamlet_group = check_add_user_group(
            realm,
        ...
        self.subscribe(hamlet, "private-channel")
        self.send_stream_message(hamlet, "private-channel")
        # The newly subscribed channel has protected history, so its
        # messages are matched via an "id IN (...)" clause.
        self._do_add_term_test(
            term,
            'WHERE ("zerver_message"."recipient_id" IN (%s, %s, %s, %s, %s) '
            'OR "zerver_message"."id" IN (%s))',
    ...
    def test_add_term_using_channels_operator_and_subscribed_operand_negated(self) -> None:
        ...
        self._do_add_term_test(
            term,
            'WHERE NOT ("zerver_message"."recipient_id" IN (%s, %s, %s, %s, %s))',
        )
        realm = get_realm("zulip")
        hamlet_group = check_add_user_group(
            realm,
