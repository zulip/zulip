...
class NarrowBuilderTest(ZulipTestCase):
    ...
    def test_add_term_using_mentions_operator_with_non_existing_user(self) -> None:
        # -1 is used to represent a non-existing user ID, Case to test operator behavior
        NON_EXISTING_USER_ID = "-1"
        term = NarrowParameter(operator="mentions", operand=NON_EXISTING_USER_ID)
        self.assertRaises(BadNarrowOperatorError, self._build_query, term)
...
class GetOldMessagesTest(ZulipTestCase):
    ...
    def test_get_visible_messages_with_mentions_narrow(self) -> None:
        ...
        content = f"Hello @_**{iago.full_name}**!"
        self.send_stream_message(
            hamlet,
            stream.name,
            content=content,
        ...
        narrow = [dict(operator="mentions", operand=str(iago.id))]

        # This should check just the messages that mentioned this user.
        self.message_visibility_test(narrow, msg_id, 2)
