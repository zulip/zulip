...
class GetOldMessagesTest(ZulipTestCase):
    ...
    def test_get_visible_messages_using_narrow_with(self) -> None:
        ...
        msg_id = self.send_stream_message(iago, "dev team")
        ...
        # Test `with` operator returns messages with corrected narrow,
        # containing message with id equivalent to `with` operand.
        msg_ids = [
            self.send_stream_message(iago, "dev team", topic_name="test 1") for _ in range(2)
        ]
        narrow = [
            dict(operator="channel", operand="Verona"),
            dict(operator="topic", operand="test 1"),
        ...
        with_narrow = [
            dict(operator="dm", operand=[othello.id]),
            dict(operator="with", operand=msg_ids[0]),
        ]
        ...
        msg_ids = [self.send_huddle_message(iago, [cordelia, hamlet]) for _ in range(2)]

        with_narrow = [
            dict(operator="dm-including", operand=hamlet.id),
            dict(operator="with", operand=msg_ids[0]),
        ]
        results = self.get_and_check_messages(dict(narrow=orjson.dumps(with_narrow).decode()))
        ...
        result = self.client_get("/json/messages", dict(post_params))
        self.assert_json_error(result, "Invalid narrow operator: Duplicate 'with' operators.")

        # Test request with an invalid message id for `with` operator
        # raises InvalidOperatorCombinationError
