...
from zerver.lib.alert_words import AlertWordData, alert_words_in_realm, user_alert_words
...
class AlertWordTests(ZulipTestCase):
    interesting_alert_word_list = [
        AlertWordData(word="alert"),
        AlertWordData(word="multi-word word"),
        AlertWordData(word="☃"),
    ]
    interesting_alert_word_list_response = [
        {"word": "alert"},
        {"word": "multi-word word"},
        {"word": "☃"},
    ]
    ...
    def test_internal_endpoint(self) -> None:
        ...
        params = {
            "alert_words": orjson.dumps([{"word": "milk"}, {"word": "cookies"}]).decode(),
        }
        result = self.client_post("/json/users/me/alert_words", params)
        self.assert_json_success(result)

        words = user_alert_words(user)
        self.assertEqual(words, [{"word": "milk"}, {"word": "cookies"}])
    ...
    def test_basics(self) -> None:
        ...
        words = user_alert_words(user)
        self.assertEqual(words, self.interesting_alert_word_list_response)
        realm_alert_words = alert_words_in_realm(user.realm)
        self.assert_length(realm_alert_words[user.id], 3)

        # Test the case-insensitivity of adding words
        do_add_alert_words(user, {AlertWordData(word="ALert"), AlertWordData(word="ALERT")})
        words = user_alert_words(user)
        self.assertEqual(words, self.interesting_alert_word_list_response)
        realm_alert_words = alert_words_in_realm(user.realm)
        self.assert_length(realm_alert_words[user.id], 3)
        ...
        words = user_alert_words(user)
        self.assertEqual(words, [{"word": "multi-word word"}, {"word": "☃"}])
        realm_alert_words = alert_words_in_realm(user.realm)
    ...
    def test_remove_word(self) -> None:
        ...
        expected_remaining_alerts = self.interesting_alert_word_list_response
        do_add_alert_words(user, self.interesting_alert_word_list)

        for alert_word in self.interesting_alert_word_list:
            do_remove_alert_words(user, [alert_word.word])
            expected_remaining_alerts.remove({"word": alert_word.word})
    ...
    def test_realm_words(self) -> None:
        ...
        do_add_alert_words(user2, [AlertWordData(word="another")])
        ...
        self.assertEqual(set(realm_words[user2.id]), {AlertWordData(word="another")})
    ...
    def test_json_list_nonempty(self) -> None:
        ...
        do_add_alert_words(
            user,
            [AlertWordData(word="one"), AlertWordData(word="two"), AlertWordData(word="three")],
        )
        ...
        response_dict = self.assert_json_success(result)
        self.assertEqual(
            (response_dict["alert_words"]), [{"word": "one"}, {"word": "two"}, {"word": "three"}]
        )
    ...
    def test_json_list_add(self) -> None:
        ...
        result = self.client_post(
            "/json/users/me/alert_words",
            {"alert_words": orjson.dumps([{"word": ("long" * 26)}]).decode()},
        )
        self.assert_json_error(
            result, """alert_words[0]["word"] is too long (limit: 100 characters)"""
        )
    ...
    def test_json_list_remove(self) -> None:
        ...
        response_dict = self.assert_json_success(result)
        self.assertEqual(response_dict["alert_words"], [{"word": "two"}, {"word": "three"}])
    ...
    def test_update_alert_words(self) -> None:
        ...
        result = self.client_post(
            "/json/users/me/alert_words",
            {"alert_words": orjson.dumps([{"word": "ALERT"}]).decode()},
        )
