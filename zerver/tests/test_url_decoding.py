...
class NarrowTermFilterTest(ZulipTestCase):
    ...
    def test_initialize_narrow_terms(self) -> None:
        ...
        for test_name, test_case in tests.items():
            with self.subTest(name=test_name):
                ...
                if test_case["invalid_terms"] != []:
                    with self.assertRaises(BadNarrowOperatorError):
                        narrow_terms_input = [
                            NarrowTerm(**term) for term in test_case["invalid_terms"]
                        ]
                        Filter(narrow_terms_input, self.realm)
    ...
    def test_check_not_both_channel_and_dm_narrow(self) -> None:
        terms = [
            NarrowTerm(negated=False, operator="channel", operand="13"),
        ...
        with self.assertRaisesRegex(
            ...
            Filter(terms, self.realm)._check_not_both_channel_and_dm_narrow()
        ...
        self.assertTrue(filter._check_not_both_channel_and_dm_narrow())
        ...
        self.assertFalse(filter._check_not_both_channel_and_dm_narrow())
    ...
    def test_generate_near_message_url(self) -> None:
        ...
        self.assertEqual(channel_url, "/#narrow/channel/13-Venice/topic/testing/near/1")
    ...
    def test_generate_dm_url(self) -> None:
        ...
        dm_terms = [
            NarrowTerm(negated=False, operator="dm", operand=[10]),
            NarrowTerm(negated=False, operator="near", operand=1),
        ]
        terms = Filter(dm_terms, self.realm)
        dm_url = terms.generate_message_url()
        self.assertEqual(dm_url, "/#narrow/dm/10-user10.40zulip.2Etestserver/near/1")
