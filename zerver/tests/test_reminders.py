...
class RemindersTest(ZulipTestCase):
    ...
    def get_channel_message_reminder_content(self, msg_content: str, msg_id: int) -> str:
        return (
            ...
            f"@_**King Hamlet|10** [said](http://zulip.testserver/#narrow/channel/3-Verona/topic/test/near/{msg_id}) in #**Verona>test@{msg_id}**:\n```quote\n{msg_content}\n```"
        )
    ...
    def test_notes_in_reminder(self) -> None:
        ...
        self.assertEqual(
            ...
            f"@_**King Hamlet|10** [said](http://zulip.testserver/#narrow/channel/3-Verona/topic/test/near/{message_id}) in #**Verona>test@{message_id}**:\n```quote\n{content}\n```",
        )
        ...
        self.assertEqual(
            ...
            " in #**Verona>{789}@" + str(message_id) + f"**:\n```quote\n{content}\n```",
        )
    ...
    def test_schedule_reminder_ones_own_message(self) -> None:
        ...
        self.assert_json_success(result)
        scheduled_message = self.last_scheduled_reminder()
        hamlet_mention = get_user_mentions_for_display([hamlet])

        self.assertEqual(
            scheduled_message.content,
            (
                "You requested a reminder for the following direct message.\n\n"
                f"{hamlet_mention} [said](http://zulip.testserver/#narrow/dm/10/near/{message_id}) to {hamlet_mention}:\n```quote\n{content}\n```"
            ),
        )
