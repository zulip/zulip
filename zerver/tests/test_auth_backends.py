...
class TestLDAP(ZulipLDAPTestCase):
    ...
    def test_external_auth_id_login(self) -> None:
        ...
        self.assertEqual(ExternalAuthID.objects.filter(user_profile=hamlet).count(), 0)
        ...
        external_auth_ids = list(ExternalAuthID.objects.filter(user_profile=hamlet))
        ...
        # The user's email changes in LDAP. The user should still be able to successfully log in with their
        # ldap credential - the account will be found based on the ExternalAuthID - and Zulip email updated
        # to match the new email in ldap.
        ...
        self.assertEqual(ExternalAuthID.objects.filter(user_profile=hamlet).count(), 1)
        self.assertIn(
        ...
        # Test the following edge case:
        # The user's ldap email is changed to an email that matches another user already existing in LDAP.
        # Expected outcome: the user can still log in to their Zulip account with their ldap credentials,
        # but their account's email is not changed due to the conflict - we log a warning instead.
        cordelia = self.example_user("cordelia")
        ...
        # If the capitalization of the current email changes in ldap, the capitalization of the
        # Zulip email should be synced.
        self.change_ldap_user_attr(username, "mail", "New-Hamlet-email@zulip.com")
        with self.assertLogs("zulip.auth.ldap", level="INFO") as mock_log:
            user_profile = self.backend.authenticate(
                request=mock.MagicMock(),
    ...
    def test_external_auth_id_user_creation(self) -> None:
        ...
        external_auth_ids = list(ExternalAuthID.objects.filter(user_profile=user_profile))
        self.assert_length(external_auth_ids, 1)
        new_external_auth_id = external_auth_ids[0]
        self.assertEqual(new_external_auth_id.realm_id, realm.id)
        self.assertEqual(new_external_auth_id.external_auth_method_name, "ldap")
