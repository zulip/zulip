...
class HomeTest(ZulipTestCase):
    ...
    def test_home_page_frame_first_for_long_term_idle_user(self) -> None:
        ...
        # Backdate the user's activity so the reported away-time is deterministic.
        ...
        UserActivity.objects.create(
            ...
            query="/json/register",
            count=1,
            last_visit=timezone_now() - timedelta(days=45),
        )
        ...
        self.assertTrue(page_params["no_event_queue"])
        self.assertIsNone(page_params["state_data"])
        self.assertEqual(page_params["long_term_idle_days"], 45)
