...
class SubdomainsTest(ZulipTestCase):
    ...
    def test_is_static_or_current_realm_url_with_s3(self) -> None:
        ...
        self.assertTrue(test(zerver.lib.upload.get_uploaded_realm_icon_url(realm.id, version=1)))
        self.assertTrue(
            test(zerver.lib.upload.get_uploaded_realm_logo_url(realm.id, version=1, night=False))
        )
