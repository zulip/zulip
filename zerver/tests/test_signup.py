...
class UserSignUpTest(ZulipTestCase):
    ...
    def test_ldap_registration_when_name_change_group_disallows_invited_role(self) -> None:
        password = self.ldap_password("newuser")
        email = "newuser@zulip.com"
        subdomain = "zulip"
    ...
    def test_ldap_registration_when_name_change_group_allows_full_members(self) -> None:
        password = self.ldap_password("newuser")
        email = "newuser@zulip.com"
        subdomain = "zulip"
        ...
        user_profile = UserProfile.objects.get(delivery_email=email)
        # Name comes from user-submitted form value when full members are allowed.
        self.assertEqual(user_profile.full_name, "Edited Name")

    def test_name_changes_disallowed_for_guest_when_everyone_group_is_not_allowed(self) -> None:
        realm = get_realm("zulip")
        setting_config = Realm.REALM_PERMISSION_GROUP_SETTINGS["can_change_name_group"]

        with patch.object(setting_config, "allow_everyone_group", False):
            self.assertTrue(name_changes_disallowed_for_role(realm, UserProfile.ROLE_GUEST))

    def test_realm_creation_name_lock_check_uses_empty_prereg_group_ids(self) -> None:
        email = "new-realm-coverage@example.com"
        realm_subdomain = f"coverage{UserProfile.objects.count()}"
        realm_name = "Coverage Realm"
        ...
        session["authenticated_full_name"] = "Validated Full Name"
        session.save()

        with patch(
            "zerver.views.registration.name_changes_disallowed_for_role", return_value=False
        ) as mock:
            result = self.submit_reg_form_for_user(
                email,
                "x",
        ...
        self.assertIsNone(mock.call_args.args[0])
        self.assertEqual(mock.call_args.args[1], UserProfile.ROLE_REALM_OWNER)
    ...
    def test_ldap_registration_when_name_change_group_allows_invite_group(self) -> None:
        password = self.ldap_password("newuser")
        email = "newuser@zulip.com"
        subdomain = "zulip"
        ...
        allowed_group = create_user_group_in_database(
            "can-change-name", [], realm, acting_user=self.example_user("hamlet")
        )
        do_change_realm_permission_group_setting(
            realm,
            "can_change_name_group",
        ...
        prereg_user.groups.add(allowed_group)
        ...
        with self.settings(
            ...
            # Click confirmation link. This sets `authenticated_full_name`.
            result = self.submit_reg_form_for_user(
                email,
                password,
                full_name="Ignore",
