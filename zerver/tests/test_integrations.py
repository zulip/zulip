...
class IntegrationsTestCase(ZulipTestCase):
    ...
    def test_no_missing_doc_screenshot_config(self) -> None:
        ...
        self.assertEqual(
            ...
            + construct_message(
                "The following integrations have a screenshot configuration but the integrations no longer exist:",
                extra_webhook_configs,
                "Remove them from zerver.lib.integrations.DOC_SCREENSHOT_CONFIG",
            )
            + construct_message(
                "The following integrations are listed in NO_SCREENSHOT_CONFIG but the integrations no longer exist:",
