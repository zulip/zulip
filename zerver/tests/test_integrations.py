...
class InterfacedSettingTestCase(ZulipTestCase):
        ...
        self.assertDictEqual(
            interfaced_settings,
            {
                "MapToChannelsT": {
                    ...
                    "unique_query": "map_to_channels_1",
                }
            },
        )
    ...
    def test_unknown_setting(self, mock_endpoint: MagicMock) -> None:
        def mock_api_unkown_setting_webhook(
            request: HttpRequest,
            user_profile: UserProfile,
            *,
        ...
        mock_endpoint.return_value = mock_api_unkown_setting_webhook
        with self.assertRaises(AssertionError) as e:
            get_interfaced_settings_for(self.mocked_integration)
        self.assertEqual(
            str(e.exception),
            "Please add context for this setting: some_new_setting",
        )
