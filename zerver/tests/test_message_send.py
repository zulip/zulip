...
class StreamMessagesTest(ZulipTestCase):
    ...
    def test_send_message_in_support_stream(self) -> None:
        ...
        # A bot's topic access follows its owner's, so a bot owned by the user
        # who started the topic can post in it even though the bot itself is
        # not in can_access_stream_topics_group and did not send the topic's
        # first message.
        internal_send_stream_message(
            normal_user_owned_bot,
            stream,
            "Topic created by normal user",
            "Test message by bot owned by the topic creator",
        )

        self.assertEqual(
            self.get_last_message().content, "Test message by bot owned by the topic creator"
        )

        # Cross-realm bots deliver Zulip's own notices, which must not be
        # blocked by topic access restrictions.
        internal_send_stream_message(
            get_system_bot(settings.NOTIFICATION_BOT, admin_profile.realm_id),
            stream,
            "Topic created by normal user",
            "Test notice from Notification Bot",
        )

        self.assertEqual(self.get_last_message().content, "Test notice from Notification Bot")
