...
class MessagePOSTTest(ZulipTestCase):
    ...
    def test_send_message_and_resolve_general_chat_topic(self) -> None:
        user = self.example_user("hamlet")
        result = self.api_post(
            user,
            "/api/v1/messages",
            {
                "type": "channel",
                "to": orjson.dumps("Verona").decode(),
                "content": "Trying to resolve the general chat topic.",
                "topic": "",
                "then_resolve_topic": orjson.dumps(True).decode(),
            },
        )
        self.assert_json_error(result, "Cannot resolve the general chat topic.")
