...
class PersonalMessageSendTest(ZulipTestCase):
    ...
    def test_direct_message_initiator_group_setting(self) -> None:
        ...
        cordelia = self.example_user("cordelia")
        realm = user_profile.realm
        huddle_1 = [user_profile, admin, polonius]
        huddle_2 = [user_profile, admin, polonius, cordelia]
        ...
        with self.assertRaises(JsonableError):
            self.send_huddle_message(user_profile, huddle_1)

        # Have the administrator send a message to the direct message group, and verify
        # that allows the user to reply.
        self.send_huddle_message(admin, huddle_1)
        with self.assert_database_query_count(20):
            self.send_huddle_message(user_profile, huddle_1)

        # We cannot sent to `huddle_2` as no message has been sent to this group yet.
        with self.assertRaises(JsonableError):
            self.send_huddle_message(user_profile, huddle_2)
    ...
    def test_direct_message_permission_group_setting(self) -> None:
        ...
        admin = self.example_user("iago")
        realm = user_profile.realm
        huddle = [user_profile, cordelia, admin]
        huddle_without_admin = [user_profile, cordelia]
        administrators_system_group = NamedUserGroup.objects.get(
            name=SystemGroups.ADMINISTRATORS, realm=realm, is_system_group=True
        )
        self.login_user(user_profile)
        ...
        with self.assert_database_query_count(24):
            self.send_huddle_message(user_profile, huddle)
        self.send_huddle_message(admin, huddle)

        # But this one does not have an administrator. So, it should throw an error.
        with self.assertRaises(JsonableError):
            self.send_huddle_message(user_profile, huddle_without_admin)
