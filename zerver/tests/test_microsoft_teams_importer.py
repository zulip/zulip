...
class MicrosoftTeamsImporterUnitTest(MicrosoftTeamsImportTestCase):
    ...
    def test_group_chat_message_from_deactivated_member_excludes_them_from_membership(
        ...
        """
        This documents a quirk where, if a deactivated member is created, they
        will have no UserMessage row for either their own messages or the
        messages they receive, since the MS Graph API doesn't include them as
        part of the group chat's members.
        """
        realm: dict[str, Any] = {
            "zerver_stream": [],
            "zerver_defaultstream": [],
