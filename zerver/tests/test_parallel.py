...
from concurrent.futures.process import _RemoteTraceback as RemoteTraceback
...
class RunNotParallelTest(ZulipTestCase):
    ...
    def test_not_parallel_throw(self):
        events = []
    ...
    def test_not_parallel_catch(self):
        events = []
        ...
        self.assertTrue(error_logs.output[0].startswith("ERROR:root:Error processing item: 103\nTraceback"))
    ...
    def test_not_parallel_queue(self):
        events = []
...
def write_number(output_dir: str, total_processes: int, fail: set[int], item: int) -> None:
    if item in fail:
...
def db_query(output_dir: str, total_processes: int, item: int) -> None:
    connection.connect()
    with open(f"{output_dir}/{os.getpid()}.output", "a") as fh:
...
class RunParallelTest(ZulipTestCase):
    ...
    def test_parallel_throw(self):
        ...
        try:
            with self.assertRaisesMessage(Exception, "Whoops"):
                run_parallel(
                    partial(write_number, output_dir, 4, set([103])),
                    range(100,105),
                    processes=2,
                    report_every=5,
                    report=lambda n: report_lines.append(f"Completed {n}"),
            ...
            for output_path in output_files:
                with open(output_path) as output_file:
                    all_lines.update(int(l) for l in output_file)
    ...
    def test_parallel_reconnect(self):
        self.skip_in_parallel_harness()
        output_dir = tempfile.mkdtemp()
        run_parallel(
