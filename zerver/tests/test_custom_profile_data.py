...
class CreateCustomProfileFieldTest(CustomProfileFieldTestCase):
    ...
    def test_rendered_fields_in_as_dict(self) -> None:
        ...
        field.rendered_name = render_inline_markdown(field.name, realm)
        field.rendered_hint = render_inline_markdown(field.hint, realm)
    ...
    def test_render_with_prebuilt_mention_data(self) -> None:
        ...
        self.assertEqual(
            render_inline_markdown(text, realm, mention_data),
            render_inline_markdown(text, realm),
        )
        self.assertEqual(
            render_inline_markdown(text, realm, mention_data),
            "<p>@Iago #Verona</p>",
    ...
    def test_render_inline_markdown_populates_fields(self) -> None:
        realm = get_realm("zulip")
        field = CustomProfileField.objects.filter(realm=realm).first()
        assert field is not None
...
class UpdateCustomProfileFieldTest(CustomProfileFieldTestCase):
    ...
    def test_update_dropdown_field_successfully(self) -> None:
        self.login("iago")
        realm = get_realm("zulip")
        field = CustomProfileField.objects.get(name="Favorite editor", realm=realm)
        data = [
    ...
    def test_removing_option_from_dropdown_field(self) -> None:
        self.login("iago")
        realm = get_realm("zulip")
        field = CustomProfileField.objects.get(name="Favorite editor", realm=realm)
        self.assertTrue(
