...
class RealmImportExportTest(ExportFile):
    ...
    def test_import_realm(self) -> None:
        ...
        export_realm_todo_message_id = self.send_stream_message(
            self.example_user(todo_message_sender_name),
            "Denmark",
            todo_widget_message,
            todo_message_topic,
        )
        export_realm_todo_submessage = SubMessage.objects.get(
            message_id=export_realm_todo_message_id
        )
        ...
        export_realm_poll_message_id = self.send_stream_message(
            self.example_user(poll_message_sender_name),
            "Denmark",
            poll_widget_message,
            poll_message_topic,
        )
        export_realm_poll_submessage = SubMessage.objects.get(
            message_id=export_realm_poll_message_id
        )
        ...
        self.assertDictEqual(
            ...
            orjson.loads(export_realm_todo_submessage.content),
        )
        self.assertEqual(
            imported_todo_message.rendered_content,
            """
<p>/todo Example Task List Title</p>
<div class="codehilite"><pre><span></span><code>task without description
</code></pre></div>

<p>task: with description    </p>
<ul>
<li>task as list : also with description</li>
</ul>
""".strip(),
        )
        ...
        self.assertDictEqual(
            ...
            orjson.loads(export_realm_poll_submessage.content),
        )
        self.assertEqual(
            imported_poll_message.rendered_content,
            """
<p>/poll What is your favorite color?</p>
<p>Red<br>
Green  </p>
<p>Blue</p>
<ul>
