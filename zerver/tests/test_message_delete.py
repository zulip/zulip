...
class DeleteMessageTest(ZulipTestCase):
    ...
    def test_do_delete_stream_messages_with_acting_user(self) -> None:
        ...
        # the event only if they have UserMessage rows for atlease one of the
        ...
        # row for any of the messages being deleted, even when they have
        # permission to subscribe to the stream.
        with self.capture_send_event_calls(expected_num_events=2) as events:
            do_delete_messages(realm, [message], acting_user=iago)
        ...
        # Acting user receives the event if they have UserMessage row for at
        # least one of the messages being deleted and the event includes
        # messages with only UserMessage rows.
