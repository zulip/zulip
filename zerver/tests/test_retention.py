...
class MoveMessageToArchiveGeneral(MoveMessageToArchiveBase):
    ...
    def test_archive_partial_messages_updates_stats(self) -> None:
        othello = self.example_user("othello")
        cordelia = self.example_user("cordelia")
        realm = self.sender.realm

        msg_ids = [
            self.send_group_direct_message(self.sender, [othello, cordelia]) for _ in range(5)
        ]
        dm_group_id_list = [self.sender.id, othello.id, cordelia.id]

        move_messages_to_archive(message_ids=[msg_ids[0]], realm=realm)
        dm_group = get_or_create_direct_message_group(dm_group_id_list)
        self.assertEqual(dm_group.total_messages, 4)
        self.assertEqual(dm_group.first_message_id, msg_ids[1])
        self.assertEqual(dm_group.last_message_id, msg_ids[4])

        move_messages_to_archive(message_ids=[msg_ids[4]], realm=realm)
        dm_group = get_or_create_direct_message_group(dm_group_id_list)
        self.assertEqual(dm_group.total_messages, 3)
        self.assertEqual(dm_group.first_message_id, msg_ids[1])
        self.assertEqual(dm_group.last_message_id, msg_ids[3])

        move_messages_to_archive(message_ids=[msg_ids[2]], realm=realm)
        dm_group = get_or_create_direct_message_group(dm_group_id_list)
        self.assertEqual(dm_group.total_messages, 2)
        self.assertEqual(dm_group.first_message_id, msg_ids[1])
        self.assertEqual(dm_group.last_message_id, msg_ids[3])
