...
class EditMessageTest(ZulipTestCase):
    ...
    def test_delete_message_edit_history(self) -> None:
        ...
        deleted_entries = [e for e in history_data if "history_deleted_by" in e]
        self.assertGreater(len(deleted_entries), 0)
        for entry in deleted_entries:
            self.assertEqual(entry["history_deleted_by"], iago.id)
            self.assertIn("history_deleted_at", entry)
        ...
        history_data = orjson.loads(history.content)["message_history"]
        content_deleted_entries = [e for e in history_data if "history_deleted_by" in e]
        self.assertGreater(len(content_deleted_entries), 0)
        # Move-only entries must not carry history_deleted_by.
        move_only_entries = [
            e for e in history_data if "prev_topic" in e and "history_deleted_by" not in e
        ]
