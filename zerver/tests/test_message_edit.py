...
from zerver.actions.message_edit import (
    get_alert_words_for_message_updates,
...
class EditMessageTest(ZulipTestCase):
    ...
    def test_alert_words_for_message_updates(self) -> None:
        hamlet = self.example_user("hamlet")
        cordelia = self.example_user("cordelia")
        ...
        alert_word_user_ids = get_alert_words_for_message_updates(message)
        self.assertEqual(alert_word_user_ids, {cordelia.id})
    ...
    @mock.patch("zerver.actions.message_edit.send_event_on_commit")
    def test_alert_word_when_editing(self, mock_send_event: mock.MagicMock) -> None:
        stream_name = "Macbeth"
        hamlet = self.example_user("hamlet")
        cordelia = self.example_user("cordelia")
        self.make_stream(stream_name, history_public_to_subscribers=True)
        ...
        # Extract the send_event call where event type is 'update_message'.
        # Here we assert 'prior_alert_word_user_ids' (who previously had
        ...
        for call_args in mock_send_event.call_args_list:
            ...
            if arg_event["type"] == "update_message":
                self.assertEqual(arg_event["type"], "update_message")
                self.assertEqual(arg_event["prior_alert_word_user_ids"], [])
                self.assertEqual(
                    sorted(arg_notified_users, key=itemgetter("id")), users_to_be_notified
                )
                called = True
    ...
    @mock.patch("zerver.actions.message_edit.send_event_on_commit")
    def test_remove_alert_word_when_editing(self, mock_send_event: mock.MagicMock) -> None:
        stream_name = "Macbeth"
        hamlet = self.example_user("hamlet")
        cordelia = self.example_user("cordelia")
        self.make_stream(stream_name, history_public_to_subscribers=True)
        ...
        # Extract the send_event call where event type is 'update_message'.
        # Here we assert that 'prior_alert_word_user_ids' (who previously had
        ...
        for call_args in mock_send_event.call_args_list:
            ...
            if arg_event["type"] == "update_message":
                self.assertEqual(arg_event["type"], "update_message")
                self.assertEqual(arg_event["prior_alert_word_user_ids"], [cordelia.id])
                self.assertEqual(
                    sorted(arg_notified_users, key=itemgetter("id")), users_to_be_notified
                )
                called = True
