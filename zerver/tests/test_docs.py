...
class DocPageTest(ZulipTestCase):
    ...
    def test_doc_endpoints(self) -> None:
        ...
        self._test('/api/send_message', 'steal away your hearts')
        self._test('/api/render_message', '**foo**')
        self._test('/api/get_streams', 'include_public')
        self._test('/api/get_stream_id', 'The name of the stream to access.')
        self._test('/api/get_subscriptions', 'Get all streams that the user is subscribed to.')
        self._test('/api/get_users', 'client_gravatar')
        self._test('/api/register-queue', 'apply_markdown')
        self._test('/api/get_events', 'dont_block')
        self._test('/api/delete_queue', 'Delete a previously registered queue')
        self._test('/api/update_message', 'propagate_mode')
        self._test('/api/get_own_user', 'takes no arguments')
        self._test('/api/subscribe', 'authorization_errors_fatal')
        self._test('/api/create_user', 'zuliprc-admin')
