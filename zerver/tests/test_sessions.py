...
class TestSessions(ZulipTestCase):
    ...
    def test_sessionstore_create_model_instance(self) -> None:
        ...
        session_data: Dict[str, Union[int, str]] = {
    ...
    def test_user_session_ip_address(self) -> None:
        ...
        # This request passes through HostDomainMiddleware which saves the request's ip_address in the session
        self.client_get("/")

        # check if one session is created for that logged-in user
        self.assertEqual(RealmSession.objects.filter(user=user_profile).count(), 1)

        # check if session has saved the IP address
        self.assertEqual(RealmSession.objects.get(user=user_profile).ip_address, "127.0.0.1")
    ...
    def test_anonymous_session_ip_address(self) -> None:
        ...
        # check if one session is created for that anonymous user
        self.assertEqual(RealmSession.objects.filter(user=None).count(), 1)

        # check if IP address is saved in the session table
        self.assertEqual(RealmSession.objects.get(user=None).ip_address, "127.0.0.1")
