...
class InviteUserTest(InviteUserBase):
    ...
    def test_invite_user_to_realm_on_manual_license_plan(self) -> None:
        ...
        ledger.workplace_licenses_at_next_renewal = get_latest_seat_count(user.realm)
        ...
        ledger.workplace_licenses = get_latest_seat_count(user.realm) + 1
        ...
        ledger.workplace_licenses = get_latest_seat_count(user.realm)
...
class MultiuseInviteTest(ZulipTestCase):
    ...
    def test_edit_multiuse_invite_with_malformed_parameters(self) -> None:
        ...
        self.assert_json_error(result, f"Invalid channel ID {1000}.")
    ...
    def test_edit_multiuse_invite_with_archived_stream(self) -> None:
        """
        An invite that already includes an archived channel cannot be
        saved, even if the request only tries to change some other field,
        until the archived channel is removed from the invite.
        """
        realm = get_realm("zulip")
        iago = self.example_user("iago")
        ...
        verona = get_stream("Verona", realm)
        invite = self.create_multiuse_invite(
            iago, include_realm_default_subscriptions=False, streams=[denmark, verona]
        )

        do_deactivate_stream(verona, acting_user=iago)

        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{invite.id}",
            {
                "stream_ids": orjson.dumps([denmark.id, verona.id]).decode(),
            },
        )
        self.assert_json_error(result, f"Invalid channel ID {verona.id}.")
        invite.refresh_from_db()
        self.assertEqual(set(invite.streams.values_list("id", flat=True)), {denmark.id, verona.id})

        # Removing the archived channel allows the edit to succeed.
        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{invite.id}",
            {
                "stream_ids": orjson.dumps([denmark.id]).decode(),
            },
        )
        self.assert_json_success(result)
        invite.refresh_from_db()
        self.assertEqual(set(invite.streams.values_list("id", flat=True)), {denmark.id})

    def test_edit_multiuse_invite_with_deactivated_group(self) -> None:
        """
        An invite that already includes a deactivated user group cannot be
        saved until the deactivated group is removed from the invite.
        """
        realm = get_realm("zulip")
        iago = self.example_user("iago")
        ...
        do_deactivate_user_group(group2, acting_user=iago)

        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{invite.id}",
            {
                "group_ids": orjson.dumps([group1.id, group2.id]).decode(),
            },
        )
        self.assert_json_error(result, "User group is deactivated.")
        invite.refresh_from_db()
        self.assertEqual(set(invite.groups.values_list("id", flat=True)), {group1.id, group2.id})

        # Removing the deactivated group allows the edit to succeed.
        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{invite.id}",
            {
        ...
        self.assert_json_success(result)
        invite.refresh_from_db()
        self.assertEqual(set(invite.groups.values_list("id", flat=True)), {group1.id})
