...
class MultiuseInviteTest(ZulipTestCase):
    ...
    def test_edit_multiuse_invite_with_malformed_parameters(self) -> None:
        ...
        * id - ID of the multiuse_invite
        ...
        def check_edit_multiuse_invite_helper(
            ...
            *,
            ...
            result = self.api_patch(
                user,
                f"/api/v1/invites/multiuse/{invite_id}",
                dict(
                    invite_as=invite_as,
        ...
        check_edit_multiuse_invite_helper(
            ...
            modified_invited_as,
            [1000],
            error_message=f"Invalid channel ID {1000}. No invites were sent.",
        )
        ...
        check_edit_multiuse_invite_helper(
            ...
            modified_invited_as,
            modified_streams,
            error_message="No such invitation",
        )
        ...
        check_edit_multiuse_invite_helper(
            ...
            invalid_invite_as,
            modified_streams,
            error_message="Invalid invite_as: Value error, Not in the list of possible values",
        )
    ...
    def test_permission_for_editing_multiuse_invite(self) -> None:
        ...
        def check_edit_multiuse_invite_helper(
            invite: MultiuseInvite,
            user: UserProfile,
            group: NamedUserGroup | UserGroup,
            *,
            error_message: str | None = None,
        ) -> None:
            do_change_realm_permission_group_setting(
                realm, "create_multiuse_invite_group", group, acting_user=None
            )
            result = self.api_patch(
                user,
        ...
        check_edit_multiuse_invite_helper(invite, othello, members_group)

        # User-defined group
        user_defined_group = check_add_user_group(realm, "editors", [iago], acting_user=othello)
        # User-defined group: Only members of the group can edit their own invites.
        check_edit_multiuse_invite_helper(invite, iago, user_defined_group)
        check_edit_multiuse_invite_helper(
            invite, othello, user_defined_group, error_message="Insufficient permission"
        )
        ...
        hamlet.is_realm_owner = True
        check_edit_multiuse_invite_helper(
            invite, hamlet, user_defined_group, error_message="Insufficient permission"
        )

        # Anonymous group
        anonymous_group = self.create_or_update_anonymous_group_for_setting([iago], [])
        check_edit_multiuse_invite_helper(invite, iago, anonymous_group)
        check_edit_multiuse_invite_helper(
            invite, othello, anonymous_group, error_message="Insufficient permission"
        )

        # General rule: Only admins or the creator can edit a multiuse invite.
        # This is tested once for clarity.
        check_edit_multiuse_invite_helper(
            invite, hamlet, members_group, error_message="Must be an organization administrator"
        )
    ...
    def test_permission_for_editing_invite_as(self) -> None:
        ...
        # Changing invite_as to OWNER
        check_changing_invite_as(
            iago,
            OWNER_INVITE,
            error_message="Must be an organization owner",
        )
        check_changing_invite_as(desdemona, OWNER_INVITE)

        # Changing invite_as to ADMIN
        check_changing_invite_as(
            shiva,
            ADMIN_INVITE,
        ...
        check_changing_invite_as(iago, ADMIN_INVITE)

        # Changing invite_as to MODERATOR
        check_changing_invite_as(
            shiva,
            MODERATOR_INVITE,
            error_message="Must be an organization administrator",
        ...
        check_changing_invite_as(iago, PreregistrationUser.INVITE_AS["MODERATOR"])

        # This check for the sake of completion of the test.
        # Invite is actually not being changed here.
        check_changing_invite_as(shiva, MEMBER_INVITE)
        check_changing_invite_as(othello, MEMBER_INVITE)
        check_changing_invite_as(iago, MEMBER_INVITE)
    ...
    def test_edit_multiuse_invite_without_permission_to_subscribe_others(self) -> None:
        ...
        def create_multiuse_invite(
            user: UserProfile, initial_stream_ids: list[int]
        ) -> MultiuseInvite:
            invite = MultiuseInvite.objects.create(
                referred_by=user,
                realm=realm,
            )
            invite.streams.set(initial_stream_ids)
            return invite

        def check_path_multiuse_invite_with_subscribe_permission(
            user: UserProfile,
            initial_stream_ids: list[int],
            new_stream_ids: list[int],
            expected_error: str | None = None,
        ) -> None:
            invite = create_multiuse_invite(user, initial_stream_ids)
            result = self.api_patch(
                user,
                f"/api/v1/invites/multiuse/{invite.id}",
        ...
        check_path_multiuse_invite_with_subscribe_permission(cordelia, [], [streams[0].id])
        ...
        check_path_multiuse_invite_with_subscribe_permission(hamlet, [], [streams[1].id])
        check_path_multiuse_invite_with_subscribe_permission(
            hamlet,
            [streams[1].id],
            [streams[0].id],
        ...
        check_path_multiuse_invite_with_subscribe_permission(cordelia, [], [streams[1].id])
        check_path_multiuse_invite_with_subscribe_permission(iago, [], stream_ids)
