...
class ChannelSubscriptionPermissionTest(ZulipTestCase):
    ...
    def test_user_unsubscribe_themselves(self) -> None:
        ...
        # Test that a user without any permission cannot
        ...
        # A guest can unsubscribe via the realm-level can_unsubscribe_group.
        polonius = self.example_user("polonius")  # guest
        ...
        check_unsubscribing_user(polonius)

    def test_bot_owner_unsubscribing_owned_bot(self) -> None:
        admin = self.example_user("iago")
        aaron = self.example_user("aaron")
        realm = admin.realm
        ...
        do_change_realm_permission_group_setting(
            realm, "can_unsubscribe_group", nobody_group, acting_user=admin
        )

        bot_stream = self.make_stream("bot_test_stream")
        bot = self.create_test_bot("testbot", aaron)
        self.subscribe(bot, bot_stream.name)
        result = self.api_delete(
            aaron,
            "/api/v1/users/me/subscriptions",
            {
                "subscriptions": orjson.dumps([bot_stream.name]).decode(),
                "principals": orjson.dumps([bot.id]).decode(),
            },
        )
        self.assert_json_error(result, "You do not have permission to unsubscribe this bot.")

        bot_unsubscribe_group = UserGroupMembersData(direct_members=[bot.id], direct_subgroups=[])
        do_change_stream_group_based_setting(
            bot_stream,
            "can_unsubscribe_group",
            bot_unsubscribe_group,
            acting_user=admin,
        )
        result = self.api_delete(
            aaron,
...
class ChannelAdministerPermissionTest(ZulipTestCase):
    ...
    def test_update_stream_default_push_notifications(self) -> None:
        """Only admins can change default_push_notifications; the update
