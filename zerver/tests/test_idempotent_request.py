...
class IdempotencyTest(ZulipTestCase):
    ...
    We test different successful and failed request scenarios.
    """
    ...
    def test_early_failure(self) -> None:
        """
        When a request fails early during validation
        and before the actual work is attempted.

        We currently do NOT cache failed responses.
        ...
        # For early failures, we expect only 1 extra query, executed
        # by @idempotent_endpoint.
        with self.assert_database_query_count(len(base_queries) + 1):
            result = self.client_post("/json/messages", post_data, headers=headers)
        self.assert_json_error(result, "Channel 'nonexistent_stream' does not exist")
        ...
        # That one row reflects a not successful work and has no cached_result.
        idempotent_rows = IdempotentRequest.objects.filter(
            realm_id=realm.id, user_id=sender.id, succeeded=None, cached_result__isnull=True
        )
        ...
        # Again, not successful work and no cached_result.
        idempotent_rows = IdempotentRequest.objects.filter(
            realm_id=realm.id, user_id=sender.id, succeeded=None, cached_result__isnull=True
        )
    ...
    def test_failure_after_successful_work(self) -> None:
        ...
        with mock.patch(
            ...
            # Our edge and undesired case that would duplicate the work.
            # In practice this either happens or not depending on
            # the client behavior:
            # a. Happens: client treats that failure as a non-transiet error
            # and tries the request with a different key.
            # b. Avoidable: client treats that failure as a transient error
            # and tries with the same key.
        ...
        # ensure the successful work and its cached_result is actually replayable.
        ...
        # 0 from the successful and duplicate request.
        self.assert_length(
    ...
    def test_different_requests_with_same_key(self) -> None:
        ...
        TODO: Ideally, in future, we should detect this behavior by comparing
        the parameters for both requests, and if they differ, we return an error
        telling the user that they should change the key.
        """
        realm = get_realm("zulip")
        sender = self.example_user("hamlet")
        self.login("hamlet")
    ...
    def test_correct_idempotent_work(self) -> None:
        """
        @idempotent_endpoint is the scope that applies idempotency
        to functions decorated with @idempotent_work. However, those functions
        are still expected to be called in other non-idempotent contexts like tests,
        so we just document that here.
        """
        realm = get_realm("zulip")
        sender = self.example_user("hamlet")
...
class TransactionIdempotencyTest(ZulipTransactionTestCase):
    ...
    def test_work_failure(self) -> None:
        """
        Test idempotency system when the core work fails.
        ...
        with mock.patch(
            ...
            # For work failure, we expect 2 extra queries:
            # 1 to insert the initial row +
            # 1 to fetch cached_result and lock the row.
            ...
            # Since we don't cache failures yet, we expect
            # the core work to be attempted twice by the previous 2 requests.
            # Note: In case of caching failures, core work should be attempted only once.
            self.assertEqual(do_work_mock.call_count, 2)
    ...
    def test_request_validation_changes(self) -> None:
        ...
        # Since client regained the permission to send (i.e., the new server validation),
        # they now get the new result which is success.
        result = self.client_post("/json/messages", post_data, headers=headers)
        self.assert_json_success(result)
        # Idempotency row should now reflect the successful work.
        self.assert_length(
            IdempotentRequest.objects.filter(
                realm_id=realm.id, user_id=sender.id, succeeded=True, cached_result__isnull=False
            ),
        ...
        # Work is done once as a result of using the same key.
        self.assert_length(
            Message.objects.filter(realm_id=realm.id, sender=sender, content=post_data["content"]),
            1,
        )
    ...
    def test_server_failure_before_work(self) -> None:
        ...
        # Make the server fail before attempting the work.
        with (
            mock.patch(
        ...
        # We currently don't cache errors, so we expect both
        # cached_result and succeeded to be None representing a
        # not-yet-attempted work or an error occurring
