...
class MessageMoveTopicTest(ZulipTestCase):
    ...
    def test_mark_resolved_topic_notifications_as_read_policy(self) -> None:
        self.login("iago")
        admin_user = self.example_user("iago")
        hamlet = self.example_user("hamlet")
        cordelia = self.example_user("cordelia")
        ...
        do_change_user_setting(
            ...
            MarkResolvedTopicNotificationsAsReadPolicyEnum.always,
            acting_user=None,
        )
        do_change_user_setting(
            cordelia,
            "mark_resolved_topic_notifications_as_read_policy",
            MarkResolvedTopicNotificationsAsReadPolicyEnum.unfollowed_topics,
            acting_user=None,
        )
        do_change_user_setting(
            hamlet,
            "mark_resolved_topic_notifications_as_read_policy",
            MarkResolvedTopicNotificationsAsReadPolicyEnum.never,
            acting_user=None,
        )
        ...
        # Check resolved topic notifications are marked unread as per
        # mark_resolved_topic_notifications_as_read_policy setting
        # for followed and unfollowed topics.
        assert (
            UserMessage.objects.filter(user_profile=aaron, message__id=messages[1].id)
        ...
        assert (
            UserMessage.objects.filter(
                user_profile__in=[cordelia, hamlet], message__id=messages[1].id
            )
        ...
        assert (
            UserMessage.objects.filter(
                user_profile__in=[aaron, cordelia], message__id=messages[1].id
            )
            .extra(where=[UserMessage.where_read()])  # noqa: S610
            .count()
            == 2
        ...
        assert (
            UserMessage.objects.filter(user_profile=hamlet, message__id=messages[1].id)
            .extra(where=[UserMessage.where_unread()])  # noqa: S610
            .count()
            == 1
        )

        # Check unresolved topic notifications are marked unread as per
        # mark_resolved_topic_notifications_as_read_policy setting
        # for followed and unfollowed topics.
        ...
        self.assert_length(messages, 3)
        assert (
            UserMessage.objects.filter(user_profile=aaron, message__id=messages[2].id)
            .extra(where=[UserMessage.where_read()])  # noqa: S610
            .count()
            == 1
        )
        assert (
            UserMessage.objects.filter(
                user_profile__in=[cordelia, hamlet], message__id=messages[2].id
            )
            .extra(where=[UserMessage.where_unread()])  # noqa: S610
            .count()
            == 2
        ...
        assert (
            UserMessage.objects.filter(
                user_profile__in=[aaron, cordelia], message__id=messages[2].id
            )
            .extra(where=[UserMessage.where_read()])  # noqa: S610
            .count()
            == 2
        ...
        assert (
            UserMessage.objects.filter(user_profile=hamlet, message__id=messages[2].id)
            .extra(where=[UserMessage.where_unread()])  # noqa: S610
            .count()
            == 1
        )
