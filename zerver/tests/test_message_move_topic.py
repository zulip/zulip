...
class MessageMoveTopicTest(ZulipTestCase):
    ...
    def test_resolved_topic_notice_unread_policy(self) -> None:
        # Test that resolved and unresolved-topic notices are marked as
        # read/unread based on the 'resolved_topic_notice_unread_policy'
        ...
        do_change_user_setting(
            ...
            ResolvedTopicNoticeUnreadPolicyEnum.always,
            acting_user=None,
        )
        do_change_user_setting(
            cordelia,
            "resolved_topic_notice_unread_policy",
            ResolvedTopicNoticeUnreadPolicyEnum.when_following,
            acting_user=None,
        )
        do_change_user_setting(
            hamlet,
            "resolved_topic_notice_unread_policy",
            ResolvedTopicNoticeUnreadPolicyEnum.never,
            acting_user=None,
        )
        ...
        # - aaron: policy 'always' -> notice remains unread.
        # - cordelia: policy 'when_following' and is following -> notice remains unread.
        # - hamlet: policy 'never' -> notice is marked as read.
        # - admin_user: is the one who resolved the topic -> notice is marked as read.
        self.assertEqual(unread_user_ids, {aaron.id, cordelia.id})
        self.assertEqual(read_user_ids, {hamlet.id, admin_user.id})
        ...
        # - aaron: policy 'always' -> notice remains unread.
        # - cordelia: policy 'when_following' but not following -> notice is marked as read.
        # - hamlet: policy 'never' -> notice is marked as read.
        # - admin_user: is the one who resolved the topic -> notice is marked as read.
        self.assertEqual(unread_user_ids, {aaron.id})
        self.assertEqual(read_user_ids, {cordelia.id, hamlet.id, admin_user.id})
    ...
    def test_send_resolve_topic_notification_with_no_topic_messages(self) -> None:
        ...
        maybe_send_resolve_topic_notifications(
            user_profile=admin_user,
            message_edit_request=message_edit_request,
            user_profile_to_topic_visibility_policy={},
        )
