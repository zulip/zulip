...
def handle_slack_webhook_message(
    ...
    match mapping_option:
        case "channels":
            check_send_webhook_message(request, user_profile, topic_name, content, stream=channel)
        case "topics":
            topic_name = f"channel: {channel}"
            check_send_webhook_message(request, user_profile, topic_name, content)
        case None:
...
def api_slack_webhook(
    ...
    *,
    ...
    mapping_option: str | None = "topics" if map_to_topics else map_to_channels

    requires_channel_name = (channels_map_to_topics or mapping_option) is not None
    channel = get_slack_channel_name(channel_id, slack_app_token) if requires_channel_name else None
    ...
    if channels_map_to_topics is not None:
        legacy_handle_slack_webhook_message(
            request, user_profile, content, channel, channels_map_to_topics
        )
    else:
        handle_slack_webhook_message(request, user_profile, content, channel, mapping_option)
    return json_success(request)
