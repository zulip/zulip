...
def topic_and_body(payload: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if event_type.startswith('charge'):
        ...
        amount_string=amount(
            payload["data"]["object"]["amount"], payload["data"]["object"]["currency"])
    ...
    if category in ['payment_intent', 'payout', 'plan', 'product', 'recipient',
        ...
        raise NotImplementedEventType()
            ...
            body = body_template.format(
                charge_id=charge_id, link=link, amount=amount_string, verb=verb)
        ...
        if event_type.startswith('customer.subscription'):
            ...
            if event_type == "customer.subscription.created":
                amount_string = amount(
                    data_object["plan"]["amount"], data_object["plan"]["currency"])
                ...
                body = body_template.format(
                    id=object_id, link=link, days=days_left)
            ...
            body = body_template.format(
                beginning=beginning, id=object_id, link=link, rest=rest)
        ...
        amount_string = amount(data_object["amount"], data_object["currency"])
        body_template = "The payout with description **{description}** and id **[{id}]({link})** for amount **{amount}** has {end}. "

        if event_type == "payout.canceled":
            end = "been canceled"
            topic = "Payout Canceled {}".format(object_id)
        elif event_type == "payout.created":
            end = "been created"
            topic = "Payout Created {}".format(object_id)
        elif event_type == "payout.failed":
            end = "failed"
            topic = "Payout Failed {}".format(object_id)
        elif event_type == "payout.paid":
            end = "been paid"
            topic = "Payout Paid {}".format(object_id)
        else:
            end = "been updated"
            topic = "Payout Updated {}".format(object_id)
