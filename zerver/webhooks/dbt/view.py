...
{emoji} {job_name} deployment {status} in **{environment}**{end_time_phrase}.
...
def extract_data_from_payload(payload: JsonBodyPayload[WildValue]) -> dict[str, str]:
    ...
    # We check for two potential end time fields that could be in the
    # DBT API payload, one for errors and one for a successful run.
