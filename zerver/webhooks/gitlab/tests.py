...
~~~
    ...
    def test_snippet_emoji_award_event_message(self) -> None:
        expected_topic_name = "sample / snippet #4831194 Sample Snippet"
        expected_message = "Varun Kolanu added the emoji **thumbsup** to [snippet #4831194](https://gitlab.com/kolanuvarun/sample/-/snippets/4831194)"

        self.check_webhook("emoji_hook__award__snippet", expected_topic_name, expected_message)

    def test_issue_emoji_revoke_event_message(self) -> None:
        expected_topic_name = "sample / issue #1 Sample Issue"
        expected_message = "Varun Kolanu removed the emoji **thumbsdown** from [issue #1](https://gitlab.com/kolanuvarun/sample/-/issues/1)"

        self.check_webhook("emoji_hook__revoke__issue", expected_topic_name, expected_message)

    def test_merge_request_emoji_event_message_without_merge_request_title(self) -> None:
        expected_topic_name = "sample / MR #1"
        expected_message = "Varun Kolanu added the emoji **thumbsup** to [MR #1](https://gitlab.com/kolanuvarun/sample/-/merge_requests/1)"
    ...
    def test_merge_request_note_emoji_revoke_event_message(self) -> None:
        expected_topic_name = "sample / MR #1 Edit README.md"
        expected_message = "Varun Kolanu removed the emoji **heart** from [a comment on MR #1](https://gitlab.com/kolanuvarun/sample/-/merge_requests/1#note_2431339003)"

        self.check_webhook(
            "emoji_hook__revoke__merge_request_note", expected_topic_name, expected_message
        )

    def test_design_note_emoji_award_event_message(self) -> None:
