...
def get_topic_based_on_event(event: str, payload: WildValue, use_merge_request_title: bool) -> str:
    if event == "Push Hook":
        ...
        return TOPIC_WITH_PR_OR_ISSUE_INFO_TEMPLATE.format(
            repo=get_repo_name(payload),
            type="issue" if not event.startswith("Work Item") else "work item",
            id=payload["object_attributes"]["iid"].tame(check_int),
...
def get_event(request: HttpRequest, payload: WildValue, branches: str | None) -> str | None:
    ...
    # This is the critical array that needs "Work Item Hook"
    if event in ["Confidential Issue Hook", "Issue Hook", "Merge Request Hook", "Wiki Page Hook", "Work Item Hook"]:
