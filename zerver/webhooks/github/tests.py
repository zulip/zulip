...
TOPIC_PR_LONG_TITLE = truncate_topic(
    TOPIC_WITH_PR_OR_ISSUE_INFO_TEMPLATE.format(
        repo="public-repo",
        type="PR",
        id=1,
        title="This is a very long Pull request title to test topic truncation when renaming",
    )
)
TOPIC_PR_EDITED_TITLE = TOPIC_WITH_PR_OR_ISSUE_INFO_TEMPLATE.format(
    repo="public-repo", type="PR", id=1, title="New Short Title"
)
TOPIC_ISSUE_LONG_TITLE = truncate_topic(
    TOPIC_WITH_PR_OR_ISSUE_INFO_TEMPLATE.format(
        repo="zulip-test",
        type="issue",
        id=33,
        title="This is a very long Issue title to test topic truncation when renaming.",
    )
)
TOPIC_ISSUE_EDITED_TITLE = TOPIC_WITH_PR_OR_ISSUE_INFO_TEMPLATE.format(
    repo="zulip-test", type="issue", id=33, title="New Short Title."
)
...
class GitHubWebhookTest(WebhookTestCase):
    ...
    def test_issues_edited_title(self) -> None:
        ...
        self.check_webhook("issues__edited_title", TOPIC_ISSUE_EDITED_TITLE, expected_message)
    ...
    def test_issue_msg(self) -> None:
        ...
        self.check_webhook("issues__opened", TOPIC_ISSUE_LONG_TITLE, expected_message)

    def test_pull_request_title_edit_moves_topic(self) -> None:
        self.url = self.build_webhook_url(enable_topic_rename="true")
        old_topic = TOPIC_PR_LONG_TITLE
        new_topic = TOPIC_PR_EDITED_TITLE
    ...
    def test_issue_title_edit_moves_topic(self) -> None:
        self.url = self.build_webhook_url(enable_topic_rename="true")
        old_topic = TOPIC_ISSUE_LONG_TITLE
        new_topic = TOPIC_ISSUE_EDITED_TITLE
    ...
    def test_title_edit_with_custom_topic_does_not_rename(self) -> None:
        ...
        self.assert_length(
            Message.objects.filter(realm=self.test_user.realm, subject=TOPIC_PR_EDITED_TITLE), 0
        )
    ...
    def test_pull_request_opened_msg(self) -> None:
        ...
        self.check_webhook("pull_request__opened", TOPIC_PR_LONG_TITLE, expected_message)
    ...
    def test_pull_request_edited_msg(self) -> None:
        ...
        self.check_webhook("pull_request__edited_title", TOPIC_PR_EDITED_TITLE, expected_message)
    ...
    def test_pull_request_review_request_removed_msg(self) -> None:
        expected_message = "soheil-star01 removed the review request for [redolat](https://github.com/redolat) from [PR #1](https://github.com/techpillars-oy/test1/pull/1)."
        self.check_webhook(
    ...
    def test_pull_request_review_request_removed_with_custom_topic_in_url(self) -> None:
        self.url = self.build_webhook_url(topic="notifications")
        expected_message = "soheil-star01 removed the review request for [redolat](https://github.com/redolat) from [PR #1 Feat/webhook](https://github.com/techpillars-oy/test1/pull/1)."
        self.check_webhook(
    ...
    def test_pull_request_review_request_removed_team_reviewer_msg(self) -> None:
        ...
        expected_message = "soheil-star01 removed the review request for [authority](https://github.com/orgs/test-org965/teams/authority) from [PR #1](https://github.com/techpillars-oy/test1/pull/1)."
        self.check_webhook(
