...
class GitHubWebhookTest(WebhookTestCase):
    ...
    # Toggle this flag to switch between previous (False) and
    # new (True) username formatting
    @override_settings(USE_MENTION_FORMAT=False)
    def format_username(self, username: str, link: str | None = None) -> str:
        """Helper method to format usernames based on USE_MENTION_FORMAT."""
        self.USE_MENTION_FORMAT = False  # To use silent mention change it to True
        if self.USE_MENTION_FORMAT and username and link:
            return f"[@_**{username}**]({link})"
        elif username and link:
            return f"[{username}]({link})"
        elif self.USE_MENTION_FORMAT and username:
            return f"@_**{username}**"
        return username

    def test_ping_event(self) -> None:
        expected_message = f"GitHub webhook has been successfully configured by {self.format_username('TomaszKolek')}."
