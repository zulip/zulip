...
RATE_EXCEEDED_TEMPLATE = "{message}. For more info: [See this]({url})"
ERROR_RESOLVED_TEMPLATE = "The error **{id}** was resolved by *{actor}*. For more info: [See this]({url})"
ERROR_OCCURRED_TEMPLATE = "{klass}: {message} The error(**{id}**) occurred at: **{last_notice}**. For more" \
                          " info: [See this]({url})"
COMMENT_TEMPLATE = "On error *{id}*, **{author}** commented \n```quote\n{body}\n```\nFor more info: " \
                   "[See this]({url})"
CHECK_IN_TEMPLATE = "{message}. *{name}* was expected to report at: **{expected_at}** and reported at: " \
                    "**{reported_at}**. For more info: [See this]({url})"
ASSIGNED_TEMPLATE = "{message}. Error **{id}** has been assigned to **{name}**. For more info: " \
                    "[See this]({url})"
UP_AND_DOWN_TEMPLATE = "{message} For more info: [See this]({url})"
...
def get_resolved_topic_and_message(payload: Dict[str, Any]) -> Tuple[str, str]:
    topic, _ = parse_topic_and_message(message=payload["message"])
