...
def api_openproject_webhook(
    ...
    *,
    ...
    if not event_type:
        return JsonResponse({"error": "Missing 'event_type'"}, status=400)  # nocoverage
    if not data:
        return JsonResponse({"error": "Missing 'data'"}, status=400)  # nocoverage
    if not heading:
        return JsonResponse({"error": "Missing 'heading'"}, status=400)  # nocoverage
