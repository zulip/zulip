...
def get_user_info(request: HttpRequest, user: WildValue) -> str:
    ...
    # Since GDPR, we don't get username; instead, we get account_id,
    # display_name, and nickname. Account IDs are specific to Atlassian Cloud products.
    # Bitbucket Data Center do not include account_id in their webhook payloads.
    if "account_id" in user:
        account_id = user["account_id"].tame(check_string)
    ...
    if "display_name" in user:
        return user["display_name"].tame(check_string)

    if "nickname" in user:
        return user["nickname"].tame(check_string)
