...
def get_jobs_details(pipeline_data: WildValue) -> str:
    ...
    for key in formatted_job_dict:
        ...
        job_details_template += f"- **{key} job(s)**: {formatted_job_list}\n"
...
def get_body(payload: WildValue) -> str:
    ...
    body_details = {
        ...
        "status": pipeline_data["stage"].get("state").tame(check_string),
    ...
    if body_details["status"] == "Building":
        ...
        result = pipeline_data["stage"].get("result").tame(check_string)
        body_details.update(
            {
                "result": result,
                "emoji": ":check:" if result == "Passed" else ":warning:",
                "end_time": pipeline_data["stage"].get("last-transition-time").tame(check_string),
            }
        )
