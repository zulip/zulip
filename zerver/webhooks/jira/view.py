...
def get_formatted_payload_field_lines(
    ...
    *,
    ...
    for entry in configured_fields.split(","):
        ...
        raw_value = get_in(payload, [*base_path, *path]).value
...
def append_custom_fields_to_content(
    content: str,
    payload: WildValue,
    custom_fields: str,
) -> str:
    """
    Append custom field information to the end of any webhook message.
    Works for all event types (created, updated, deleted, commented).

    Args:
        content: The base message generated by event handler
        payload: The webhook payload containing issue data
        custom_fields: Comma-separated list of field names to include

    Returns:
        The content with custom fields appended (if any exist)
    """
    if not custom_fields:
        return content

    extra = get_formatted_payload_field_lines(
        payload, custom_fields, base_path=["issue", "fields"], field_aliases=CUSTOM_FIELD_ALIASES
    )
    if extra:
        return f"{content}\n{extra}"
...
def api_jira_webhook(
    ...
    *,
    ...
    content = append_custom_fields_to_content(content, payload, custom_fields)
