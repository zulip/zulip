...
def api_newrelic_webhook(
    ...
    *,
    ...
    if missing_fields:
        ...
        body_detail = f"**Warning**: Unable to use the default notification format because at least one expected field was missing from the incident payload. See [New Relic integration documentation](/integrations/doc/newrelic).\n\n**Missing fields**: {formatted_missing_fields}"
        body = NOTIFICATION_TEMPLATE.format(header=header, body=body_detail).strip()
        ...
        topic = TOPIC_TEMPLATE.format(**message_context).title()
