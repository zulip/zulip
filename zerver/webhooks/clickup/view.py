...
def api_clickup_webhook(
    ...
    *,
    ...
    if not team_id:
        raise RequestVariableMissingError("team_id")
    if not clickup_api_key:
        raise RequestVariableMissingError("clickup_api_key")

    try:
        topic, body = topic_and_body(payload, clickup_api_key, team_id)
    except APIUnavailableError:
        logger.warning("Could not reach ClickUp API.")
    except BadRequestError:
        logger.warning("ClickUp service had internal error")
    ...
    if history_items:
        for history_data in history_items:
            ...
            if updated_field in SpammyFields.as_list():
                # Updating these fields may trigger multiple notifications at a time.
...
def get_item_data(
    ...
    if event_item_type == EventItemType.TASK.value:
        ...
        item_data.update(
            goal_data.get("goal")
        )  # in case of Goal payload, useful datas are stored 1 level deeper
