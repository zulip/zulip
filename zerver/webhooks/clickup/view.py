...
def body_message_for_special_fields(history_dict: WildValue, updated_field: str) -> str:
    ...
    if updated_field == "name":
        ...
        return MESSAGE_WRAPPER.format(
            icon=":folder:", content=f"Moved from **{old_value}** to **{new_value}**"
        )
        ...
        return MESSAGE_WRAPPER.format(
            icon=":stopwatch:", content=f"Time spent changed to **{new_time_spent}**"
        )
        ...
        return MESSAGE_WRAPPER.format(
            icon=":ruler:",
            content=f"Time estimate changed from **{old_value}** to **{new_value}** by **{event_user}**",
        )
    else:
...
def get_item_data(
    ...
    if event_item_type in ["task", "list", "folder", "space", "goal"]:
