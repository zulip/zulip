...
class ClickUpHookTests(WebhookTestCase):
    ...
    def test_task_created(self) -> None:
        with (
            patch("zerver.webhooks.clickup.view.get_task") as mock_get_task,
            open("zerver/webhooks/clickup/callback_fixtures/get_task.json") as f,
        ):
    ...
    def test_list_created(self) -> None:
        with (
            patch("zerver.webhooks.clickup.view.get_list") as mock_get_list,
            open("zerver/webhooks/clickup/callback_fixtures/get_list.json") as f,
        ):
            mock_get_list.return_value = json.load(f)
            self.url = self.build_webhook_url(team_id="XXXXXXXX", clickup_api_key="123")
            expected_message = ":new: **[List: Listener](https://app.clickup.com/XXXXXXXX/home)** has been created in your ClickUp space!"
            self.check_webhook(
    ...
    def test_list_updated(self) -> None:
        with (
            ...
            self.url = self.build_webhook_url(team_id="XXXXXXXX", clickup_api_key="123")
            expected_message = (
                "**[List: Listener](https://app.clickup.com/XXXXXXXX/home)** has been updated!\n"
                "~~~ quote\n"
    ...
    def test_folder_created(self) -> None:
        with (
            patch("zerver.webhooks.clickup.view.get_folder") as mock_get_folder,
            open("zerver/webhooks/clickup/callback_fixtures/get_folder.json") as f,
        ):
    ...
    def test_space_created(self) -> None:
        with (
            patch("zerver.webhooks.clickup.view.get_space") as mock_get_space,
            open("zerver/webhooks/clickup/callback_fixtures/get_space.json") as f,
        ):
    ...
    def test_goal_created(self) -> None:
        with (
            patch("zerver.webhooks.clickup.view.get_goal") as mock_get_goal,
            open("zerver/webhooks/clickup/callback_fixtures/get_goal.json") as f,
        ):
    ...
    def test_webhook_api_callback_unavailable_error(self) -> None:
        ...
        with (
            patch("zerver.decorator.webhook_logger.exception") as mock_exception,
            self.assertRaisesRegex(APIUnavailableCallBackError, exception_msg),
        ):
    ...
    def test_webhook_api_callback_bad_request_error(self) -> None:
        ...
        with (
            patch("zerver.decorator.webhook_logger.exception") as mock_exception,
            self.assertRaisesRegex(BadRequestCallBackError, exception_msg),
        ):
            my_webhook_raises_exception(request)
        mock_exception.assert_called_once()
    ...
    def test_clickup_api_endpoints(self) -> None:
        ...
        for item, call_api in endpoint_map.items():
            ...
            with (
                ...
                ) as mock_clickup_session,
                open(mock_fixtures_path) as f,
            ):
                mock_response = MagicMock()
