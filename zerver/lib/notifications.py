...
from django.db import django_models
...
def pm_narrow_url(participants):
    # type: (List[django_models.EmailField]) -> str
    participants.sort()
...
def build_message_list(user_profile, messages):
    # type: (UserProfile, List[UserMessage]) -> List[Dict[str, Any]]
    """
    ...
    def message_header(user_profile, message):
        # type: (UserProfile, UserMessage) -> Dict[str, str]
...
def do_send_missedmessage_events_reply_in_zulip(user_profile, missed_messages, message_count):
    # type: (UserProfile, List[UserMessage], int) -> None
    """
...
def clear_followup_emails_queue(email, mail_client=None):
    # type: (django_models.EmailField, Optional[mandrill.Mandrill]) -> None
    """
