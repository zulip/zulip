...
class RegisterPushDeviceToBouncerPayload(TypedDict):
    user_profile_id: int
...
def handle_registration_to_bouncer_failure(
    ...
    * Stores the `error_code` in the `PushDevice` table. This is later
    ...
    if error_code == PushDevice.ErrorCode.REQUEST_EXPIRED_ERROR:
        logging.error(
...
def handle_register_push_device_to_bouncer(payload: RegisterPushDeviceToBouncerPayload) -> None:
    user_profile_id = payload["user_profile_id"]
    user_profile = get_user_profile_by_id(user_profile_id)
    bouncer_public_key = payload["bouncer_public_key"]
    encrypted_push_registration = payload["encrypted_push_registration"]
    push_account_id = payload["push_account_id"]
    ...
    try:
        ...
        return
    # TODO: Handle MISSING_REMOTE_REALM error raised.
