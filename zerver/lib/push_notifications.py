...
def get_info_from_apns_result(
    ...
    if isinstance(result, aioapns.exceptions.ConnectionError):
        ...
        if settings.ZILENCER_ENABLED and isinstance(device, RemotePushDevice):
            result_info.delete_token_id = b64encode_token_id(device.token_id)
        else:
...
def send_push_notifications(
    ...
    for push_device in push_devices:
        ...
        if push_device.push_token_kind == Device.PushTokenKind.APNS:
            ...
            apns_push_request = APNsPushRequest(
                token_id=b64encode_token_id(push_device.push_token_id),
    ...
    delete_token_ids_int = [b64decode_token_id(token_id) for token_id in delete_token_ids]
