...
def get_encrypted_data(payload_data_to_encrypt: dict[str, Any], push_key_bytes: bytes) -> str:
    # Currently, we drop the first byte. It'll play a role
    # in future when we'll potentially revise the cryptosystem we use.
    encryption_key_bytes = push_key_bytes[1:]

    secret_box = SecretBox(encryption_key_bytes)
