...
def get_topic_lock_status(user_profile: UserProfile, topic_name: str, stream_id: int) -> bool:
    print(f"{user_profile.realm.id}, {stream_id} {topic_name}")
...
def update_topic_lock_status(
    user_profile: UserProfile,
    old_topic_name: str,
    new_topic_name: str | None,
    current_stream_id: int,
    new_stream_id: int | None,
) -> None:
    print(f"{user_profile.realm.id}, {current_stream_id} {old_topic_name} {new_stream_id}")
    topic = TopicSettings.objects.filter(
        realm_id=user_profile.realm.id,
        stream_id=current_stream_id,
        topic_name=old_topic_name.lower(),
    ).first()
