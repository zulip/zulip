...
def find_date_anchor(
    *,
    ...
    inner_msg_id_col: ColumnElement[Integer] | None = None,
) -> int | None:
...
def parse_anchor_value(
    ...
    if anchor_val == "date":
        ...
        if isinstance(anchor_date, datetime):
            ...
            anchor_datetime = datetime.combine(anchor_date, time.min, tzinfo=timezone.utc)
    ...
    with get_sqlalchemy_connection() as sa_conn:
        if client_requested_message_ids is not None:
            ...
            if anchor_value is None:
                anchor_type = DEFAULT_ANCHOR_INFO["type"]
                anchor_value = DEFAULT_ANCHOR_INFO["value"]
