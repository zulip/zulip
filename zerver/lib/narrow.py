...
class NarrowBuilder:
    ...
    def get_protected_history_condition(self) -> ClauseElement:
        assert self.user_profile is not None
        message_ids = get_messages_with_protected_history_queryset(self.user_profile).values_list(
    ...
    def by_channels(self, query: Select, operand: str, maybe_negate: ConditionTransform) -> Select:
        ...
        if operand == "public":
            ...
            full_history_recipient_queryset = (
                get_subscribed_streams_without_protected_history_queryset(self.user_profile)
            )
            full_history_recipient_cond = self.get_recipient_condition(
                full_history_recipient_queryset
            )
