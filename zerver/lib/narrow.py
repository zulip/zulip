...
class NarrowBuilder:
    ...
    def by_mention(
        ...
        try:
            if isinstance(operand, str):
                user = get_user_including_cross_realm(operand, self.realm)
            else:
                user = get_user_by_id_in_realm_including_cross_realm(operand, self.realm)
        except (JsonableError, UserProfile.DoesNotExist):
            raise BadNarrowOperatorError("unknown user " + str(operand))

        # Only check for direct (visible) personal mentions here. We intentionally do
        # not consider other mention-related flags (e.g. wildcard, stream, or silent
        # mentions), since this condition is meant to match only explicit @-mentions
