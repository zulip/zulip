...
class Filter:
    ...
    def reformat_channel_operand(operand: str | int) -> str | int:
        match operand:
            ...
            case str():
                if operand.strip() == "":
                    raise BadNarrowOperatorError("Invalid 'channel' operand")  # nocoverage
                if operand.isdigit():
                    return int(operand)
        ...
        match operand:
            ...
            case str():
                if operand.isdigit():
                    return [int(operand)]
                if operand.strip() == "":
                    raise BadNarrowOperatorError("Invalid user ID")  # nocoverage
    ...
    def _check_not_both_channel_and_dm_narrow(self) -> bool:
        """
        Asserts that the given terms does not narrow to both
    ...
    def generate_channel_url(self) -> str:
        self._check_not_both_channel_and_dm_narrow()
        ...
        return f"/#narrow/channel/{encode_stream(channel.id, channel.name)}"
    ...
    def generate_dm_with_url(self) -> str:
        ...
        if not len(recipients) == 1:
            raise InvalidOperatorCombinationError("Must be exactly one 'dm' operand")
        ...
        try:
            ...
            if len(users_ids_or_emails) == 1:
                ...
                full_name = user_profiles[0].get_username()
    ...
    def generate_base_message_url(self) -> str:
        is_channel_narrow = self._check_not_both_channel_and_dm_narrow()
