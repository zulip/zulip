...
def check_only_scheduled_end_time_updated(
    ...
    if isinstance(scheduled_end_time, MissingType):
        return False
    ...
    if (
        ...
        and ((old_status_text in ["", None] and status_text is None) or status_text == "")
    ) and ((old_emoji_name in ["", None] and emoji_name is None) or emoji_name == ""):
        raise JsonableError(_("Client does not have any status set."))

    if status_text is not None or emoji_name is not None:
        return False
