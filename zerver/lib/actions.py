...
def do_change_realm_subdomain(realm: Realm, new_subdomain: str) -> None:
    ...
    placeholder_realms = Realm.objects.filter(
        deactivated_redirect=old_uri, deactivated=True
    )
...
def do_delete_user(user_profile: UserProfile) -> None:
    ...
    subs_to_recreate = [
        ...
        for recipient in Recipient.objects.filter(
            id__in=subscribed_huddle_recipient_ids
        )
    ]
    ...
    with transaction.atomic():
        ...
        for send_request in send_message_requests:
            ...
            for um in user_messages:
                user_message_flags[send_request.message.id][
                    um.user_profile_id
                ] = um.flags_list()
...
def get_recipient_from_user_profiles(
    ...
    recipient_profiles_map = {
        user_profile.id: user_profile for user_profile in recipient_profiles
    }
...
def internal_send_stream_message(
    ...
    message = internal_prep_stream_message(
        realm, sender, stream, topic, content, email_gateway
    )
...
def bulk_get_subscriber_user_ids(
    ...
    recip_to_stream_id = {
        stream["recipient_id"]: stream["id"] for stream in target_stream_dicts
    }
...
def do_rename_stream(
    stream: Stream, new_name: str, user_profile: UserProfile
) -> Dict[str, str]:
    old_name = stream.name
...
def gather_subscriptions_helper(
    ...
    sub_dicts = [
        sub for sub in sub_dicts if recip_id_to_stream_id.get(sub["recipient_id"])
    ]
    ...
    if user_profile.can_access_public_streams():
        ...
        web_public_stream_ids = {
            stream["id"] for stream in all_streams if stream["is_web_public"]
        }
    ...
    if include_subscribers:
        ...
        subscribed_stream_ids = {
            get_stream_id(sub) for sub in sub_dicts if sub["active"]
        }
...
def do_send_user_group_update_event(
    user_group: UserGroup, data: Dict[str, str]
) -> None:
