...
class SessionStore(CachedDbSessionStore):
    ...
    django.contrib.sessions.backends.cached_db does write-through to the
    cache and the backing database.  If the database is in a
    ...
    the session in assertions which enforce that the database cannot be
    in a transaction before writing.
    """
