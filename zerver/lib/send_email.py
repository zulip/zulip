...
def send_custom_email(
    ...
    *,
    ...
    # NOTE: There is a subtle edge case with this deduplication logic when
    # multiple accounts share an email address:
    # If 3 accounts share an email, we send one email
    # and create an audit log entry for only one of those accounts. If that specific account
    # later changes their email address and we retry sending, the deduplication
    # becomes unreliable - the account with the changed email may be excluded, while the
    # other accounts with the original shared email may receive the email again.
    # This happens because we log which USER received
    # the email but deduplicate based on the CURRENT email address of
    # that user.
