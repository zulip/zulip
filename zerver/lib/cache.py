...
def flush_user_profile(sender, **kwargs):
    ...
    # Invalidate realm-wide slash commands cache if any user in the realm
    # adds or deactivates a bot of type UserProfile.SLASH_COMMANDS
    if user_profile.is_bot and user_profile.bot_type == UserProfile.SLASH_COMMANDS:
        if kwargs.get('update_fields') is None or "is_active" in kwargs['update_fields']:
            cache_delete(slash_commands_by_realm_key(user_profile.realm, None))
...
def slash_commands_by_realm_key(realm, is_active=None):
    # type: (Realm, Optional[bool]) -> Text
