...
def fetch_initial_state_data(
    ...
    *,
    ...
    if want("pinned_views"):
        state["pinned_views"] = [] if user_profile is None else do_get_pinned_views(user_profile)
    ...
    *,
    ...
    if event["type"] == "message":
        ...
        if event["op"] == "add":
            ...
            for i, view in enumerate(state["pinned_views"]):
                if view["view_id"] == event["pinned_view"]["view_id"]:
                    state["pinned_views"][i] = event["pinned_view"]
                    break
