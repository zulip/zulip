...
def post_process_state(user_profile: UserProfile, ret: Dict[str, Any],
    ...
    if 'realm_user_groups' in ret:
        all_realm_user_groups = ret['realm_user_groups'].copy()
        ret['realm_user_groups'] = [g for g in all_realm_user_groups if g['is_active']]
        ret['realm_non_active_user_groups'] = [g for g in all_realm_user_groups if not g['is_active']]

        '''
        See the doctring above; the same applies here for removing `is_active` field
        '''
        for g in all_realm_user_groups:
            g.pop('is_active')
