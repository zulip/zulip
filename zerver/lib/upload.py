...
# method to check key exists or not in bucket
def check_key_exists(s3, bucket, key):
    # type: (boto3.Session, Text, Text) -> bool
    try:
...
def get_realm_for_filename(path):
    ...
    if not check_key_exists(s3, settings.S3_AUTH_UPLOADS_BUCKET, path):
        return None
...
class S3UploadBackend(ZulipUploadBackend):
    ...
    def delete_message_image(self, path_id):
        ...
        if check_key_exists(s3, settings.S3_AUTH_UPLOADS_BUCKET, path_id):
