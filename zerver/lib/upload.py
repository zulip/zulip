...
class ZulipUploadBackend:
    ...
    def get_realm_logo_url(self, realm_id: int, version: int) -> Text:
        raise NotImplementedError()
...
class S3UploadBackend(ZulipUploadBackend):
    ...
    def get_realm_logo_url(self, realm_id: int, version: int) -> Text:
        bucket = settings.S3_AVATAR_BUCKET
        # ?x=x allows templates to append additional parameters with &s
        return u"https://%s.s3.amazonaws.com/%s/realm/logo.png?version=%s" % (bucket, realm_id, version)
...
class LocalUploadBackend(ZulipUploadBackend):
    ...
    def get_realm_logo_url(self, realm_id: int, version: int) -> Text:
        # ?x=x allows templates to append additional parameters with &s
        return u"/user_avatars/%s/realm/logo.png?version=%s" % (realm_id, version)
