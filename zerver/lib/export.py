...
def do_export_user(user_profile: UserProfile, output_dir: Path) -> None:
    ...
    # Expand this iterator, since we need to look at its values now;
    # we can't just stream it.
    response["zerver_reaction"] = list(response["zerver_reaction"])
...
# We only require Python 3.10, which does not include itertools.batched; include our own equivalent
