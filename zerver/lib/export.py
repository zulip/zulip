...
def export_partial_message_files(realm: Realm,
    ...
    if len(consented_user_ids) != 0:
        ...
        ).exclude(recipient__in=recipient_ids_for_them).order_by('id')
...
def get_consented_user_ids(consent_message_id: int) -> Set[int]:
    return set(Reaction.objects.filter(message__id=consent_message_id, emoji_name="+1").
