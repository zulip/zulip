...
def _check_key_metadata(email_gateway_bot: Optional[UserProfile],
                        s3o: boto3.resources.base.ServiceResource,
                        processing_avatars: bool,
                        realm: Realm, user_ids: Set[int]) -> None:
    # Helper function for export_files_from_s3
    if 'realm_id' in s3o.metadata and s3o.metadata['realm_id'] != str(realm.id):
        if email_gateway_bot is None or s3o.metadata['user_profile_id'] != str(email_gateway_bot.id):
            raise AssertionError("Key metadata problem: %s %s / %s" % (s3o.name, s3o.metadata, realm.id))
        # Email gateway bot sends messages, potentially including attachments, cross-realm.
        print("File uploaded by email gateway bot: %s / %s" % (s3o.key, s3o.metadata))
    elif processing_avatars:
        if 'user_profile_id' not in s3o.metadata:
            raise AssertionError("Missing user_profile_id in key metadata: %s" % (s3o.metadata,))
        if int(s3o.metadata['user_profile_id']) not in user_ids:
            raise AssertionError("Wrong user_profile_id in key metadata: %s" % (s3o.metadata,))
    elif 'realm_id' not in s3o.metadata:
        raise AssertionError("Missing realm_id in key metadata: %s" % (s3o.metadata,))
...
def _get_exported_s3_record(
    ...
    record = dict(s3_path=s3o.key, bucket=s3o.bucket_name,
                  size=s3o.content_length, last_modified=s3o.last_modified,
                  content_type=s3o.content_type, md5=s3o.e_tag)
    record.update(s3o.metadata)

    if processing_emoji:
        record['file_name'] = os.path.basename(s3o.key)
...
def _save_s3_object_to_file(s3o: boto3.resources.base.ServiceResource,
                            output_dir: str, processing_avatars: bool,
    ...
    if processing_avatars or processing_emoji or processing_realm_icon_and_logo:
        filename = os.path.join(output_dir, s3o.key)
    else:
        fields = s3o.key.split('/')
        if len(fields) != 3:
            raise AssertionError("Suspicious key with invalid format %s" % (s3o.key,))
        filename = os.path.join(output_dir, s3o.key)
...
def export_files_from_s3(realm: Realm, bucket_name: str, output_dir: Path,
    ...
    for s3_object_summary in bucket.objects.filter(Prefix=object_prefix):
        s3o = s3_object_summary.Object()
        if processing_avatars and s3o.key not in avatar_hash_values:
            continue

        # This can happen if an email address has moved realms
        _check_key_metadata(email_gateway_bot, s3o, processing_avatars, realm, user_ids)
        record = _get_exported_s3_record(s3o, processing_emoji)

        record['path'] = s3o.key
        _save_s3_object_to_file(s3o, output_dir, processing_avatars, processing_emoji,
