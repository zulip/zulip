...
class ZulipSCIMGroup(SCIMGroup):
    ...
    def members(self) -> list[dict[str, object]]:
        ...
        members.
        """
        users = UserProfile.objects.filter(id__in=get_user_group_direct_member_ids(self.obj))
    ...
    def delete(self) -> None:
        ...
        # TODO: Settle how DELETE requests should work for groups.
        super().delete()
...
def validate_group_member_ids_from_request(realm: Realm, member_ids: list[int]) -> None:
    if member_ids:
        member_realm_ids = list(
            UserProfile.objects.filter(id__in=member_ids)
            .distinct("realm_id")
