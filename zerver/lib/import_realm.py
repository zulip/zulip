...
def bulk_import_named_user_groups(data: ImportedTableData) -> None:
    vals = [
        (
            ...
            group["name"],
            group["description"],
            group.get("rendered_description_html", ""),
    ...
    query = SQL(
        """
        INSERT INTO zerver_namedusergroup (usergroup_ptr_id, realm_id, name, description, rendered_description_html, is_system_group, can_add_members_group_id, can_join_group_id,  can_leave_group_id, can_manage_group_id, can_mention_group_id, can_remove_members_group_id, deactivated, date_created)
        VALUES %s
        """
    )
    with connection.cursor() as cursor:
