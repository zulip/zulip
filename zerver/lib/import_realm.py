...
def load_id_map_from_disk(import_dir: Path) -> dict[str, dict[int, int]]:
    """
...
def import_message_data(
    ...
    Args:
        ...
        import_dir: Directory containing the export data
    ...
    if processes > 1 and total_files > 1:
        ...
        logging.info("Using sequential processing for message import")
        for file_id in message_files:
            process_message_file_worker((file_id, import_dir, realm.id, sender_map))
        logging.info("Sequential message import completed!")
