...
def can_access_streams_user_ids(streams: list[Stream], realm: Realm) -> dict[int, set[int]]:
    ...
    private_streams = [stream for stream in streams if not stream.is_public()]
    if len(private_streams) != 0:
        users_dict_with_metadata_access_via_groups = (
            get_users_dict_with_metadata_access_to_streams_via_permission_groups(
    ...
    for stream in streams:
        if stream.is_public():
            ...
            users_with_access_dict[stream.id] = (
                admin_user_ids
                | subscribers_dict[stream.id]
                | users_dict_with_metadata_access_via_groups[stream.id]
            )
