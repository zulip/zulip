...
def bulk_can_unsubscribe_self_from_streams(
    ...
    for stream in streams:
        if not is_user_in_can_unsubscribe_group(stream, user_recursive_group_ids):
            return False
...
def list_to_streams(
    ...
    if unsubscribing_others is False and not bulk_can_unsubscribe_self_from_streams(
