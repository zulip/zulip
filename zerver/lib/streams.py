...
def list_to_streams(
    ...
    if not unsubscribing_others and people_to_unsub:
        for target in people_to_unsub:
            if not bulk_can_unsubscribe_self_from_streams(
                list(existing_stream_map.values()), target
            ):
                if target.is_bot:
