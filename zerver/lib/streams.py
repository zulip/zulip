...
def get_stream_post_policy_value_based_on_group_setting(setting_group: UserGroup) -> int:
    if (
        ...
        named_user_group = setting_group.named_user_group
        if named_user_group.name == SystemGroups.ADMINISTRATORS:
            return Stream.STREAM_POST_POLICY_ADMINS

        if named_user_group.name == SystemGroups.MODERATORS:
            return Stream.STREAM_POST_POLICY_MODERATORS

        if named_user_group.name == SystemGroups.FULL_MEMBERS:
            return Stream.STREAM_POST_POLICY_RESTRICT_NEW_MEMBERS
