...
def idempotent_work(
    *,
    ...
    This decorator is expected to decorate the function doing the work
    (e.g. do_send_messages) that is called within the scope of
    an endpoint/view decorated with @idempotent_endpoint.
    That decorated function must take idempotency_key and user as kwargs;
    the latter is required if the former is non-null.
    ...
    def idempotency_decorator(
        ...
        def wrapper(*args: ParamT.args, **kwargs: ParamT.kwargs) -> WorkReturnT:
            ...
            # Idempotency-Key is NOT mandatory, so we proceed with
            ...
            with transaction.atomic(durable=durable, savepoint=savepoint):
                try:
                    ...
                    try:
                        ...
                        # Sanity check: to ensure we are within the scope of @idempotent_endpoint.
...
def idempotent_endpoint(
    ...
    def _wrapped_view_func(
        ...
        **kwargs: ParamT.kwargs,
        ...
        # Idempotency-Key is NOT mandatory.
        if idempotency_key is None:
