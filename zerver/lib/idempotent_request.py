...
def idempotent_work_transaction(
    ...
    *,
    ...
    """
    CRITICAL: In case of applying idempotency to some work,
    this decorator MUST ONLY decorate a function that's called
    within the scope of an endpoint/view decroated with @idempotent_endpoint.
        ...
        def wrapper(*args: ParamT.args, **kwargs: ParamT.kwargs) -> WorkReturnT:
            ...
            with transaction.atomic(durable=durable, savepoint=savepoint):
                ...
                try:
                    ...
                    # but abort if another concurrent request is already holding a lock.
                    try:
                        idempotent_request = (
                            IdempotentRequest.objects.select_for_update(nowait=True)
                            .values("completed", "cached_result")
                            .get(
                        ...
                        # Sanity check: to ensure we are within the scope of
                        # an endpoint decorated with @idempotent_endpoint.
                        raise AssertionError(
                            "No matching row found for IdempotentRequest, make sure you are inside @idempotent_endpoint."
                        )

                    # Duplicate succeeded work.
                    if idempotent_request["completed"] is True:
                    ...
                    # This clause will only be taken if the exception in the previous
                    # request was due to an error inside do_work; most validation
                    # (especially for message sending) happens outside of do_work,
                    # so while exceptions there will fill `cached_result` by way of `@idempotent_endpoint`,
                    # we will never reach this point to re-raise it, because the same
                    # validation will catch it in the repeated request before hitting
                    # this decorator.
                    if idempotent_request["completed"] is False:
                        # We know cached_result is already json-deserialized.
