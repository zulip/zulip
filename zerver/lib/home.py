...
def build_page_params_for_home_page_load(
    ...
    # These parameters are used both for the server-side
    # do_events_register() call during initial page load, and sent to
    # the client in page_params["register_params"] so that the client
    # can make an identical /json/register API call during reloads.
    # Using local variables as a single source of truth prevents the
    # two code paths from diverging.
    register_apply_markdown = True
    register_client_gravatar = True
    register_slim_presence = True
    register_presence_history_limit_days = settings.PRESENCE_HISTORY_LIMIT_DAYS_FOR_WEB_APP
    register_include_subscribers: bool | Literal["partial"] = "partial"
    register_client_capabilities = ClientCapabilities(
    ...
    if user_profile is not None:
        ...
        state_data = do_events_register(
            ...
            realm,
            client,
            apply_markdown=register_apply_markdown,
            client_gravatar=register_client_gravatar,
            slim_presence=register_slim_presence,
            presence_last_update_id_fetched_by_client=-1,
            presence_history_limit_days=register_presence_history_limit_days,
            client_capabilities=register_client_capabilities,
            narrow=narrow,
            include_streams=False,
            include_subscribers=register_include_subscribers,
        )
    ...
    if user_profile is not None:
        page_params["register_params"] = dict(
            apply_markdown=register_apply_markdown,
            client_gravatar=register_client_gravatar,
            slim_presence=register_slim_presence,
            presence_history_limit_days=register_presence_history_limit_days,
            include_subscribers=register_include_subscribers,
            client_capabilities=dict(register_client_capabilities),
        )
