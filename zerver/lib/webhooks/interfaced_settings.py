...
def setting_prefix_key(param: str) -> str:
    # This is to make sure interfaced settings query are unique to
    # other query the integration might declare.
    param_id = NEXT_ID("param")
    key = f"{param}_{param_id}"
    return key


MapToChannelsT: TypeAlias = Annotated[bool, ApiParamConfig(setting_prefix_key("map_to_channels"))]
...
SUPPORTED_INTERFACED_SETTINGS: dict[str, TypeAlias] = {"MapToChannelsT": MapToChannelsT}
...
def get_settings_context(arg_name: str, arg_type: Any) -> SettingContextT | None:
    ...
    raise AssertionError(f"Please add context for this setting: {arg_name}")
