...
def do_delete_old_unclaimed_attachments(weeks_ago: int) -> None:
    ...
    unique_path_ids = old_unclaimed_attachments.values_list("path_id", flat=True).union(
        old_unclaimed_archived_attachments.values_list("path_id", flat=True)
    )

    ImageAttachment.objects.filter(path_id__in=unique_path_ids).delete()
    with delete_message_attachments() as delete_one:
        for path_id in unique_path_ids.iterator():
            delete_one(path_id)

    old_unclaimed_attachments.delete()
    old_unclaimed_archived_attachments.delete()
