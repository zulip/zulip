...
from zerver.lib.topic import (
    check_access_based_on_stream_topic_access_group,
...
def get_recipient_info(
    *,
    ...
    topic_creator_user_id: Optional[int] = None,
    ...
    if stream and is_support_stream and not message_sender.is_bot:
        assert stream_topic is not None
        authorized_user_ids = [
            row["id"]
            for row in rows
            if check_access_based_on_stream_topic_access_group(row["id"], stream)
        ]
    ...
    if addressee.is_stream():
        ...
        if stream.is_support_stream():
            ...
            if sender.is_bot and sender.bot_owner is not None:
                user_id = sender.bot_owner.id
            topic_creator_user_id = get_topic_creator_user_id(realm.id, recipient.id, topic_name)
            if (
                not check_access_based_on_stream_topic_access_group(user_id, stream)
