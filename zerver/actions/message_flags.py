...
def do_clear_mobile_push_notifications_for_ids(
    ...
    if user_profile_ids is not None:
        ...
        usermessage_queryset = UserMessage.objects.filter(
        ...
        usermessage_queryset = (
            # Uses index: zerver_usermessage_message_with_active_mobile_push_notification_idx
            UserMessage.objects.filter(
                message_id__in=message_ids,
            ).extra(  # noqa: S610
                where=[UserMessage.where_active_push_notification()],
    ...
    for um in usermessage_queryset:
        messages_by_user[um.user_profile_id].append(um.message_id)
    ...
    usermessages_to_update = []
    for um in usermessage_queryset:
        if um.user_profile_id in push_device_not_registered_user_ids:
            um.flags &= ~UserMessage.flags.active_mobile_push_notification
            usermessages_to_update.append(um)
    if usermessages_to_update:
        UserMessage.objects.bulk_update(usermessages_to_update, ["flags"])
