...
def do_deactivate_user(
    ...
    *,
    ...
    message_delete_action: int = Message.NO_DELETE_ACTION,
) -> None:
    ...
    with transaction.atomic(savepoint=False):
        ...
        if is_spammer:
            ...
            # Bots created by a spam user are also considered as spammers.
            user_bots = get_owned_bot_dicts(user_profile)
            for bot_profile in user_bots:
                bot_user = UserProfile.objects.get(id=bot_profile["user_id"])
                if bot_user.full_name != "Deactivated bot":
                    check_change_full_name(bot_user, "Deactivated bot", acting_user=acting_user)
