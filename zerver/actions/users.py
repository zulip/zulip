...
def get_service_dicts_for_bot(user_profile_id: int) -> list[dict[str, Any]]:
    ...
    if user_profile.bot_type == UserProfile.OUTGOING_WEBHOOK_BOT:
        ...
        try:
            return [{"integration_name": get_bot_config(user_profile).get("integration_name", "")}]
        # A ConfigError just means that there are no config entries for user_profile.
        except ConfigError:
            return []
    else:
...
def get_service_dicts_for_bots(
    ...
    bot_configs = get_bot_configs(bot_profile_ids)
    ...
    for bot_dict in bot_dicts:
        ...
        if bot_type == UserProfile.OUTGOING_WEBHOOK_BOT:
            ...
            service_dicts = [
                {
                    "integration_name": (bot_config.get("integration_name") if bot_config else ""),
                }
            ]
