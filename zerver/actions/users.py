...
def _apply_permission_setting_change(
    ...
    if isinstance(setting_object, Stream):
        permission_settings = Stream.stream_permission_group_settings
        audit_log_kwargs: dict[str, Any] = dict(modified_stream=setting_object)
    elif isinstance(setting_object, NamedUserGroup):
        permission_settings = NamedUserGroup.GROUP_PERMISSION_SETTINGS
        audit_log_kwargs = dict(modified_user_group=setting_object)
    else:
        permission_settings = Realm.REALM_PERMISSION_GROUP_SETTINGS
        audit_log_kwargs = {}

    replacement_group = system_groups_name_dict[
        permission_settings[setting_name].replacement_group_name
    ]
