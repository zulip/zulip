...
@transaction.atomic(durable=True)
def do_change_stream_topics_policy(
    stream: Stream, acting_user: UserProfile, topics_policy: int
) -> None:
    old_topics_policy_value = stream.topics_policy
    stream.topics_policy = topics_policy
    ...
    RealmAuditLog.objects.create(
        ...
        event_time=timezone_now(),
        extra_data={
            RealmAuditLog.OLD_VALUE: old_topics_policy_value,
            RealmAuditLog.NEW_VALUE: topics_policy,
    ...
    event = dict(
        ...
        type="stream",
        property="topics_policy",
        value=StreamTopicsPolicyEnum(topics_policy).name,
        stream_id=stream.id,
        name=stream.name,
    )
    send_event_on_commit(stream.realm, event, can_access_stream_metadata_user_ids(stream))
    ...
    with override_language(stream.realm.default_language):
        internal_send_stream_message(
            ...
            _(
                ...
                old_topics_policy=f"**{Stream.TOPICS_POLICY_DISPLAY_NAME_MAP[old_topics_policy_value]}**",
                new_topics_policy=f"**{Stream.TOPICS_POLICY_DISPLAY_NAME_MAP[topics_policy]}**",
            ),
        )
