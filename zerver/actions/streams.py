...
def get_users_string_with_permission(setting_value: int | AnonymousSettingGroupDict) -> str:
    ...
    groups_with_permission = NamedUserGroup.objects.filter(id__in=setting_value.direct_subgroups)
    group_names = [silent_mention_syntax_for_user_group(group) for group in groups_with_permission]

    users_with_permission = UserProfile.objects.filter(id__in=setting_value.direct_members)
    user_names = [silent_mention_syntax_for_user(user) for user in users_with_permission]

    return ", ".join(group_names + user_names)
