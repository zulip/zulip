...
Since Zulip is 100% open-source software, security researchers have full access
to [Zulip’s codebase](https://github.com/zulip/). To learn about Zulip’s
security model, check out:
...
list](https://groups.google.com/g/zulip-announce) to get notified about new
security releases.
...
To allow us to responsibly remediate security issues, please _do not_ report
them publicly on GitHub, in the Zulip development community, or anywhere else.

Please include the following information in your report:

1. The product where you’ve identified a vulnerability, including the versions
   of the software that you tested.
2. Details of the issue you found, and how it can be reproduced.
...
- Follow our
  [guide](https://zulip.readthedocs.io/en/latest/development/overview.html) to
  set up Zulip’s development environment. This will let you test Zulip without
  risk of disrupting Zulip’s users. The development environment is designed for
  developer convenience (you can log in as any user with `/devlogin`; see also
  `/devtools` features). Historically, the vast majority of security issues
  found in Zulip could be reproduced in the development environment.
...
Zulip operates a HackerOne disclosure program.
...
These classes of issues are out of scope for this program:

- Issues with specific production installations of Zulip. **Do not test against
  an installation of Zulip that you do not own**. This includes
  `chat.zulip.org`, `zulipchat.com`, and any other existing install you might
  find.
- Vulnerabilities in third-party libraries are in scope only if they can be
  fixed by upgrading the version of the third-party library used by Zulip, or
  you have been unable to get the relevant third-party vendor to address the
  vulnerability.

### Requirements for a successful report

- Provide a clear series of steps that maintainers can use to reproduce the
  vulnerability, and any investigation you’ve done into the root cause and
  potential mitigations.
- Responsibly disclose a previously unknown vulnerability directly to us. Do not
  publicly disclose an issue prior to us notifying you that a fix has been
  released.
- You are welcome to use automated tools, including AI, to research
  vulnerabilities. However, please take the time to **_personally_** verify the
  issue, and write the vulnerability description yourself to avoid errors.
  Reporting “vulnerabilities” that were hallucinated by AI wastes the time of
  open-source maintainers.
- Issues that only affect the Zulip development environment must explain how
  they violate the security model _for the development environment_, which
  allows developers to log in as any user.

Thank you for helping us protect Zulip’s user community!
