...
def social_auth_sync_user_attributes(
    ...
    for field_name, attr_name in profile_field_name_to_attr_name.items():
        if field_name == "role":
            ROLE_NAME_TO_TYPE = {
                ...
                "administrator": UserProfile.ROLE_REALM_ADMINISTRATOR,
                "moderator": UserProfile.ROLE_MODERATOR,
                "member": UserProfile.ROLE_MEMBER,
                "guest": UserProfile.ROLE_GUEST,
            }
            ROLE_TYPE_TO_NAME = {v: k for k, v in ROLE_NAME_TO_TYPE.items()}
            attr_value = extra_attrs.get(attr_name)
            if not attr_value:
                continue
            if attr_value not in ROLE_NAME_TO_TYPE:
                ...
                continue
            new_role = ROLE_NAME_TO_TYPE[attr_value]
    ...
    if user_profile is None:
        ...
        if new_role is not None:
            backend.logger.info("Returning role %s for user creation", ROLE_TYPE_TO_NAME[new_role])
        return new_role
    ...
    if new_role is not None and old_role != new_role:
        ...
        backend.logger.info("Set role %s for user %s", ROLE_TYPE_TO_NAME[new_role], user_profile.id)
