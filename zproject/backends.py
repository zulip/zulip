...
class SAMLAuthBackend(SocialAuthMixin, SAMLAuth):
    ...
    def _check_entitlements(self, idp: SAMLIdentityProvider, attributes: Dict[str, List[str]]) -> None:
        ...
        if subdomain not in entitlements:
            # The root subdomain is a special case, as sending an empty string in the list of values
            # of the attribute may not be viable. So, any of the ROOT_SUBDOMAIN_ALIASES can be used
            # to signify the user is authorized for the root subdomain.
            if (subdomain == Realm.SUBDOMAIN_FOR_ROOT_DOMAIN and not settings.ROOT_DOMAIN_LANDING_PAGE and
                    any(alias in entitlements for alias in settings.ROOT_SUBDOMAIN_ALIASES)):
                return
