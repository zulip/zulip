    ...
    def handle_message(self, message, client, state_handler):
        ...
        if original_content_split[1] == "new":
            ...
            try:
                ...
                self = process_guess(original_content, self)
                if self.ended:
...
def get_new_word(bot_self):
    ...
    try:
        ...
        if len(bot_self.currentword) == 0:
            ...
            bot_self.output = "There is already a game of hangman active!" \
                " To stop the current game, type @hangman stop."
    return bot_self


def process_guess(message, bot_self):
    end = ""
    ...
    if len(split) == 2:
        ...
        bot_self.output = "You have already guessed the letter " \
            "" + split[2] + "! To guess again, type @hangman guess <letter>."
    elif split[2] not in "abcdefghijklmnopqrstuvwxyz":
        bot_self.output = split[2] + " is not a letter! To guess a letter, " \
            "type @hangman guess <letter>."
    else:
        bot_self.guessed.append(split[2])
        if split[2] not in bot_self.currentword:
            bot_self.stage += 1
            if bot_self.stage == 10:
                end = "\n·\n:crying_cat_face: Oh no, you lost! " \
                    "The correct word was " + str(bot_self.currentword) + "." \
        ...
        outcur = []
        wrongguess = []
        for i in range(0, len(bot_self.currentword)):
        ...
        if "_" not in outcur:
            ...
            bot_self.ended = True
        for i in range(0, len(bot_self.guessed)):
