...
def resolve_version_string(version: str, *, deploy_path: str) -> str:
    return subprocess.check_output(
        ["git", "rev-parse", version], cwd=deploy_path, preexec_fn=su_to_zulip, text=True
    ).strip()


def maybe_resolve_version_string(version: str, *, deploy_path: str) -> str | None:
    if version in {"", "0.0.0"}:
        return None
    return resolve_version_string(version, deploy_path=deploy_path)
...
def main() -> None:
    ...
    if args.from_git:
        ...
        env["ZULIP_OLD_COMMIT"] = resolve_version_string(old_version, deploy_path=deploy_path)
        env["ZULIP_NEW_COMMIT"] = resolve_version_string(NEW_ZULIP_VERSION, deploy_path=deploy_path)

        old_merge_base_commit = maybe_resolve_version_string(
            old_merge_base, deploy_path=deploy_path
        )
        if old_merge_base_commit is not None:
            env["ZULIP_OLD_MERGE_BASE_COMMIT"] = old_merge_base_commit

        new_merge_base_commit = maybe_resolve_version_string(
            NEW_ZULIP_MERGE_BASE, deploy_path=deploy_path
        )
        if new_merge_base_commit is not None:
            env["ZULIP_NEW_MERGE_BASE_COMMIT"] = new_merge_base_commit
