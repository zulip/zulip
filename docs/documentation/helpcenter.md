...
- [Icon](#icons) components are used to refer to features in the Zulip UI.

- [Include files](#include-files) allow us to reuse repeated content in the
  documentation.

- Our customer [Aside](#asides) components create special highlighted
...
Note that if the include file has any h2 or h3 headers (`##`, `###`), then
you will need to add the same headers to any files where you are importing
and using that include file:
...
There are a built in astro components for creating tabbed instructions:
...
instruction lists, we use a custom `NavigationSteps` component.
...
You can find the code for any custom components at
`starlight_help/src/components`.
