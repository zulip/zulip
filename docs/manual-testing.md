...
There are certain types of bugs that are best caught with old
fashioned manual testing (also called manual QA).  Manual testing
not only catches bugs, but it also helps developers learn more
about the system.
...
**Important**: Make sure that Cordelia is subscribed to Verona but not subscribed
to Denmark.
...
manual testing is targeted more to other interactions.
...
- Markdown stuff
    ...
    - Use the eyeball icon to do print preview and send from preview mode.
    ...
    - Use the emoji icon to find an emoji.
