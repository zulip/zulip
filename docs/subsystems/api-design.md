...
* All API changes must be *compatible*, meaning they
  don't cause existing Zulip clients
  (e.g., existing installs of the Zulip mobile app)
  to break.

* All API changes should be *clean APIs*, meaning
...
* All API changes should be *clearly documented*
...
(TODO who are the channel maintainers; how does one find out?)
...
* Adding new fields to an endpoint's response,
  ...
  * Conversely, this means that if an existing piece of API
    starts including items it previously didn't,
    that change is *not* automatically compatible,
    even if the new items have some new field to distinguish them —
...
* In [Zulip content HTML][],
  ...
  Adding new CSS classes, however, is *not* automatically compatible.
...
* If a change affects only API features that are used
  only by the Zulip web app and no other clients,
  then the change is compatible
  even if it does things like remove or rename a field.

  Typically this is true if a feature is
  used only in the settings UI.
  ...
  * Perhaps check that the terminal app doesn't use the feature
    either.
...
[supported-clients]: ../overview/release-lifecycle.html#server-and-client-app-compatibility
...
remove support for the old form.
...
See [the description above](#process-summary).
...
please use a different channel such as [#backend][].
...
doesn't mistakenly think the changes are already approved.
