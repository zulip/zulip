...
Ideally, when reviewing a branch you are working on, the maintainer should be
able to merge the first few commits and leave comments on the rest. It is by
far the most efficient way to do collaborative development, since one is
constantly making progress, we keep branches small, and developers don't end up
repeatedly reviewing the earlier parts of a PR.

Here is some advice on how to proceed:

- Use `git rebase` as much as you need to shape your commit structure.
