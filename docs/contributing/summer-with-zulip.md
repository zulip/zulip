...
- Don't focus too much on doing precisely what's in the project proposal or
  ...
  - Claim issues using zulipbot only when you actually start work on them. And
    ...
    of issues to work on :D. We'll just find something else for you.
  ...
  - Help test new features! It's fun, and one of the most valuable ways one can
    contribute to an open source project is finding bugs in it :).
...
- Mentors and students should stay in close contact, both with each other and
  ...
  - Daily checkins on #checkins on chat.zulip.org; ideally at some time of day
    ...
    - We prefer checkins in public streams, since it makes easier for other
      contributors to keep track of what everyone else is working on and share
      ideas. Though, of course, feel free to have much more involved/detailed
      checkins.
  ...
  - Video calls are great! Mentors should do 1-3 video calls with their students
  ...
  - Communicate clearly, especially in public places! You'll get much more
    ...
    - And when you update your PR having addressed a set of review feedback, be
      clear about which issues you've resolved (and how!).

    - Post screenshots or brief videos of UI changes; a picture can be worth
      1000 words, especially for verifying whether a design change is working.

    - Use #design and similar forums to get feedback on issues where we need
      community consensus on what something should look like.

  - Bring up problems as soon as you think of them. If you find you're stressed
    about something, mention it your mentor immediately, so they can help you
    solve the problem. If you're stressed about something involving your mentor,
    bring it up with an organization admin.
  ...
  - Join Zulip's GitHub teams that relate to your projects and/or interests, so
    ...
    You can browse the area teams here: https://github.com/orgs/zulip/teams
...
- Everyone's goal is to avoid students ending up blocked and feeling stuck.
  ...
  - Get really good at using `git rebase -i` to produce a really clean commit
    history that's fast to review. We should probably do some workshops on how
    to do relatively complex rebases (any volunteers?).
...
- Help with code review! Reviewing others' changes is one of the best ways to
  ...
  - It's best to start with areas where you know the surrounding code and
    expertise, but don't be afraid to open up the code in your development
    environment and read it rather than trying to understand everything from the
    context GitHub will give you. Tim reads the code path locally all the time
    when reviewing things, and so should you :).
