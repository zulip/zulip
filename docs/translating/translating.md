...
1. Optionally, use the "Components" tab at the top to translate only
   ...
   - `Destop` is for the parts of the Zulip desktop apps that are not
     shared with the Zulip web app.  This is a fairly small number of
     strings.
   - `Django` and `Frontend` and have strings for the next major
     release of the Zulip server and web app (which is what we run on
...
- When context is unclear, you may find it helpful to following at the
...
- While one should definitely prioritize translating the `Frontend`
  component, since the most prominent user-facing strings are there,
  API error messages in `Django` are presented to users, so a full
  translation should include them.
...
1. Add the Weblate remote to your `git` repository:
  ...
  ```shell
  git remote add weblate https://hosted.weblate.org/git/zulip/django/https://hosted.weblate.org/git/zulip/django/
  ```
