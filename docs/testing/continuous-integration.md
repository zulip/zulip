...
[Travis CI](https://travis-ci.org/) for it's CI. CircleCI is the
primary CI that runs frontend and backend tests across a wide range
of Ubuntu distributions. Travis CI is used only for running the
end-to-end production installer test. This page documents useful
tools and tips to know about when using CircleCI and Travis CI and
debugging issues with them.
...
We currently run three different jobs during a CircleCI build. They are
named as
* trusty-python-3.4
* xenial-python-3.5
* bionic-python-3.6

What this means is we run tests on three different Ubuntu releases with
each having it's own Python version pre-installed. `bionic-python-3.6`
job for example runs the tests in Ubuntu Xenial with Python 3.6
pre-installed. Your build for the PR would be marked successful only if
all the 3 jobs are executed successfully. If one job fails the entire
build is marked as failed.

What happens during a job is described in the `jobs` section. Each `job`
...
After spinning the container from image, CircleCI will create the
directory mentioned in `working_directory` and all the steps would be
run from here.

The `steps` section describes describes everything from fetching the
Zulip code, restoring cache, installing dependencies etc to running
tests and uploading coverage reports. The steps with prefix `*` is an
alias. The actual description of the alias is in `aliases` section.
...
* CircleCI [allows you to SSH][circleci-ssh] into the job container if a job
fails. SSHing into the containers can be helpful for figuring out what went
wrong especially in rare cases where the tests are passing in your computer
while failing in the CI. Make sure that you have uploaded your SSH keys to
GitHub else you wont be able to SSH into the container than ran the job.
...
* You can [sign up your personal repo for CircleCI][circleci-setup] so that every
remote branch you push will be tested, which can be helpful when debugging something
complicated.
