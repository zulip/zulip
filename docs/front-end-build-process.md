...
All JavaScript we provide will eventually be migrated to Typescript which make
refactoring the frontend code easier and allow the static analyzers reason about
out code more easily.

Delcare and entry points in webpack.assets.json. Any modules you add will need to
be required or imported from this file (or one of its dependencies) in order to
be included in the script bundle.
