...
- Redesigned the left sidebar visually, adding convenient "New topic"
  ...
  to 2 lines. The collapsed state of the direct messages section not
...
- Invitation links can now encode initial groups for a user.
...
- Group settings was redesigned, with many new administration
...
- Added a right sidebar buddy list style option showing user avatars,
  designed for smaller organizations. You can now change the buddy
  list style directly from a menu in the sidebar.
...
- Reworked the "archive channel" functionality to not mutate their
  names or always make content inaccessible, with a view towards
  supporting unarchiving channels.
...
- Added support for personal Microsoft accounts in the EntraID
  (formerly AzureAD) authentication backend. Updated documentation to
  consistently refer to this authentication method as Microsoft Entra
  ID.
- Added a new "Forward message" message in the message actions menu,
...
- Added a wizard for creating collaborative TODO widgets.
...
- Fixed a bug that allowed the creation of invalid linkifirs.
