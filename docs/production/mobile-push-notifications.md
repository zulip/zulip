...
1. Uncomment the `PUSH_NOTIFICATION_BOUNCER_URL = 'https://push.zulipchat.com'` line in your `/etc/zulip/settings.py`
...
(`zulip_org_key`), you need to use `manage.py register_server --rotate-key` option; it will automatically generate a new
...
-   We have added a key APNS_TOPIC in `settings.py` and initialized it with a dummy constant string and in `push_notifications.py` we have used this constant and replaced the `topic` with this constant.
-   With this change, nothing in the standard install is impacted, but anyone doing a custom iOS app to allow for push notifications directly from private server can send notifications. With this, they can add their app's Bundle identifier to topic for sending push notifications in case of iOS.
