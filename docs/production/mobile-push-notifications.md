...
Zulip Server 12.0+ supports end-to-end encrypted (E2EE) mobile push
notifications. The Zulip server encrypts the contents of each notification
...
Mobile app support for the E2EE protocol is [in progress][e2ee-flutter-issue];
clients without E2EE support continue to use the [legacy
protocol](#legacy-protocol) described below.

[e2ee-flutter-issue]: https://github.com/zulip/zulip-flutter/issues/1764

### What the Push Notification Service sees

For each notification, the Push Notification Service receives and forwards:

- The opaque APNs or FCM token identifying the target device (known to the
  service from registration; never seen in plaintext by the Zulip server).
- An identifier selected by the client (`push_key_id`) that the mobile app
  uses to select the correct decryption key. This identifier is opaque to
  the service.
- The encrypted notification payload (a libsodium `crypto_secretbox_easy`
...
- The delivery priority (higher for a new message, lower for a notification
  removal) and the size of the encrypted payload.

The Push Notification Service does **not** see the message contents, the
sender's name or avatar, the recipient set (channel and topic, or direct
message participants), or any other Zulip-specific metadata about the
...
- [Basic organization metadata](#uploading-basic-metadata),
  ...
  statistics about how many push notifications are sent by each customer.
...
APNs and FCM necessarily learn which of their devices received a
notification, when, at what priority, and how large the payload was. The
notification contents — including the message, sender, recipient, channel,
and topic — are inside an encrypted ciphertext that Apple and Google
cannot decrypt. Beyond that ciphertext, the push payload contains a
`push_key_id` integer that the mobile app uses to select the correct
decryption key, and — on APNs only — a fixed fallback title ("New
notification") that iOS displays if the Zulip app's notification service
extension does not decrypt the notification and substitute its contents
before iOS presents it. FCM messages are data-only, and the Android app
decrypts them before any notification is displayed.
...
delivery. E2EE makes notification contents opaque to Apple, Google,
and the Push Notification Service; it does **not** protect against a
compromised or malicious Zulip server. Server administrators should
continue to secure their Zulip server with the same rigor as the
...
- **No forward secrecy by default.** A device's symmetric key persists
  until the mobile app rotates it, and is stored by both the mobile app
  and the Zulip server. An attacker who obtains both a historical capture
  of encrypted push payloads and the device's current key — for example,
  by compromising the mobile device or the Zulip server — can decrypt
  previously captured notifications.
...
- **Metadata side channels.** The Push Notification Service, and
  ...
  coarse patterns of activity even when message contents are encrypted.

- **Lost devices.** The mobile app stores its copy of the per-device
  symmetric key locally, without backup. A user who loses access to a
  device cannot recover its historical notifications; reinstalling the
  app generates a fresh key and re-registers the device with your Zulip
  server and the Push Notification Service.
...
a per-device 32-byte symmetric key and a fresh 192-bit random nonce per
...
[`zerver/lib/push_notifications.py`](https://github.com/zulip/zulip/blob/main/zerver/lib/push_notifications.py)
and
[`zilencer/views.py`](https://github.com/zulip/zulip/blob/main/zilencer/views.py)
in the Zulip server repository.

### Governance and operations

The code for the push notification forwarding service is 100% open source
and available as part of the [Zulip server project on
GitHub](https://github.com/zulip/zulip) (specifically,
[here](https://github.com/zulip/zulip/tree/main/zilencer)). The push
notification forwarding servers are professionally managed by a small team
of security-sensitive engineers.
...
For mobile clients that do not yet support E2EE, the Zulip server sends
notification contents to the Push Notification Service in plaintext (over
TLS); the service forwards the payload to Apple or Google without storing
it. Under this protocol, the Push Notification Service additionally
receives (but does not store) the contents of individual mobile push
notifications:

- The numeric message ID generated by your Zulip server.
- Metadata on the message's sender (name and avatar URL).
- Metadata on the message's recipient (channel name + ID, topic, direct
  message recipients, etc.).
- A timestamp.
- The message's content.

For each device registered under this protocol, the Push Notification
Service also stores the user ID number generated by your Zulip server
(used to route notifications to the correct set of devices). These user
ID numbers are opaque to the Push Notification Service and Kandra Labs.

Organization administrators can require E2EE for all push notifications by
enabling **Require end-to-end encryption for push notification content**.
When this is enabled and a user's mobile client does not support E2EE, the
server omits the message contents and the user sees "New message" in place
of the notification text. See the [help center
documentation](https://zulip.com/help/mobile-notifications#end-to-end-encryption-e2ee-for-mobile-push-notifications)
for the user-facing behavior. (Prior to Zulip Server 11.0, the same
redaction behavior was controlled by the `PUSH_NOTIFICATION_REDACT_CONTENT`
