...
* You cannot modify JavaScript, CSS, or other frontend files this way,
  because they need to be compiled by Webpack, and we don't include
  the uncompiled/source versions of these assets in our production
  releases as they are very large and are not used at runtime.
...
* Decide where you're going to edit Zulip's code.  We recommend [using
  ...
  strictly need it; just an environment with Git installed.
...
* Because of how container images work, editing files directly is an
  even worse idea, because Docker is designed for working with
...
Because Zulip can be deployed in a wide variety of configurations and
environments, we often merge fixes to bugs that we can reproduce in an
automated test but have not directly reproduced in a production
system.  In these cases, we'll ask the user(s) who reported the bug to
apply the patch to their production system to verify the fix works for
them.
...
* Changes that are stacked on top of other changes to the same system
  (generally this will be clear because).
...
* Master is our main development branch under very active development
  and has dozens of new changes added most days.  Master can have
  thousands of changes not present in the latest release (all of which
  will be included in our next release).  There are probably some bugs.
...
* The development community is very interested in helping debug issues
  when upgrading from the latest release to master, since they provide
  us an opportunity to fix that category of issue before our next
  major release.  (Much more so than we are in helping folks debug
  other custom changes).  That said, we cannot make any guarantees
  about how quickly we'll resolve an issue to folks without a formal
  support contract.
