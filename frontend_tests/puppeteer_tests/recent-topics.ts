...
async function mute_topic_test(page: Page): Promise<void> {
    ...
    const topics_quantitiy = await page.evaluate(() => $('#recent_topics_table tr[id^="recent_topic"]').length);
    const last_topic_id = await click_mute_and_return_last_topic_id(page);

    await page.waitForFunction(
        (expected_length: number) => $('#recent_topics_table tr[id^="recent_topic"]').length === expected_length,
        {},
        topics_quantitiy - 1,
    );
