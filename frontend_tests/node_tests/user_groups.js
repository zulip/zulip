...
run_test("user_groups", () => {
    ...
    const id_user_not_in_group = 0;
    const id_user_in_group = 2;
    ...
    const groups_of_users = user_groups.get_user_groups_of_user(id_user_in_group);
    assert.equal(groups_of_users.length, 1);
    assert.equal(groups_of_users[0].name, "Everyone");

    const groups_of_users_nomatch = user_groups.get_user_groups_of_user(id_user_not_in_group);
