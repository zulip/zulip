...
/**
 * Returns index of the first element in {@link array} greater than or equal to {@link e}.
 * @param {T[]} array A sorted array.
 * @param {T} e       An element of the array.
 * @template T
 * @returns {number}
 */
function binary_search(array, e) {
    // Finding largest power of 2 that is less than the length.
    var l = 1;
...
function get_bidi_class(ch) {
    ...
    var i = binary_search(rl_ranges, ch);
    if (i < rl_ranges.length && (rl_ranges[i] === ch || i % 2 === 1)) {
        return 'R'; // R, AL
    }
    i = binary_search(lr_ranges, ch);
