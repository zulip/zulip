...
export function process_messages(messages) {
  ...
  // if this is a single message event, do an inplace_rerender
  const len = messages.length;
  if (len == 1 && topic_data_changed) {
    const topic_key = get_key_from_message(messages[0]);
    inplace_rerender(topic_key)
  }
  // if this is not single message, do a complete rerender
  if (len != 1 && topic_data_changed) {
    complete_rerender();
  }
  // what it was before 
