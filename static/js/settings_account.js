...
export function update_full_name(new_full_name) {
  ...
  const $full_name_input = $(
    ".full-name-change-container input[name='full_name']",
  );
  if ($full_name_input) {
...
function display_avatar_upload_complete() {
  $("#user-avatar-upload-widget .upload-spinner-background").css({
    visibility: "hidden",
  });
...
function update_custom_profile_field(field, method) {
  ...
  const $spinner_element = $(
    `.custom_user_field[data-field-id="${CSS.escape(
      field_id,
    )}"] .custom-field-status`,
  ).expectOne();
...
export function set_up() {
  ...
  function do_change_password(e) {
    e.preventDefault();
    e.stopPropagation();
    const $change_password_error = $("#change_password_modal").find(
      "#dialog_error",
    );
    ...
    if (new_pw !== "") {
      if (password_quality === undefined) {
        ...
        settings_change_error(
          $t_html({defaultMessage: "New password is too weak"}),
        );
        return;
      }
    }
  ...
  function do_change_email(e) {
    ...
    const opts = {
      ...
      success_msg_html: $t_html(
        {
          defaultMessage:
            "Check your email to confirm the new address: <br/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp {email}",
        },
        {email: data.email},
      ),
      sticky: true,
  ...
  function change_email_post_render() {
    const $input_elem = $("#change_email_container").find(
      "input[name='email']",
    );
