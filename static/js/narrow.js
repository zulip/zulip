...
function show_invalid_narrow_message(invalid_narrow_message) {
    var search_string_display = $("#empty_search_stop_words_string");
    // Also removes previous text if any
    search_string_display.text(i18n.t(invalid_narrow_message));
}
...
function show_search_query() {
    ...
    // Add in stream:foo and topic:bar
    ...
    var stream_topic_terms = _.filter(current_filter.operators(), function (term) { return term.operator === "stream" || term.operator === "topic";});
    if (stream_topic_terms.length !== 0) {
...
function pick_empty_narrow_banner() {
    ...
    if (num_operators !== 1) {
        ...
        if (streams.length > 1) {
            invalid_narrow_message = "You are searching for messages that belong to more than one stream, which is not possible.";
        }
        // No message can have multiple topics
        if (current_filter.operands("topic").length > 1) {
            invalid_narrow_message = "You are searching for messages that belong to more than one topic, which is not possible.";
        }
        // No message can have multiple senders
        if (current_filter.operands("sender").length > 1) {
            invalid_narrow_message = "You are searching for messages that are sent by more than one person, which is not possible.";
        }
        if (invalid_narrow_message !== "") {
            show_invalid_narrow_message(invalid_narrow_message);
