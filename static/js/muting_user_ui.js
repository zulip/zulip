...
exports.persist_user_mute = function (muted_user) {
    var data = { muted_user_profile_id: muted_user[0] };
    last_update = timestamp_ms();
    channel.put({
        url: '/json/users/me/user_mute/' + muted_user[0],
        idempotent: true,
        data: data,
        success: function () {
        ...
        error: function (xhr) {
                var response = channel.xhr_error_message("Error muting user",xhr);
                blueslip.warn(response);
        },
   });
};

exports.persist_user_unmute = function (muted_user) {
    var data = { muted_user_profile_id: muted_user[0] };
    last_update = timestamp_ms();
    channel.del({
        url: '/json/users/me/user_mute/' + muted_user[0],
        ...
        error: function (xhr) {
                var response = channel.xhr_error_message("Error unmuting user",xhr);
                blueslip.warn(response);
        },
    });
};

exports.handle_mute_updates = function (muted_users) {
    // muted_users is a list of tuples with each tuple
   // containing muted_user_id and muted_user_name
...
exports.mute_user = function (muted_user) {
    exports.persist_user_mute(muted_user);
    exports.notify_mute_with_undo_option(muted_user[0], muted_user[1]);
...
exports.unmute_user = function (muted_user) {
    ...
    // if someone accidentally unmutes a user rather than if they mute it
    // and miss out on info.
    exports.persist_user_unmute(muted_user);
    if (muting_user.is_user_muted(muted_user[0])) {
      muting_user.remove_muted_user(muted_user);
    }
    exports.set_up_muted_users_ui(muting_user.get_muted_user_names());
