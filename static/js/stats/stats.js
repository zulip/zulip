...
function populate_messages_sent_over_time(data) {
    ...
    function add_hover_handler() {
        document.getElementById('id_messages_sent_over_time').on('plotly_hover', function (data) {
            ...
            var hovers = ['hover_human', 'hover_bot', 'hover_me'];
            var hovers_values = ['hover_human_value', 'hover_bot_value', 'hover_me_value'];
            for (var i = 0; i < values.length; i += 1) {
                if (values[i] !== null) {
                    document.getElementById(hovers[i]).style.display = 'inline';
                    document.getElementById(hovers_values[i]).style.display = 'inline';
                    document.getElementById(hovers_values[i]).innerText = values[i];
                } else {
                    document.getElementById(hovers[i]).style.display = 'none';
                    document.getElementById(hovers_values[i]).style.display = 'none';
                }
            }
        });
    }
