...
When a Zulip message of form `![title](url)` is sent linking to one of
the supported audio file formats -- `.aac`, `.flac`, `.mp4`, `.mpeg`,
`.wav`, `.webm`, Zulip will generate a HTML audio player element with
