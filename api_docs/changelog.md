...
* [`GET /events`](/api/get-events): Added `realm_export_consent` event type
  which is sent to realm administrators when a user changes their
  `allow_private_data_export` privacy setting or a human user is reactivated.
