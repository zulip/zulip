...
type StreamContext = {
    ...
    channel_unread_count_color: string;
    stream_url: string;
    stream_id: number;
    is_hidden: boolean;
    is_collapsed: boolean;
...
function format_stream(stream_id: number): StreamContext {
    ...
    return {
        ...
        channel_unread_count_color: stream_color.get_channel_unread_count_color(stream_info.color),
    ...
    for (const [topic, {topic_count, latest_msg_id}] of topic_dict) {
        ...
        if (topic_count) {
            const topic_data = format_topic(
                ...
                topic_count,
                latest_msg_id,
                stream_data.channel_unread_count_color,
            );
...
function get_folder_name_from_id(folder_id: number): string {
    ...
    return channel_folders.get_channel_folder_name_from_id(folder_id);
}
...
function reset_data(): {
    ...
    if (is_only_other_channels_folder_visible()) {
...
function is_only_other_channels_folder_visible(): boolean {
...
class InboxTopicListWidget extends TopicListWidget {
    ...
    override build(): this {
        ...
        const channel_color = sub_store.get(this.my_stream_id)!.color;
        const channel_unread_count_color =
            stream_color.get_channel_unread_count_color(channel_color);
        ...
        if (channel_folders.has(folder_id)) {
            ...
            if (index === 0) {
                ...
                const $previous_folder = $(
                    `#${CSS.escape(get_channel_folder_header_id(previous_folder_id))}`,
                );
...
export function update(): void {
    ...
    for (const [stream_id, topic_dict] of unread_streams_dict) {
        ...
        if (stream_unread_count > 0) {
            ...
            const channel_unread_count_color = stream_color.get_channel_unread_count_color(
                sub_store.get(stream_id)!.color,
            );
            for (const [topic, {topic_count, latest_msg_id}] of topic_dict) {
                const topic_key = get_topic_key(stream_id, topic);
                if (topic_count) {
...
export function initialize({hide_other_views}: {hide_other_views: () => void}): void {
    ...
    $("body").on("click", "#inbox-list .inbox-left-part-wrapper", function (this: HTMLElement, e) {
        ...
        if (href !== undefined) {
            ...
            $elt = $elt.closest(".inbox-folder");
            col_focus = COLUMNS.COLLAPSE_BUTTON;
            collapse_or_expand($elt.closest(".inbox-folder").attr("id")!);
        }
        focus_clicked_list_element($elt);
    });
