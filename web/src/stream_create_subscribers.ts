...
export async function add_subscribers_from_pills(): Promise<
    {status: "synced" | "form_closed"} | {status: "fetch_failed"; failed_stream_id: number}
> {
    ...
    // Disable the pill input while we wait, since pills added or
    // removed during the wait would not be reflected in the channel.
    ...
    $add_subscribers_container.find(".input").prop("contenteditable", false);
    const current_pill_widget = pill_widget;
    const user_ids = await add_subscribers_pill.get_pill_user_ids(pill_widget);
    $add_subscribers_container.removeClass("add_subscribers_disabled");
    $add_subscribers_container.find(".input").prop("contenteditable", true);
    ...
    if (!Array.isArray(user_ids)) {
        return {status: "fetch_failed", failed_stream_id: user_ids.failed_stream_id};
    }
