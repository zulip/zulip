...
async function await_prejoin_decision(state: PrejoinState): Promise<PrejoinResult | null> {
    return new Promise<PrejoinResult | null>((resolve) => {
        const cleanup = (): void => {
            if (state.detach_meter !== null) {
                state.detach_meter();
...
export async function run_prejoin(is_video_call: boolean): Promise<PrejoinResult | null> {
    ...
    return await_prejoin_decision(state);
}
