...
export function message_unhover(): void {
    ...
    clearTimeout(Number($current_message_hover.attr("data-edit-timeout-id")));
    $current_message_hover.removeAttr("data-edit-timeout-id");
...
function change_edit_content_button($message_row: JQuery, message: Message): void {
    ...
    if (!$message_row.attr("data-edit-timeout-id")) {
        const remaining_edit_time = message_edit.remaining_content_edit_time(message) * 1000;
        if (remaining_edit_time > 0) {
            const timeout_id = setTimeout(() => {
                change_edit_content_button($message_row, message);
            }, remaining_edit_time);
            $message_row.attr("data-edit-timeout-id", JSON.stringify(timeout_id));
        }
    }
}
