...
export function set_expires_on_text(
    $elem: JQuery,
    ...
    if ($elem.val() === "custom") {
        ...
        set_custom_expires_on_text($elem, custom_input_time_value, valid_to_text);
    } else {
...
export function set_custom_expires_on_text(
    $elem: JQuery,
    custom_input_time_value: number,
    valid_to_text: string,
): void {
    if (util.validate_custom_time_input(custom_input_time_value)) {
        $elem.parent().find(".custom-input-time-valid-till").text(valid_to_text);
    } else {
        $elem
            .parent()
            .find(".custom-input-time-valid-till")
            .text($t({defaultMessage: "Invalid custom time"}));
    }
}
