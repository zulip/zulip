    ...
    for (const input_elem of properties_elements) {
        ...
        if (check_realm_settings_property_changed(input_elem)) {
            ...
            if (input_value !== undefined && input_value !== null) {
                ...
                if (property_name === "direct_message_initiator_group") {
                    data[property_name] = JSON.stringify({
                        new: input_value,
                        old: realm.realm_direct_message_initiator_group,
                    });
                    continue;
                }
                if (property_name === "direct_message_permission_group") {
                    data[property_name] = JSON.stringify({
                        new: input_value,
                        old: realm.realm_direct_message_permission_group,
                    });
                    continue;
                }
