...
const ZulipUploadResponse = z.object({
    url: z.string(),
    filename: z.string(),
});
...
export function setup_upload(config: Config): Uppy<Meta, TusBody> {
    ...
    uppy.on("upload-success", (file, response) => {
        ...
        let upload_response;
        try {
            upload_response = ZulipUploadResponse.parse(JSON.parse(response.body!.xhr.response));
        } catch {
            blueslip.warn("Invalid JSON response from tus server", response.body!.xhr.response);
            return;
        }
