...
import render_generate_integration_url_jira_additional_fields_modal from "../templates/settings/generate_integration_url_jira_additional_fields_modal.hbs";
...
type JiraCustomFieldPill = {
    type: "jira_custom_field";
    value: string;
};

const DEFAULT_JIRA_CUSTOM_FIELDS = ["assignee", "priority"];

function create_item_from_jira_custom_field_name(
    value: string,
    current_items: JiraCustomFieldPill[],
): JiraCustomFieldPill | undefined {
...
function create_jira_custom_field_pills(
    $pill_container: JQuery,
): input_pill.InputPillContainer<JiraCustomFieldPill> {
    return input_pill.create({
        $container: $pill_container,
        create_item_from_text: create_item_from_jira_custom_field_name,
...
export function show_generate_integration_url_modal(api_key: string): void {
    ...
    function generate_integration_url_post_render(): void {
        ...
        let jira_custom_field_pill_widget:
            | input_pill.InputPillContainer<JiraCustomFieldPill>
            | undefined;
        ...
        function show_jira_custom_fields_ui(): void {
            const show_additional_fields = $(
                "#integration-url-jira-additional-fields-checkbox",
            ).prop("checked");
            $("#integration-url-jira-additional-fields").toggleClass(
                "hide",
                !show_additional_fields,
            );

            const $pill_container = $("#integration-url-jira-additional-fields .pill-container");
            if ($pill_container.length > 0 && jira_custom_field_pill_widget === undefined) {
                jira_custom_field_pill_widget = create_jira_custom_field_pills($pill_container);
                jira_custom_field_pill_widget.onPillCreate(() => {
                    update_url();
                });
                jira_custom_field_pill_widget.onPillRemove(() => {
                    update_url();
                });

                for (const field_name of DEFAULT_JIRA_CUSTOM_FIELDS) {
                    jira_custom_field_pill_widget.appendValidatedData(
                        {
                            type: "jira_custom_field",
                            value: field_name,
                        },
            ...
            if (show_additional_fields) {
                $("#integration-url-jira-additional-fields-text").trigger("focus");
            }
            update_url();
        }
        ...
        function render_url_options(config: UrlOption[]): void {
            ...
            for (const option of validated_config) {
                ...
                case PresetUrlOption.JIRA_CUSTOM_FIELDS: {
                    const jira_additional_fields_html =
                        render_generate_integration_url_jira_additional_fields_modal();
                    $config_element = $(jira_additional_fields_html);
                    $config_element
                        .find("#integration-url-jira-additional-fields-checkbox")
                        .on("change", () => {
                            show_jira_custom_fields_ui();
                        });
        ...
        function update_url(render_events = false): void {
            ...
            if (url_options) {
                for (const option of url_options) {
                    ...
                    case PresetUrlOption.JIRA_CUSTOM_FIELDS: {
                        if (
                            !$("#integration-url-jira-additional-fields-checkbox").prop("checked")
                        ) {
                            continue;
                        }

                        const custom_fields = jira_custom_field_pill_widget
                            ?.items()
                            .map((item) => item.value)
                            .filter(
                                (value) =>
                                    !DEFAULT_JIRA_CUSTOM_FIELDS.includes(value.toLowerCase()),
                            )
                            .join(",");
                        if (custom_fields) {
        ...
        function reset_to_blank_state(): void {
            ...
            jira_custom_field_pill_widget = undefined;
        }
    }
