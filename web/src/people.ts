...
function get_combined_promise_for_user_ids(user_ids: Set<number>): {
    promise_for_user_ids_in_transit: Promise<unknown>;
    ...
    for (const user_id of user_ids_pending_fetch) {
        if (people_by_user_id_dict.has(user_id)) {
            /* istanbul ignore next */
            user_ids_pending_fetch.delete(user_id);
        }
    }

    return {
        promise_for_user_ids_in_transit: Promise.all(promises),
...
function start_fetch_for_requested_users(): void {
    ...
    const {promise_for_user_ids_in_transit, user_ids_pending_fetch} =
        get_combined_promise_for_user_ids(user_ids_to_fetch);

    // This promise will be resolved when all users are fetched.
    fetch_users_storage.promise_for_in_transit.set(user_ids_to_fetch, {
...
export async function fetch_users_from_ids_internal(user_ids: number[]): Promise<unknown> {
    ...
    if (
        ...
        return promise_for_user_ids_in_transit;
    }
