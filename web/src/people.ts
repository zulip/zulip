...
export function get_valid_user_ids(): Set<number> {
    // Return a copy of the set to avoid external mutation.
    return new Set(valid_user_ids);
}
...
export async function fetch_users(user_ids: Set<number>): Promise<UsersFetchResponse["members"]> {
    const invalid_user_ids = user_ids.difference(get_valid_user_ids());
    if (invalid_user_ids.size > 0) {
        blueslip.info("Ignored invalid user_ids: " + [...invalid_user_ids].join(", "));
    }
    const user_ids_to_fetch = get_valid_user_ids().intersection(user_ids);
...
export async function initialize(
    ...
    const user_ids_to_fetch = get_valid_user_ids();
