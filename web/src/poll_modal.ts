...
function create_option_row($last_option_row_input: JQuery<HTMLInputElement>): void {
    const row = render_poll_modal_option();
    const $row_container = $last_option_row_input.closest(".simplebar-content");
    $row_container.append(row);
}

function add_option_row(e: {target: JQuery<HTMLInputElement>}): void {
...
function delete_option_row(e: {target: JQuery<HTMLInputElement>}): void {
...
export function poll_options_setup(): void {
    ...
    $question_input.on("input", () => {
        if ($question_input.val()!.toString().trim() !== "") {
...
export function frame_poll_message_content(): string {
    const question = $("#poll-question-input").val()!.toString().trim();
