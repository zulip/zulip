...
export function setup_topic_search_typeahead(): void {
    ...
    const options = {
        source() {
            ...
            return topic_filter_pill.filter_options.filter(
                (option) => !current_syntaxes.has(option.syntax),
            );
        },
    ...
    $input.on("keydown", (e: JQuery.KeyDownEvent) => {
        if (e.key === "Enter") {
            ...
            if (text === "-is:followed" && topic_filter_pill_widget) {
                topic_filter_pill_widget.appendValue(text);
                set_search_bar_text("");
            }
            e.preventDefault();
            e.stopPropagation();
