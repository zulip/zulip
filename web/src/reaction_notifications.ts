...
export function reaction_is_notifiable(message: Message): boolean {
    ...
    // Do not notify reactions if the user has set reaction notifications to "Never".
    if (
    ...
    // Notify for reactions in private messages if the user has enabled reaction notifications.
    if (message.type === "private") {
        return true;
    }

    // Notify for reactions in stream messages if the user has set reaction notifications to "Always".
    if (
        message.type === "stream" &&
        user_settings.reaction_notifications_policy ===
    ...
    // and the user has set reaction notifications to "DMs and followed topics".
    if (
        message.type === "stream" &&
    ...
    // and the user has set reaction notifications to "Unmuted messages".
    if (
        message.type === "stream" &&
