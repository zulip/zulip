...
const message_reactions = new Map<string, MessageReactions>();
...
function get_reaction_notification_title(emoji_set: Set<string>, user_ids: Set<number>): string {
    ...
    for (const emoji_name of [...emoji_set].toReversed()) {
...
export function process_notification(notification: {
    ...
    const msg_count = 1;
    let emojis = new Set([emoji_name]);
    let user_ids = new Set([user_id]);

    const notice_memory = desktop_notifications.notice_memory.get(key);
    if (notice_memory) {
        const message_reaction = message_reactions.get(key);
        assert(message_reaction);

        emojis = message_reaction.emojis;
        user_ids = message_reaction.user_ids;

        // We want the newest reactions to show up earlier when we
        // construct the notification body even if it already exists.
        emojis.delete(emoji_name);
    ...
    const title = get_reaction_notification_title(emojis, user_ids);
    ...
    if (notification.desktop_notify && desktop_notifications.NotificationAPI !== undefined) {
        ...
        message_reactions.set(key, {emojis, user_ids});
