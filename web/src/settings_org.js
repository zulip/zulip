...
export function init_dropdown_widgets() {
    ...
    const direct_message_permission_group = new dropdown_widget.DropdownWidget({
        ...
        item_click_callback(event, dropdown, widget) {
            ...
            settings_components.save_discard_widget_status_handler(
                $("#org-direct-message-permissions"),
            );
            if (widget.current_value === user_groups.get_user_group_from_name("role:nobody").id) {
        ...
        default_id: realm.realm_direct_message_permission_group,
        unique_id_type: dropdown_widget.DataTypes.NUMBER,
        on_mount_callback(dropdown) {
            $(dropdown.popper).css("min-width", "300px");
        },
    });
    settings_components.set_direct_message_permission_group_widget(direct_message_permission_group);
    ...
    const direct_message_initiator_group = new dropdown_widget.DropdownWidget({
        ...
        default_id: realm.realm_direct_message_initiator_group,
        unique_id_type: dropdown_widget.DataTypes.NUMBER,
        on_mount_callback(dropdown) {
            $(dropdown.popper).css("min-width", "300px");
        },
    });
    settings_components.set_direct_message_initiator_group_widget(direct_message_initiator_group);
