...
export function get_org_type_dropdown_options() {
    const current_org_type = state_data.realm_org_type;
...
function set_realm_waiting_period_setting() {
    const setting_value = state_data.realm_waiting_period_threshold;
...
function update_jitsi_server_url_custom_input(dropdown_val) {
    ...
    $custom_input_elem.val(state_data.realm_jitsi_server_url);
}
...
function set_jitsi_server_url_dropdown() {
    ...
    if (state_data.realm_jitsi_server_url) {
        dropdown_val = "custom";
    }
...
function set_video_chat_provider_dropdown() {
    const chat_provider_id = state_data.realm_video_chat_provider;
...
function set_giphy_rating_dropdown() {
    const rating_id = state_data.realm_giphy_rating;
...
function set_message_content_in_email_notifications_visibility() {
    settings_components.change_element_block_display_property(
        ...
        state_data.realm_message_content_allowed_in_email_notifications,
    );
}
...
function set_digest_emails_weekday_visibility() {
    settings_components.change_element_block_display_property(
        ...
        state_data.realm_digest_emails_enabled,
    );
}
...
function set_create_web_public_stream_dropdown_visibility() {
    settings_components.change_element_block_display_property(
        ...
        state_data.server_web_public_streams_enabled &&
            state_data.zulip_plan_is_not_limited &&
...
function update_dependent_subsettings(property_name) {
    ...
    switch (property_name) {
        case "realm_allow_message_editing":
            update_message_edit_sub_settings(state_data.realm_allow_message_editing);
            break;
        case "realm_delete_own_message_policy":
            set_delete_own_message_policy_dropdown(state_data.realm_delete_own_message_policy);
            break;
...
export function discard_property_element_changes(elem, for_realm_default_settings, sub, group) {
    ...
    switch (property_name) {
        ...
        case "realm_org_type":
            ...
            // dropdown menu options whenever state_data.realm_org_type
...
export function init_dropdown_widgets() {
    ...
    const notifications_stream_widget = new dropdown_widget.DropdownWidget({
        ...
        default_id: state_data.realm_notifications_stream_id,
    ...
    const signup_notifications_stream_widget = new dropdown_widget.DropdownWidget({
        ...
        default_id: state_data.realm_signup_notifications_stream_id,
        unique_id_type: dropdown_widget.DATA_TYPES.NUMBER,
        text_if_current_value_not_in_options: $t({defaultMessage: "Cannot view stream"}),
    });
    ...
    const default_code_language_widget = new dropdown_widget.DropdownWidget({
        ...
        $events_container: $("#settings_overlay_container #organization-settings"),
        default_id: state_data.realm_default_code_block_language,
    ...
    const create_multiuse_invite_group_widget = new dropdown_widget.DropdownWidget({
        ...
        default_id: state_data.realm_create_multiuse_invite_group,
        unique_id_type: dropdown_widget.DATA_TYPES.NUMBER,
    ...
    const can_access_all_users_group_widget = new dropdown_widget.DropdownWidget({
        ...
        default_id: state_data.realm_can_access_all_users_group,
        unique_id_type: dropdown_widget.DATA_TYPES.NUMBER,
        on_mount_callback(dropdown) {
            $(dropdown.popper).css("min-width", "300px");
        },
...
export function build_page() {
    ...
    populate_realm_domains_label(state_data.realm_domains);

    // Populate authentication methods table
    populate_auth_methods(state_data.realm_authentication_methods);
    ...
    $("#id_realm_org_join_restrictions").on("change", (e) => {
        ...
        if (org_join_restrictions === "only_selected_domain") {
            $node.show();
            if (state_data.realm_domains.length === 0) {
    ...
    if (state_data.zulip_plan_is_not_limited) {
