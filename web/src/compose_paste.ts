...
export function paste_handler(
    ...
    if (clipboardData.getData) {
        ...
        // We don't place this code block inside the isUrl code block above since
        // we might be looking to replace text that might return false from `isUrl`
        // but our url_pattern might be targeting.
        if (!compose_ui.cursor_inside_code_block($textarea) && !compose_ui.shift_pressed) {
