...
// There might be some false positives while pasting only tables from something like LibreOffice Writer.
function is_from_libreoffice_calc(body_tag: HTMLBodyElement): boolean {
    ...
    if (
        !body_tag ||
        // Check that <body> has only one child element to avoid misclassification.
        // Multiple children may appear when pasting mixed content from something like
        // LibreOffice Writer,but Calc always pastes a single <table> element.
