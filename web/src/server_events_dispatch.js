...
export function dispatch_normal_event(event) {
    ...
    switch (event.type) {
        ...
        case "delete_message": {
            ...
            // to calculate the conversation key.  We iterate all deleted IDs
            // since some may not be in the local cache.
            ...
            // If we potentially emptied a DM conversation, verify with the
            // server and remove it from the sidebar if confirmed empty.
            if (private_conversation_id !== null) {
                const user_ids_string = private_conversation_id;

                // remove_messages has already run, so the cache reflects
                // the post-deletion state.  If any local messages survive,
                // the conversation is definitely not empty.
                if (message_store.has_messages_in_dm(user_ids_string)) {
                    break;
                }
                ...
                channel.get({
                    ...
                    success(data) {
                        ...
                        if (max_message_id_after === undefined) {
                            // Conversation was already removed.
                            return;
                        }
                        // A new message arrived while the request was in flight.
                        if (
