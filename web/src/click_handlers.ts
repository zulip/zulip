...
export function initialize(): void {
    ...
    function is_clickable_message_element($target: JQuery<Element>): boolean {
        ...
        if ($target.closest(".remove-preview-button").length > 0) {
            return true;
        }
    ...
    // Remove a specific link preview embed from a message.
    $("#main_div").on("click", ".remove-preview-button", function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        function do_remove_preview(): void {
            void channel.patch({
                url: "/json/messages/" + message_id + "/link_previews",
                data: {op: "remove", urls: JSON.stringify([preview_url])},
                success() {
                ...
                error(xhr) {
                    dialog_widget.hide_dialog_spinner();
                    ui_report.error(
                        $t_html({defaultMessage: "Error removing link preview"}),
                        xhr,
                        $("#dialog_error"),
                    );
                },
        ...
        confirm_dialog.launch({
            ...
            modal_content_html: $t_html({
                defaultMessage: "This will remove it for everyone, and it cannot be restored.",
            }),
            is_compact: true,
            on_click: do_remove_preview,
