...
import {user_list_style_values} from "./settings_config.ts";
...
export function initialize(): void {
    ...
    $(".buddy-list-section").on(
        ...
        function (this: HTMLElement, e) {
            ...
            const isCompactMode =
                user_settings.user_list_style === user_list_style_values.compact.code;
            const status_el = $elem?.find(".status-text")[0];
            const isTruncated = status_el && status_el.scrollWidth > status_el.clientWidth;
            const should_show_status = Boolean(isCompactMode || isTruncated);
