...
export function set_up(settings_panel: SettingsPanel): void {
    ...
    $container.find(".info-density-button").on("click", function (this: HTMLElement, e) {
        ...
        const changed_property = z
            ...
            .parse($(this).parent().attr("data-property"));
        ...
        original_values[changed_property] = user_settings[changed_property];

        const changed_settings = information_density.update_information_density_settings(
            $(this),
            changed_property,
            new_value,
        );
        ...
        const error_continuation: () => void = () => {
            information_density.update_information_density_settings(
                $(this),
                changed_property,
                z.number().parse(original_values[changed_property]),
            );
            if (changed_settings.dense_mode !== undefined) {
                user_settings.dense_mode = true;
                $("body").toggleClass("less-dense-mode");
                $("body").toggleClass("more-dense-mode");
            }
            information_density.enable_or_disable_control_buttons($container);
        };
        change_display_setting(changed_settings, $status_element, undefined, error_continuation);
    });
