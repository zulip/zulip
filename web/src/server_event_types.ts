...
// This wrapper, along with the transforms in `server_event_schema`, exist to
// work around Zod’s missing support for nested discriminatedUnion.
...
const message_details_schema = z.record(
    z.coerce.number(),
...
const wrapped_server_event_schema = z.discriminatedUnion("type", [
    ...
    event_type(
        "realm",
        z.discriminatedUnion("op", [
            z.object({op: z.literal("deactivated"), realm_id: z.number()}),
            z.object({
                op: z.literal("update"),
                property: z.string(),
            ...
            z.object({
                ...
                property: z.enum(["default", "icon", "logo", "night_logo"]),
                data: z.union([
                    ...
                    z.object({icon_url: z.string(), icon_source: z.string()}),
                    z.object({logo_url: z.string(), logo_source: z.string()}),
                    z.object({allow_message_editing: z.boolean()}),
                    z.object({message_content_edit_limit_seconds: z.optional(z.number())}),
                    z.object({night_logo_url: z.string(), night_logo_source: z.string()}),
                    z.record(
                        z.enum([
    ...
    event_type(
        "stream",
        z.discriminatedUnion("op", [
            z.object({op: z.literal("create"), streams: z.array(api_stream_schema)}),
            z.object({op: z.literal("delete"), streams: z.array(api_stream_schema)}),
            z.object({
                op: z.literal("update"),
