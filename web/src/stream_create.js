...
class StreamNameError {
    report_already_exists(error) {
        const error_message =
            error || $t({defaultMessage: "A stream with this name already exists."});
...
export function initialize() {
    $("#streams_overlay_container").on("click", "#archived_stream_rename", (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    function save_stream_info() {
        ...
        dialog_widget.submit_api_request(channel.patch, url, data, {
            success_continuation() {
                // After changing the archived stream name
