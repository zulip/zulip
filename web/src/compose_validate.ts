...
export function inform_if_topic_is_moved(orig_topic?: string, old_stream_id?: number): void {
    ...
    const sub = stream_data.get_sub_by_id(stream_id);
    const topic_name = compose_state.topic();
    if (sub && message_content !== "" && old_stream_id) {
        ...
        let narrow_url;
        let is_empty_topic;
        if (orig_topic !== "" && orig_topic !== undefined) {
            narrow_url = hash_util.by_stream_topic_url(old_stream_id, orig_topic);
            is_empty_topic = false;
        } else {
            narrow_url = hash_util.by_stream_url(old_stream_id);
            is_empty_topic = true;
        }
        const context = {
