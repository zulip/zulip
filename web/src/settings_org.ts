...
export function discard_realm_property_element_changes(elem: HTMLElement): void {
    ...
    // TODO(evy): I added this safeparse (instead of parse) because there's
    // a test for an invalid settings property, but I don't think we actually
    // expect it to happen? So maybe we should remove the test.
    const validated_property_name = realm_setting_properties_schema.safeParse(property_name);
    if (!validated_property_name.success) {
        blueslip.error("Element refers to unknown property", {property_name});
        return;
    }
    const property_value = settings_components.get_realm_settings_property_value(
        validated_property_name.data,
    );
