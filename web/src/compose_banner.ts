...
export function show_partial_send_failure(sent_count: number): void {
    const text = $t(
        {
            defaultMessage:
                "{n, plural, one {# message was} other {# messages were}} already sent before this one failed. The unsent content is back in your compose box.",
        },
        {n: sent_count},
    );
...
function split_messages_info_banner_content(): string {
    const context = {
        banner_type: INFO,
        button_text: $t({defaultMessage: "Send as a single message"}),
        // Only report a count when the content actually splits.
        message_count: compose_split_messages.will_split_into_multiple_messages()
            ? compose_split_messages.count_message_content_split_parts()
            : undefined,
