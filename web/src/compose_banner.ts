...
export function show_convert_pasted_text_to_file_banner(cb: () => void): void {
    const $banner_container = $("#compose_banners");
    if ($(`#compose_banners .${CSS.escape(CLASSNAMES.convert_pasted_text_to_file)}`).length > 0) {
        return;
    }
    const $new_row = $(
        render_compose_banner({
            banner_type: INFO,
            banner_text: $t({
                defaultMessage: "Do you want to convert this long pasted text to a file?",
            }),
            button_text: $t({defaultMessage: "Yes, convert to file"}),
