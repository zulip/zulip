...
export function can_access_topics_in_stream(stream: StreamSubscription): boolean {
    // Note that there is deliberately no organization administrator bypass
    // here: the server grants access purely on membership in
    // can_access_stream_topics_group, and the two must agree.
    return settings_data.user_has_permission_for_group_setting(
