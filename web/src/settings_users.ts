...
export function redraw_imported_users_list(): void {
    if (!should_redraw_imported_users_list || !imported_section.list_widget) {
        return;
    }
...
export function remove_user_from_imported_users_panel(user_id: number): void {
    if (!imported_section.list_widget) {
        return;
    }

    const $row = get_user_info_row(user_id);
    imported_section.list_widget.remove_rendered_row($row);
    should_redraw_active_users_list = true;
}
