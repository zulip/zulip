...
function reset_role_filter_if_stale(
    section: UserSettingsSection,
    get_role_options: () => dropdown_widget.Option[],
): void {
    if (section.filters.role_code === 0) {
        return;
    }
    const valid_role_codes = new Set(get_role_options().map((option) => option.unique_id));
    if (!valid_role_codes.has(section.filters.role_code)) {
        // 400 vs 401/402 depends on waiting_period_threshold, which can
        // change while the overlay is closed. section.filters persists
        // across overlay open/close, so a stale role_code (from before
        // the waiting period changed) must not be handed to
        // DropdownWidget as default_id — it'll throw in render().
        section.filters.role_code = 0;
    }
}
...
function create_role_filter_dropdown(
    ...
    reset_role_filter_if_stale(section, get_role_options);
...
function initialize_user_sections(
    ...
    if (imported_user_ids !== undefined) {
        // Imported stub user IDs are not passed when re-rendering
        // the table after fetching presence data, because we do
        // not show last active date in the imported users table.
...
export function update_user_data(
    ...
    if (new_data.role !== undefined) {
        const user_type = people.get_user_type_with_waiting_period(user_id);
        if (user_type) {
