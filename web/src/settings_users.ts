...
function count_users_by_role(user_ids: number[]): Record<number, number> {
    ...
    for (const user_id of user_ids) {
        ...
        const waiting_period_info = people.get_waiting_period_info(user_id);
        ...
        if (
            ...
            if (waiting_period_info.account_age < waiting_period_info.waiting_period_time) {
                role_code = settings_config.new_member_code;
            } else if (waiting_period_info.account_age > waiting_period_info.waiting_period_time) {
                role_code = settings_config.full_member_code;
            }
        }
...
function get_roles_with_counts(user_ids: number[]): dropdown_widget.Option[] {
    ...
    const dropdown_widgets = Object.values(settings_config.user_role_values).flatMap((role) => {
        if (waiting_period > 0 && role.code === settings_config.user_role_values.member.code) {
            return [
                {
                    code: settings_config.new_member_code,
                    description: $t({defaultMessage: "New member"}),
                },
                {
                    code: settings_config.full_member_code,
