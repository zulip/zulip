...
type BotSection = {
    handle_events: () => void;
    create_table: () => void;
    bot_list_widget?: ListWidget.ListWidget<number, BotInfo | undefined>;
};
...
let bot_list_widget: ListWidget.ListWidget<number, BotInfo | undefined> | undefined;
        ...
        create_table() {
            ...
            bot_list_widget = ListWidget.create($bots_table, bot_user_ids, {
                ...
                html_selector(item) {
                    ...
                    return $(`tr[data-user-id='${CSS.escape(user_id)}']`);
                },
                filter: {
                    $element: $bots_table
...
function sort_bot_role(a: BotInfo | undefined, b: BotInfo | undefined): number {
    return user_sort.compare_a_b(a?.role ?? 0, b?.role ?? 0);
}

function sort_bot_email(a: BotInfo | undefined, b: BotInfo | undefined): number {
    function email(bot: BotInfo | undefined): string {
...
function sort_bot_owner(a: BotInfo | undefined, b: BotInfo | undefined): number {
    function owner_name(bot: BotInfo | undefined): string {
...
function bot_info(bot_user_id: number): BotInfo | undefined {
...
function handle_edit_form($tbody: JQuery): void {
    $tbody.on("click", ".open-user-form", function (this, e) {
        e.stopPropagation();
        e.preventDefault();
