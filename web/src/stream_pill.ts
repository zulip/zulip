...
export async function get_user_ids(
    ...
    const results = await Promise.all(
        stream_ids.map(async (stream_id) => peer_data.get_all_subscribers(stream_id, false)),
    );
    ...
    for (const [index, stream_id] of stream_ids.entries()) {
        ...
        // (1) `null` means the request failed. We don't want to keep retrying
        // since awaiting that will delay this function's return of other
        // channel's data.
        // (2) Double check if the stream pill has been removed from the pill
        // widget while we were doing fetches.
        if (subscribers !== null && current_stream_ids_in_widget.includes(stream_id)) {
