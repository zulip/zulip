    ...
    if (filter.has_operator("date")) {
        ...
        if (first_message_in_filtered_data.timestamp >= unix_seconds_for_date_local_midnight) {
            if (superset_data.fetch_status.has_found_oldest()) {
                ...
                // Propagate the fetch status so that the
                // is_first_message_in_narrow check in
                // render_message_list_with_selected_message correctly
                // identifies this as the first message before the server
                // fetch completes.
                msg_data.fetch_status.finish_older_batch({
                    found_oldest: true,
                    history_limited: superset_data.fetch_status.history_limited(),
