        ...
        if (restore_rendered_list) {
...

            {
                ...
                switch (id_info.final_select_id) {
                    case undefined:
                        date_term = raw_terms.find((raw_term) => raw_term.operator === "date");
                        if (date_term) {
                            anchor = "date";
                            // We use local midnight for a given date operand.
                            anchor_date = new Date(date_term.operand).toISOString();
                            const d = new Date(date_term.operand);

                            const local_midnight = new Date(
                                d.getFullYear(),
                                d.getMonth(),
                                d.getDate(),
                            );

                            anchor_date = local_midnight.toISOString();
                        } else {
                            anchor = "first_unread";
                        }
                        break;
...
export function maybe_add_local_messages(opts: {
    ...
    // filtering messages. We just need to know what message we want to select.
    if (filter.has_operator("date")) {
        ...
        const message_to_select = msg_data._items.find(
            (message) => message.timestamp >= unix_seconds_for_date_local_midnight,
        );
