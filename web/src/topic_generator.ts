...
export function next_topic(
    ...
    if (curr_stream_index >= 0) {
        ...
        // Find unreads in the next channel if we didn't find any unread
        // after the current topic.
    }

    for (let i = curr_stream_index + 1; i < sorted_channels_info.length; i += 1) {
        const channel_info = sorted_channels_info[i]!;
        // Skip channels in collapsed folders in this iteration since our goal is to
        // find the next unread topic after the current channel in an uncollapsed folder.
    ...
    // Just get the first unread topic we can find.
    // Uncollapsed folders are already sorted at the top.
