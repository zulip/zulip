...
export function update_messages(events: UpdateMessageEvent[]): void {
    ...
    for (const event of events) {
        ...
        if (anchor_message !== undefined) {
            ...
            // Rendering-only events, such as link previews, aren't anyone's
            // edit, so they leave the edit UI alone entirely; handling them
            // here would consume the marker for an edit of ours that's still
            // in flight, and treating them as non-content edits would close
            // the form.
            if (!event.rendering_only) {
                const message_was_moved =
                    util.get_edit_event_topic(event) !== undefined ||
                    event.new_stream_id !== undefined;
