...
function populate_invites(invites_data: {invites: Invite[]}): void {
    ...
    ListWidget.create($invites_table, invites_data.invites, {
        ...
        modifier_html(item) {
            ...
            item.disable_resend_button = item.disable_edit_button;
...
export function on_load_success(
    ...
    $(".admin_invites_table").on("click", ".edit-invite", function (this: HTMLElement, e) {
        ...
        const filtered_invite_as_options = get_filtered_invite_as_options(invite.invited_as);
        const html_body = render_edit_invite_user_modal({
            invite_as_options: filtered_invite_as_options,
...
function get_filtered_invite_as_options(
    current_role: number,
): Partial<typeof settings_config.user_role_values> {
    const filtered_options: Partial<typeof settings_config.user_role_values> = {
        guest: settings_config.user_role_values.guest,
        member: settings_config.user_role_values.member,
    };

    if (current_user.is_admin) {
        filtered_options.moderator = settings_config.user_role_values.moderator;
        filtered_options.admin = settings_config.user_role_values.admin;
    }
    if (current_user.is_owner) {
        filtered_options.owner = settings_config.user_role_values.owner;
    }

    if (
        current_role === settings_config.user_role_values.moderator.code &&
        !current_user.is_admin
    ) {
        filtered_options.moderator = settings_config.user_role_values.moderator;
    } else if (
        current_role === settings_config.user_role_values.admin.code &&
        !current_user.is_admin
    ) {
        filtered_options.admin = settings_config.user_role_values.admin;
    } else if (
        current_role === settings_config.user_role_values.owner.code &&
        !current_user.is_owner
    ) {
        filtered_options.owner = settings_config.user_role_values.owner;
    }
