...
function validate_stream_ids_for_invitation(stream_ids: number[]): boolean {
    for (const stream_id of stream_ids) {
        ...
        const channel_name_html = render_decorated_channel_name({
        ...
        if (sub.is_archived) {
            ui_report.message(
                $t_html(
                    {defaultMessage: "Failed: <b><z-link></z-link></b> is archived."},
                    {"z-link": () => channel_name_html},
                ),
                $("#dialog_error"),
                "alert-error",
            );
        ...
        if (
            ...
            ui_report.message(
                $t_html(
                    {
                        defaultMessage:
                            "Failed: You are not allowed to subscribe others to <b><z-link></z-link></b>.",
                    },
                    {"z-link": () => channel_name_html},
                ),
                $("#dialog_error"),
