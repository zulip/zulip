...
function handle_keyboard_navigation_on_gif(e: JQuery.KeyDownEvent): void {
    ...
    const key = e.key;
    const is_alpha_numeric = /^[a-z0-9]$/i.test(key);
    if (is_alpha_numeric) {
    ...
    if (key === "Enter" || key === " " || key === "Spacebar") {
        ...
        e.preventDefault();
        handleGifClick(e.currentTarget);
        return;
    }
...
function render_gifs_to_grid(data: unknown, next_page: boolean): void {
    ...
    const parsed_data = tenor_result_schema.parse(data);
...
function render_featured_gifs(next_page: boolean): void {
    ...
    void channel.get({
        ...
        data,
        success(data) {
            render_gifs_to_grid(data, next_page);
        },
    });
}
...
function register_click_handlers(): void {
    $("body").on(
        "click",
        ".compose_control_button.compose_gif_icon_tenor",
