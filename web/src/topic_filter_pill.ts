...
export function get_single_pill_syntax(
    pills: TopicFilterPill[],
    multiple_pills_warning_message: string,
): string {
    if (pills.length === 0) {
        return "";
    }

    // For now, there is only one topic filter pill in the input.
    // If we allow multiple pills in the future, we should update
    // this function and callers to support combining syntaxes.
    if (pills.length > 1) {
        blueslip.warn(multiple_pills_warning_message);
    }
