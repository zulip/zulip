...
export function is_browser_idle_detector_supported(): boolean {
...
export async function request_browser_idle_detector_permission(): Promise<"granted" | "rejected"> {
    if (!is_browser_idle_detector_supported()) {
        return "rejected";
    }
    return window.IdleDetector.requestPermission();
}

export async function init_browser_idle_detector({
    idle_timeout,
    on_idle,
    on_active,
}: {
...
export function listen_to_idle_detector_permission_change(on_granted: () => void): void {
