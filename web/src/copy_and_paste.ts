...
export function try_stream_topic_syntax_text(text: string): string | null {
    ...
    let avoid_transform = topic_link_util.will_produce_broken_stream_topic_link(
        stream_topic.stream_name,
    );
    if (stream_topic.topic_name !== undefined) {
        avoid_transform ||= topic_link_util.will_produce_broken_stream_topic_link(
    ...
    if (clipboardData.getData) {
        ...
        if (isUrl(trimmed_paste_text)) {
            ...
            if (!compose_ui.cursor_inside_code_block($textarea) && !compose_ui.shift_pressed) {
                event.preventDefault();
                event.stopPropagation();
                // Try to transform the url text to #**stream>topic** syntax
                ...
                compose_ui.insert_and_scroll_into_view(trimmed_paste_text, $textarea);
                return;
            }
        }
