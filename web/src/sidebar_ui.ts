...
export function rewire_left_sidebar_cursor(value: typeof left_sidebar_cursor): void {
    left_sidebar_cursor = value;
}
...
function actually_update_left_sidebar_for_search(): void {
    ...
    const is_left_sidebar_search_ui_active = search_value !== "";
    left_sidebar_cursor.set_is_highlight_visible(is_left_sidebar_search_ui_active);
    ...
    if (
        ...
        is_left_sidebar_search_ui_active
    ) {
    ...
    $("#left-sidebar-empty-list-message").toggleClass(
        "hidden",
        !is_left_sidebar_search_ui_active || all_rows().length > 0,
    );
}
...
let previous_narrow_stream_id: number | undefined;
let previous_narrow_topic: string | undefined;
...
function left_sidebar_search_active(search_state: LeftSidebarSearchState): boolean {
    return search_state.search_term !== "" || search_state.topics_state !== "";
}

function left_sidebar_search_ui_active(search_state: LeftSidebarSearchState): boolean {
    return search_state.search_term !== "";
}

function save_left_sidebar_search_state(search_state: LeftSidebarSearchState): void {
    previous_search_term = search_state.search_term;
    previous_topics_state = search_state.topics_state;
    previous_narrow_stream_id = search_state.narrow_stream_id;
    previous_narrow_topic = search_state.narrow_topic;
}
...
const update_left_sidebar_for_search = _.throttle(() => {
    ...
    const was_left_sidebar_search_ui_active = previous_search_term !== "";
    const is_left_sidebar_search_ui_active = left_sidebar_search_ui_active(search_state);
    save_left_sidebar_search_state(search_state);

    const left_sidebar_scroll_container = scroll_util.get_left_sidebar_scroll_container();
    if (!is_left_sidebar_search_ui_active) {
        requestAnimationFrame(() => {
            actually_update_left_sidebar_for_search();
            if (was_left_sidebar_search_ui_active) {
                left_sidebar_navigation_area.restore_views_state();
                // Restore previous scroll position after leaving text search mode.
...
export function refresh_left_sidebar_search_for_narrow_change(): void {
    const search_state = get_left_sidebar_search_state();
    if (
        !left_sidebar_search_active(search_state) &&
    ...
    if (
        search_state.search_term === previous_search_term &&
        search_state.topics_state === previous_topics_state &&
        search_state.narrow_stream_id === previous_narrow_stream_id &&
        search_state.narrow_topic === previous_narrow_topic
    ) {
        return;
    }
...
export function set_event_handlers(): void {
    ...
    function keydown_enter_key(): void {
        ...
        save_left_sidebar_search_state(get_left_sidebar_search_state());
