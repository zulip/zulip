...
function has_active_left_sidebar_filter(search_state: LeftSidebarSearchState): boolean {
...
function apply_and_save_left_sidebar_search_state({
    search_state,
...
function apply_left_sidebar_search_ui_transition({
    ...
    const is_left_sidebar_search_active = left_sidebar_search_active(search_state);
...
const update_left_sidebar_for_search = _.throttle(() => {
    ...
    apply_and_save_left_sidebar_search_state({
        search_state,
        was_left_sidebar_search_active,
...
export function refresh_left_sidebar_search_for_narrow_change(): void {
    const search_state = get_left_sidebar_search_state();
    if (
        !has_active_left_sidebar_filter(search_state) &&
    ...
    if (
        search_state.search_term === previous_search_state.search_term &&
        search_state.topics_state === previous_search_state.topics_state &&
        search_state.narrow_stream_id === previous_search_state.narrow_stream_id &&
        search_state.narrow_topic === previous_search_state.narrow_topic
    ) {
        return;
    }
...
export function set_event_handlers(): void {
    ...
    function keydown_enter_key(): void {
        ...
        const was_left_sidebar_search_active = left_sidebar_search_active(
            pre_navigation_search_state,
        );
