...
export function load_messages(opts, attempt = 1) {
    ...
    // all messages view and the (hacky) page_params.narrow feature
    ...
    if (msg_list_data.filter.is_in_home()) {
        ...
        // Otherwise, we don't pass narrow for all messages view; this is
        // required because it shares its data with all_msg_list, and
        // so we need the server to send us message history from muted
        // streams and topics even though all messages view's in:home
...
export function initialize(finished_initial_fetch) {
    ...
    // with always contains the latest message, it makes sense for
    ...
    load_messages({
        ...
        // `num_after` is 1 to help with correctly setting the fetch status
        // without adding special conditions `all_messages_data`.
        num_after: 1,
        msg_list_data: all_messages_data,
        cont: load_more,
    });
