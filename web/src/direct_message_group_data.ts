...
export function process_loaded_messages(messages: Message[]): void {
    for (const message of messages) {
        const direct_message_group_string = people.direct_message_group_string(message);

        if (direct_message_group_string) {
            const old_timestamp = direct_message_group_timestamps.get(direct_message_group_string);

            if (!old_timestamp || old_timestamp < message.timestamp) {
                direct_message_group_timestamps.set(direct_message_group_string, message.timestamp);
            }
        }
    }
}
