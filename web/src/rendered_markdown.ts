...
export const update_elements = ($content: JQuery): void => {
    ...
    // The button is revealed on hover by CSS keyed on `.can-edit-content`
    // (see rendered_markdown.css), and the permission is re-checked in
    // click_handlers.ts. Compose/edit previews are skipped: they render
    // inside a message row, and so would otherwise get a stray button.
    const $link_previews = $content.find(
        ".message_embed, .message-media-preview-image, .message-media-preview-video",
    );
    if ($link_previews.length > 0) {
        message ??= get_message_for_message_content($content);
        if (message !== undefined && $content.closest(".preview_content").length === 0) {
            $link_previews.each(function (): void {
                const $preview = $(this);
