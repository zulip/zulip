...
export function check_profile_incomplete() {
    ...
    if (!should_display_profile_incomplete_alert(state_data.realm_date_created)) {
        return false;
    }

    // Eventually, we might also check state_data.realm_icon_source,
    // but it feels too aggressive to ask users to do change that
    // since their organization might not have a logo yet.
    if (
        state_data.realm_description === "" ||
        /^Organization imported from [A-Za-z]+[!.]$/.test(state_data.realm_description)
    ) {
        return true;
    }
    return false;
...
export function get_demo_organization_deadline_days_remaining() {
    ...
    const deadline = new Date(state_data.demo_organization_scheduled_deletion_date * 1000);
...
export function initialize() {
    ...
    if (state_data.demo_organization_scheduled_deletion_date) {
    ...
    } else if (state_data.server_needs_upgrade) {
    ...
    } else if (page_params.warn_no_email === true && state_data.is_admin) {
