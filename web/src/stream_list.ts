...
function build_stream_sidebar_li(sub: StreamSubscription, for_modal = false): JQuery {
    ...
    const args = {
        ...
        // Only this setting needs a separate control for the sidebar
        // topic list, since clicking the channel navigates instead of
        // expanding it.
...
function deselect_stream_items(): void {
    $("ul#stream_filters li").removeClass(
        "active-filter stream-expanded topic-list-expanded hide-expand-topics-button",
    );
    $("ul#stream_filters .channel-expand-topics-button").attr("aria-expanded", "false");
}
...
export function update_stream_sidebar_for_narrow(filter: Filter): JQuery | undefined {
    ...
    if (channel_links_to_list_of_topics()) {
        ...
        $stream_li.toggleClass("hide-expand-topics-button", rerender_topics_for_search);
    }
...
export function on_sidebar_channel_click(
    ...
    if (channel_links_to_list_of_topics()) {
        // Navigating is all this click does; expanding the sidebar
        // topic list is the chevron's business.
