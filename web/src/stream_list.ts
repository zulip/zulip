...
export function update_stream_sidebar_for_narrow(filter: Filter): JQuery | undefined {
    ...
    if (
        ...
        if (expanded_topic_list_stream_id !== stream_id) {
            // Narrowing away from a channel resets its expanded state.
            reset_topic_list_expanded();
        }
        if (info.topic_selected) {
            // When narrowed to a topic, the middle pane no longer
            // shows the channel's topic list, so expand it in the
            // sidebar for navigating between sibling topics.
        ...
        if (!is_expanded && !rerender_topics_for_search) {
            // With the "List of topics" channel-link setting, the
            // middle pane already shows the channel's topic list, so
            // keep the redundant sidebar topic list collapsed until
            // the user expands it via the expand chevron.
...
export function set_event_handlers({
    ...
    $("#stream_filters").on(
        ...
        function (this: HTMLElement, e) {
            ...
            // Don't leave the button focused after a mouse click, so
            // that the row's focus-within highlight doesn't linger;
            // keyboard activation keeps focus for further navigation.
            $(this).trigger("blur");
        },
    );
