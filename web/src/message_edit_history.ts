    ...
    void channel.get({
        ...
        success(raw_data) {
            ...
            const first_deleted_entry = data.message_history.find(
                (e) => e.history_deleted_by !== undefined,
            );
            for (const [index, msg] of data.message_history.entries()) {
                ...
                let history_deleted_by_mention_html: string | undefined;
                if (
                    ...
                    const deleted_by_full_name = people.get_user_by_id_assert_valid(
                        msg.history_deleted_by,
                    ).full_name;
                    history_deleted_by_mention_html = `<span class="user-mention silent" data-user-id="${msg.history_deleted_by}">${_.escape(deleted_by_full_name)}</span>`;
                } else if (index === 0 && first_deleted_entry !== undefined) {
                    const deleted_by_user_id = first_deleted_entry.history_deleted_by!;
                    const deleted_by_full_name =
                        people.get_user_by_id_assert_valid(deleted_by_user_id).full_name;
                    history_deleted_by_mention_html = `<span class="user-mention silent" data-user-id="${deleted_by_user_id}">${_.escape(deleted_by_full_name)}</span>`;
                }
