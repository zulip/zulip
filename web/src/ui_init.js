...
$(() => {
    ...
    function wait_for_reactivation_and_reload(reactivating_data) {
        // The current narrow is preserved across the reload because it lives in
        // the URL hash, which the reload retains.
        let last_event_id = reactivating_data.last_event_id;
        let failures = 0;
        // Stop waiting if the backfill never signals us (worker down or
        // backlogged): an actively polling client keeps its queue alive, so
        // BAD_EVENT_QUEUE_ID would never fire on its own to end the wait.
        const give_up_time = Date.now() + 5 * 60 * 1000;
        function poll() {
            if (Date.now() > give_up_time) {
            ...
            channel.get({
                ...
                // Bound each long-poll so a stalled connection settles and we
                // re-check give_up_time. The server holds it up to
                // EVENT_QUEUE_LONGPOLL_TIMEOUT_SECONDS (90s), so keep this larger
                // to not mistake a healthy poll for a failure.
                timeout: 120 * 1000,
                success(data) {
                    failures = 0;
                ...
                error(xhr, error_type) {
                    ...
                    if (error_type === "timeout") {
                        // A full-duration long-poll with no response is expected,
                        // not a failure; poll again, still bounded by give_up_time.
                        failures = 0;
                        poll();
                        return;
                    }
