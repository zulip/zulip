...
const update_stream_color_debounced = _.debounce(
    ...
    200, // Wait for 200ms of inactivity
    {leading: false}, // Don't execute immediately on the first color change
);
    ...
    popover_menus.register_popover_menu(".choose_stream_color", {
        ...
        onMount(instance) {
            ...
            $popper.on("click focus", ".color-swatch-option", function (this: HTMLElement) {
                ...
                const new_color = $(this).attr("data-color")!;
                if (prev_color === new_color) {
                    return;
                }
                $color_picker.val(new_color);
                $popper
