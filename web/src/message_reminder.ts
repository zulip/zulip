...
export type TimeFormattedReminder = Reminder & {
...
export function add_reminders(reminders: Reminder[]): void {
    ...
    for (const reminder of reminders) {
        ...
        const message_reminders = reminders_by_message_id.get(reminder.reminder_target_message_id)!;
...
export function rerender_reminders_for_message(message_id: number): void {
    ...
    const message_reminders = reminders_by_message_id.get(message_id) ?? [];
    if (message_reminders.length === 0) {
        $rows.find(".message-reminders").remove();
        return;
    }

    const rendered_message_reminders = render_message_reminders({
        msg: {
