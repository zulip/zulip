...
// reminders_data is a map where key=reminder id and value=reminders
export const reminders_data = new Map<number, Reminder>();
...
export function add_reminders(reminders: Reminder[]): void {
    for (const reminder of reminders) {
        reminders_data.set(reminder.scheduled_message_id, reminder);
    }
}
...
export function remove_reminder(reminder_id: number): void {
    if (reminders_data.has(reminder_id)) {
        reminders_data.delete(reminder_id);
    }
}
