...
export function create<ItemType extends {type: string}>(
    ...
    const funcs = {
        ...
        commitEditingPill($edit: JQuery, triggered_by_blur: boolean): void {
            ...
            // Remember where the pill was so the replacement lands in the same spot.
            ...
            if (success) {
                // appendValidatedData pushed the new pill to the end; move it back to
                // original_idx so store.pills stays in DOM order.
                const new_pill = store.pills.pop()!;
                store.pills.splice(original_idx, 0, new_pill);
                ...
                // Validation failed; restore the original pill at the same position.
                // quiet=false so onPillCreate fires to keep state consistent.
                funcs.appendValidatedData(pill.item, pill.disabled, false);
                const restored_pill = store.pills.pop()!;
                store.pills.splice(original_idx, 0, restored_pill);
                $edit.before(restored_pill.$element);
                $edit.remove();
                if (!triggered_by_blur) {
                    restored_pill.$element.trigger("focus");
                }
            }
        },
        ...
        cancelEditingPill($edit: JQuery): void {
            ...
            // Focus before removing $edit so the browser doesn't move focus to
            // <body> between the remove and our trigger("focus") call.
