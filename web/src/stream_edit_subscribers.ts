...
export function enable_subscriber_management({
    ...
    if (user_can_remove_subscribers) {
        ...
        remove_pill_widget = add_subscribers_pill.create({
            ...
            get_user_groups: user_groups.get_all_realm_user_groups,
            with_add_button: true,
            // Removing "everyone subscribed to another channel" has no
            // clear use case, so the remove widget omits #channel pills.
...
export type BulkUnsubscribeModalProps = {
    needs_modal: boolean;
    // True when the modal is shown because of private-channel consequences
    // (acting user can't rejoin, or the channel would lose all subscribers),
    // which drives the warning copy. False for a purely volume-based
    // confirmation, where the modal just confirms the count.
...
export function compute_bulk_unsubscribe_modal_props({
    ...
    if (invite_only) {
        ...
        if (self_without_rejoin || will_empty_channel) {
