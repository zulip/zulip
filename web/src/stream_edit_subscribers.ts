...
function update_notification_choice_checkbox(added_user_count: number): void {
    ...
    if (added_user_count > realm.max_subs_for_notification) {
...
function stream_edit_update_notification_choice(): void {
    update_notification_choice_checkbox(add_subscribers_pill.get_pill_user_ids(pill_widget).length);
}
...
export function enable_subscriber_management({
    ...
    pill_widget = add_subscribers_pill.create({
        ...
        add_btn_pill_update_callback: stream_edit_update_notification_choice,
