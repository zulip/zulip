...
function get_channel_privacy_options(sub?: StreamSubscription): dropdown_widget.Option[] {
    return Object.values(settings_config.stream_privacy_policy_values)
        .filter((privacy_type) => {
            if (privacy_type.code === settings_config.stream_privacy_policy_values.private.code) {
                const is_default_stream = sub
                    ? stream_data.is_default_stream_id(sub.stream_id)
                    : $("#id_new_is_default_stream").is(":checked");
                if (is_default_stream) {
                    return false;
                }

                return sub?.invite_only ?? settings_data.user_can_create_private_streams();
            }

            if (privacy_type.code === settings_config.stream_privacy_policy_values.public.code) {
                return (
                    (sub && !sub.invite_only && !sub.is_web_public) ??
                    settings_data.user_can_create_public_streams()
                );
            }

            return sub?.is_web_public ?? settings_data.user_can_create_web_public_streams();
        })
        .map((privacy_type) => ({
...
export function set_up_channel_privacy_dropdown_widget(sub?: StreamSubscription): DropdownWidget {
    let default_id;
    if (sub) {
        ...
        } else if (settings_data.user_can_create_private_streams()) {
    ...
    const channel_privacy_widget = new dropdown_widget.DropdownWidget({
        widget_name,
        get_options: () => get_channel_privacy_options(sub),
        $events_container,
...
export function show_settings_for(node: HTMLElement): void {
    ...
    set_up_channel_privacy_dropdown_widget(sub);
    ...
    $("#channels_overlay_container").on(
        ...
        function (e) {
            ...
            if ($subsection.attr("id") === "channel-subscription-permissions") {
                const $edit_container = stream_settings_containers.get_edit_container(sub);
                stream_ui_updates.update_can_subscribe_group_label($edit_container);
                stream_ui_updates.update_history_public_to_subscribers_state($("#stream_settings"));
            }
        },
    );
