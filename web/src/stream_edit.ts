...
export function initialize(): void {
    ...
    function save_stream_info(): void {
        ...
        dialog_widget.submit_api_request(channel.patch, url, data, {
            error_continuation(xhr: JQuery.jqXHR) {
                $("#dialog_error").hide().empty();
                $("#stream_name_error_modal").hide().empty();

                if (xhr.status === 409 && data.new_name) {
                    const existing_stream = stream_data.get_sub_by_name(data.new_name);

                    if (existing_stream) {
                        const can_view = stream_data.has_metadata_access(existing_stream);
                        ...
                        $("#stream_name_error_modal").html(rendered_error).show();
