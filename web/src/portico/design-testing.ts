...
import render_alerts_banner from "../../templates/dev_alert_banner.hbs";
...
const banner_html = (banner: Banner | AlertBanner): string => render_alerts_banner(banner);

const alert_banners: Record<string, AlertBanner> = {
    "custom-banner": {
        process: "custom-banner",
        intent: "neutral",
        label: "Use the controls below to modify this banner.",
        buttons: [
            {
                type: "quiet",
                intent: "neutral",
...
let current_banner = Object.assign({}, alert_banners["custom-banner"]);
...
$(window).on("load", () => {
    ...
    $banner_select.on("change", function (this: HTMLElement) {
        ...
        current_banner = Object.assign({}, alert_banners[banner_type]);
        update_banner();
    });
