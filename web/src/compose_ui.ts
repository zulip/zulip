...
export function render_and_show_preview(
    ...
    function build_preview_html(
        ...
        if (message_number !== "single") {
            ...
            return postprocess_content(
                render_enumerated_split_message_part({message_number, rendered_preview_html}),
            );
        }
        return postprocess_content(rendered_preview_html);
    }

    if (content.length === 0) {
        clear_thumbnail_polling();
        $preview_content_box.html(
            build_preview_html($t_html({defaultMessage: "Nothing to preview"}), "single"),
        );
        return;
    }

    const parts = $preview_container.is("#compose")
        ? compose_split_messages.get_all_split_parts(content)
        : [content];
    const should_enumerate = parts.length > 1;

    const any_needs_backend = parts.some((part) => markdown.contains_backend_only_syntax(part));
    if (any_needs_backend && show_spinner) {
        const $spinner = $preview_spinner.expectOne();
        loading.make_indicator($spinner);
    } else {
        let local_preview_html = "";
        for (const [i, part] of parts.entries()) {
            const message_number: number | "single" = should_enumerate ? i + 1 : "single";
            local_preview_html += build_preview_html(
                markdown.render(part).content,
                message_number,
                part,
            );
        }
        clear_thumbnail_polling();
        $preview_content_box.html(local_preview_html);
    ...
    void (async () => {
        ...
        for (const [i, result] of results.entries()) {
            ...
            const parsed =
                result.status === "fulfilled"
                    ? message_render_response_schema.safeParse(result.value)
                    : undefined;
            if (parsed?.success) {
                preview_html += build_preview_html(parsed.data.rendered, message_number, parts[i]);
            } else {
        ...
        $preview_content_box.html(preview_html);
        rendered_markdown.update_elements($preview_content_box);
