...
type selectionOffSelectedLines = {
    before_lines: string;
...
function get_focus_area(msg_type: messageType, opts: optsType): string {
...
export function set_focus(msg_type: messageType, opts: optsType): void {
...
export function smart_insert_inline($textarea: JQuery<HTMLTextAreaElement>, syntax: string): void {
    ...
    if (
        !(
            (after_str && after_str.length > 0 && is_space(after_str[0])) ??
...
export function replace_syntax(
    ...
    if (replacement_offset && prev_caret >= replacement_offset + old_syntax.length) {
        $textarea.caret(prev_caret + new_syntax.length - old_syntax.length);
    } else if (replacement_offset && prev_caret > replacement_offset) {
...
export function compute_placeholder_text(opts: {message_type: messageType} & optsType): string {
...
export function format_text(
    ...
    const section_off_selected_lines = (): selectionOffSelectedLines => {
