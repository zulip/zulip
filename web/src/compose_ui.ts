...
export function render_and_show_preview(
    ...
    if (content.length === 0) {
        ...
        function render_message_part(whole_content: string, message_number: number): void {
            // Changed `content` to `content_part` so it doesn't clash with the outer variable
            const [content_part, remaining_content] =
            ...
            if (markdown.contains_backend_only_syntax(content_part) && show_spinner) {
                ...
                // render using the frontend Markdown processor.
                markdown.render(content_part);
            }
            ...
            void channel.post({
                ...
                success(response_data) {
                    ...
                    if (markdown.contains_backend_only_syntax(content_part)) {
                        loading.destroy_indicator($preview_spinner);
                    }
                    show_preview(data.rendered, message_number, content_part);
