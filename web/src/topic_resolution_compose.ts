...
export function start_resolution_compose(
    ...
    // `compose_actions.start` only auto-saves the current compose box
    // content to a draft when the recipient differs (or `opts.content`
    // is set). The user can click the resolve checkmark while already
    // composing in that channel+topic, so save explicitly before we
    // hand off and clear the box.
    if (compose_state.composing() && compose_state.message_content().length > 0) {
        drafts.update_draft();
 ...
 *
 * `inert` blocks pointer events on the recipient subtree, so a tippy
 * attached directly to `#compose-recipient` would never fire. We show
 * a non-inert overlay sibling (`#topic-resolution-recipient-tooltip`,
 * defined in compose.hbs) that picks up hover and explains why the
 * area is locked.
 */
function disable_recipient_area(): void {
    $("#compose-recipient").prop("inert", true);
    $("#topic-resolution-recipient-tooltip").prop("hidden", false);
}
...
function enable_recipient_area(): void {
    ...
    $("#topic-resolution-recipient-tooltip").prop("hidden", true);
}
