...
const EmptyEmojiRenderingDetails: EmojiRenderingDetails = {
    emoji_name: "",
    emoji_code: "",
    reaction_type: "",
};

let selected_emoji_info: EmojiRenderingDetails = EmptyEmojiRenderingDetails;
let default_status_messages_and_emoji_info: StatusInfo[];

export function set_selected_emoji_info(emoji_info: EmojiRenderingDetails): void {
...
export function submit_button(): JQuery<HTMLInputElement> {
...
export function submit_new_status(): void {
    ...
    const old_emoji_info = user_status.get_status_emoji(user_id) ?? EmptyEmojiRenderingDetails;
    ...
    user_status.server_update_status({
        ...
        emoji_code: (selected_emoji_info.emoji_code ?? "").toString(),
...
function rebuild_status_emoji_selector_ui(selected_emoji_info: EmojiRenderingDetails): void {
...
function user_status_post_render(): void {
    ...
    $("#set-user-status-modal .user-status-value").on("click", (event) => {
        ...
        const statusInfo = default_status_messages_and_emoji_info.find(
            (status) => status.status_text === user_status_value,
        );

        if (statusInfo) {
            const emoji_info = statusInfo.emoji;
    ...
    $("#clear_status_message_button").on("click", () => {
        clear_message();
        set_selected_emoji_info(EmptyEmojiRenderingDetails);
        update_button();
    });
}
