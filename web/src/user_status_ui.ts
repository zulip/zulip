...
function compute_end_time(
    ...
    switch (end_time_string) {
        ...
        case "in_thirty_minutes":
            return new Date(now).setMinutes(now.getMinutes() + 30);
        case "in_one_hour":
...
function select_scheduled_end_time(custom_time_selected?: number): void {
    ...
    $("#scheduled-end-time-description").text(scheduled_timestamp);
    $("#scheduled-end-time-description").toggleClass("invisible", false);
}
    ...
    if (scheduled_end_time !== undefined) {
        $("#scheduled_end_time_widget .dropdown_widget_value").text(
            $t(
                {defaultMessage: "{N}"},
                {
                    N: timerender.get_full_datetime(new Date(scheduled_end_time), "time"),
                },
            ),
        );
        $("#scheduled-end-time-description").toggleClass("invisible", true);
    }
}
...
export function submit_new_status(): void {
    ...
    user_status.server_update_status({
        ...
        scheduled_end_time: scheduled_end_time_in_seconds ?? JSON.stringify(null),
        success() {
            dialog_widget.close();
        },
    });
