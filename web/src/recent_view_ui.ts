...
function get_unread_count(data: ConversationData): number {
    if (data.type === "private") {
        return unread.num_unread_for_user_ids_string(data.to_user_ids);
    }
    return unread.num_unread_for_topic(data.stream_id, data.topic);
}

function get_display_reply_to(conversation_data: ConversationData): string {
    assert(conversation_data.type === "private");
    const user_ids = conversation_data.to_user_ids
        .split(",")
        .map((user_id) => Number.parseInt(user_id, 10));
    return message_store.get_pm_full_names(user_ids);
}

function get_pm_url(conversation_data: ConversationData): string {
    assert(conversation_data.type === "private");
    const user_ids = conversation_data.to_user_ids
        .split(",")
        .map((user_id) => Number.parseInt(user_id, 10));
    const pm_url = people.pm_with_url_from_user_ids(user_ids);
    assert(pm_url !== undefined);
    return pm_url;
}

// This mirrors the structure of `echo.build_display_recipient`, using data
// found in `ConversationData`.
function get_display_recipient(conversation_data: ConversationData): DisplayRecipientUser[] {
    assert(conversation_data.type === "private");
    const user_ids = conversation_data.to_user_ids
        .split(",")
        .map((user_id) => Number.parseInt(user_id, 10));
    const to_users = user_ids.map((user_id) => people.get_by_user_id(user_id));

    let sender_in_display_recipients = false;
    const display_recipient: DisplayRecipientUser[] = to_users.map((user) => {
        ...
        if (user.user_id === conversation_data.sender_id) {
    ...
    if (!sender_in_display_recipients) {
        ...
        const sender = people.get_by_user_id(conversation_data.sender_id);
...
function format_conversation(conversation_data: ConversationData): ConversationContext {
    ...
    const unread_count = get_unread_count(conversation_data);
    ...
    if (type === "stream") {
        ...
        const display_recipient = get_display_recipient(conversation_data);
        const rendered_pm_with = display_recipient
            .filter(
                (recipient: DisplayRecipientUser) =>
                    !people.is_my_user_id(recipient.id) || display_recipient.length === 1,
            )
        ...
        const pm_url = get_pm_url(conversation_data);
        const is_group = display_recipient.length > 2;
...
export function filters_should_hide_row(topic_data: ConversationData): boolean {
    ...
    if (filters.has("unread")) {
        const unread_count = get_unread_count(topic_data);
        if (unread_count === 0) {
            return true;
        }
    }
    ...
    if (topic_data.type === "stream") {
        ...
        const participants = [...get_display_recipient(topic_data)].map((recipient) =>
...
function stream_sort(a: Row, b: Row): number {
    if (a.type === b.type) {
        ...
        return sort_comparator(get_display_reply_to(a), get_display_reply_to(b));
    }
...
function topic_sort_key(conversation_data: ConversationData): string {
    if (conversation_data.type === "private") {
        return get_display_reply_to(conversation_data);
    }
...
function unread_sort(a: ConversationData, b: ConversationData): number {
    const a_unread_count = get_unread_count(a);
    const b_unread_count = get_unread_count(b);
...
function has_unread(row: number): boolean {
    ...
    assert(current_row !== undefined);
    return get_unread_count(current_row) > 0;
}
