...
import {check_dm_permissions_and_get_error_string} from "./compose_validate";
...
export function pick_empty_narrow_banner(): NarrowBannerData {
    ...
    switch (first_operator) {
        ...
        case "dm": {
            ...
            const user_ids_string = sorted_ids(user_ids).join(",");
            const direct_message_error_string =
                check_dm_permissions_and_get_error_string(user_ids_string);
            if (direct_message_error_string) {
                return {title: direct_message_error_string};
            }
            if (!first_operand.includes(",")) {
        ...
        case "dm-including": {
            ...
            const direct_message_error_string =
                check_dm_permissions_and_get_error_string(person_id_string);
            if (direct_message_error_string) {
                return {title: direct_message_error_string};
            }
            if (people.is_current_user(first_operand)) {
