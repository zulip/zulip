...
function undo_draft_deletion(): void {
    ...
    // Only the last add notifies, so we rerender once rather than N times.
    const last_index = draft_undo_delete_list.length - 1;
    for (const [index, draft] of draft_undo_delete_list.entries()) {
...
function focus_first_row_of_current_tab(): void {
    const first_draft_id = keyboard_handling_context.get_items_ids().at(0);
    if (first_draft_id !== undefined) {
        messages_overlay_ui.try_set_initial_element(first_draft_id, keyboard_handling_context);
    }
}
...
function update_bulk_outbox_ui(): void {
...
const OVERLAY_LIST_SELECTORS = [".drafts-list", ".outbox-list"];

function get_checked_draft_ids(): Set<string> {
    const checked_draft_ids = new Set<string>();
    for (const row of document.querySelectorAll<HTMLElement>(
        "#drafts_table .overlay-message-row",
    )) {
        const draft_id = row.dataset["draftId"];
        const $checkbox = $(row).find(".draft-selection-checkbox, .outbox-selection-checkbox");
        if (draft_id !== undefined && is_checkbox_icon_checked($checkbox)) {
            checked_draft_ids.add(draft_id);
        }
    }
    return checked_draft_ids;
}

function restore_checked_draft_ids(checked_draft_ids: Set<string>): void {
    ...
    for (const row of document.querySelectorAll<HTMLElement>(
        ...
        if (draft_id !== undefined && checked_draft_ids.has(draft_id)) {
            const $checkbox = $(row).find(".draft-selection-checkbox, .outbox-selection-checkbox");
            toggle_checkbox_icon_state($checkbox, true);
        }
    }
}

function rerender_drafts(): void {
    // A rerender replaces the whole list DOM, so capture the transient state
    // it would otherwise discard -- focus, checkbox selection, scroll -- and
    // restore it below.
    const checked_draft_ids = get_checked_draft_ids();
    const focused_draft_id = messages_overlay_ui.get_focused_element_id(keyboard_handling_context);
    const scroll_tops = new Map<string, number>();
    for (const selector of OVERLAY_LIST_SELECTORS) {
        const $list = $(selector);
        if ($list.length > 0) {
    ...
    restore_checked_draft_ids(checked_draft_ids);
