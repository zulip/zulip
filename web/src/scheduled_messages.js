...
// scheduled_messages_data is a dictionary where key=timestamp and value=scheduled_messages
...
function sort_scheduled_messages_data() {
    ...
    scheduled_messages_data = Object.fromEntries(messagesArray);
}
...
export function add_scheduled_messages(scheduled_messages) {
    for (const scheduled_message of scheduled_messages) {
        const timestamp = scheduled_message.scheduled_delivery_timestamp;
        scheduled_messages_data[timestamp] = scheduled_message;
    }
...
export function remove_scheduled_message(scheduled_message_id) {
    const [timestamp, messageObject] = Object.entries(scheduled_messages_data).find(
        ([, message]) => message.scheduled_message_id === scheduled_message_id,
    );

    if (timestamp && messageObject) {
        delete scheduled_messages_data[timestamp];
...
export function update_scheduled_message(scheduled_message) {
    const timestamp = Object.keys(scheduled_messages_data).find(
        (timestamp) =>
            scheduled_messages_data[timestamp].scheduled_message_id ===
...
export function edit_scheduled_message(scheduled_message_id, should_narrow_to_recipient = true) {
    const [timestamp, scheduled_msg] = Object.entries(scheduled_messages_data).find(
        ([, msg]) => msg.scheduled_message_id === scheduled_message_id,
    );

    if (!timestamp || !scheduled_msg) {
        return;
    }
...
export function initialize(scheduled_messages_params) {
    for (const scheduled_message of scheduled_messages_params.scheduled_messages) {
        const timestamp = scheduled_message.scheduled_delivery_timestamp;
        scheduled_messages_data[timestamp] = scheduled_message;
    }
