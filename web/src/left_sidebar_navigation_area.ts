...
export function get_built_in_popover_condensed_views(): navigation_views.BuiltInViewMetadata[] {
    let condensed_views = navigation_views.get_built_in_views().slice(5);

    const scheduled_message_count = scheduled_messages.get_count();
    if (scheduled_message_count < 1) {
        condensed_views = condensed_views.filter(
            (view) =>
                view.fragment !==
                navigation_views.built_in_views_meta_data.scheduled_messages.fragment,
        );
    }
