...
export function get_built_in_pinned_views(
    ...
    return navigation_views
        ...
        .filter(
            (view) =>
                score(view) > 0 || (active_fragment ? view.fragment === active_fragment : false),
        );
}

export function get_built_in_unpinned_views(): navigation_views.BuiltInViewMetadata[] {
    return navigation_views.get_built_in_views().filter((view) => score(view) === 0);
}
...
export function update_sidebar_for_navigation_views(): void {
    const primary_condensed_views = get_built_in_pinned_views().slice(0, 5);
