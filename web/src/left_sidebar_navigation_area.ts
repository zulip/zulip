...
export function get_home_view_element(home_view: string): JQuery {
    if (home_view === settings_config.web_home_view_values.all_messages.code) {
        return $(`.${settings_config.web_home_view_values.all_messages.class}`);
    }

    if (home_view === settings_config.web_home_view_values.recent_topics.code) {
        return $(`.${settings_config.web_home_view_values.recent_topics.class}`);
    }

    // default home view
    return $(`.${settings_config.web_home_view_values.inbox.class}`);
}

// Reorder the <li> elements in the DOM based on the current home_view.
// Called in 2 places: on initial page load and when home_view changes.
export function reorder_left_sidebar_navigation_list(home_view: string): void {
    ...
    const entries = Object.entries(settings_config.web_home_view_values).sort(
        ([, a], [, b]) => a.order - b.order,
    );
    // Re-insert them in DOM
    for (const entry of entries) {
        const $view = $(`.${entry[1].class}`);
