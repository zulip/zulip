...
export let get_suggestions = function (
    ...
    function get_people_suggestions(
        ...
        all_person_suggestions.sort((a, b) => {
            const term_a = Filter.convert_suggestion_to_term(util.the(Filter.parse(a)));
            const term_b = Filter.convert_suggestion_to_term(util.the(Filter.parse(b)));

            // Should normally be valid, but avoid crashing on edge suggestions.
            if (term_a === undefined || term_b === undefined) {
                return 0;
            }
