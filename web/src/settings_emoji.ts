...
export function add_custom_emoji_post_render(): void {
    ...
    function image_is_not_transparent(): boolean {
        const input = get_file_input()[0];
        if (!input?.files?.length) {
            return false;
        }
        ...
        return !/\.(png|webp|avif|gif)$/.test(filename);
    }
    ...
    $clear_button.on("click", (e) => {
        ...
        $preview_image.removeClass("pad crop not-transparent").hide();
    ...
    $resize_by_crop_button.on("click", (e) => {
        ...
        $preview_image
            .removeClass("crop")
            .addClass("pad")
            .toggleClass("not-transparent", image_is_not_transparent());
    });
