...
// user emails.
function handle_operators_supporting_id_based_api(narrow_string: string): string {
    ...
    const parsed_narrow_data = z.array(narrow_term_schema).parse(JSON.parse(narrow_string));
    ...
    for (const raw_term of parsed_narrow_data) {
        ...
        if (operators_supporting_id.has(canonical_operator)) {
            ...
            if (canonical_operator === "channel") {
                // An unknown channel may have an empty string set for
                ...
                if (stream !== undefined) {
                    narrow_term.operand = Number.parseInt(raw_term.operand, 10);
                }
                narrow_terms.push(narrow_term);
                continue;
            }
...
export function load_messages(opts: MessageFetchOptions, attempt = 1): void {
    ...
    if (!msg_list_data.filter.is_in_home()) {
        const message_list_terms = msg_list_data.filter.public_terms();
        narrow_data = [...narrow_data, ...message_list_terms];
    }
