...
function get_footer_html_for_topic_typeahead(stream_id: number | undefined, dm_only: boolean): string {
    ...
    return render_topic_typeahead_hint({can_create_new_topics_in_stream, dm_only});
}
...
export function initialize({
    ...
    stream_message_topic_typeahead = new Typeahead(stream_message_typeahead_input, {
        ...
        footer_html(matching_items: (UserPillData | string)[]): string {
            const has_dm_only = !matching_items.some(item => typeof item === 'string');
            return get_footer_html_for_topic_typeahead(compose_state.stream_id(), has_dm_only);
        },
    });
