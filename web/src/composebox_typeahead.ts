...
export function get_pm_people(query: string, opts: {include_groups: false}): UserPillData[];
export function get_pm_people(
    query: string,
    opts?: {include_groups?: true},
): (UserGroupPillData | UserPillData)[];
export function get_pm_people(
    query: string,
    {include_groups = true}: {include_groups?: boolean} = {},
): (UserGroupPillData | UserPillData)[] {
    const opts = {
        ...
        new Typeahead(
            {$element: $edit, type: "contenteditable"},
            {
                ...
                source(query: string): UserPillData[] {
                    ...
                    return get_pm_people(query, {include_groups: false});
                },
                updater(item: UserGroupPillData | UserPillData): undefined {
                    if (item.type === "user") {
