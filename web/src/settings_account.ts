...
export function set_up(): void {
    ...
    $("#profile-settings").on("focus", ".custom_user_field_value", function () {
        const value: unknown = $(this).val();
    ...
    $("#profile-settings").on("change", ".custom_user_field_value", function (this: HTMLElement) {
        ...
        if (value.trim() !== "" || !isDateField) {
            $(this).data("lastValidValue", value);
            fields.push({id: field_id, value});
            update_user_custom_profile_fields(fields, channel.patch);
        } else {
            const lastValidValue: unknown = $(this).data("lastValidValue");
            if (typeof lastValidValue === "string") {
                $(this).val(lastValidValue); // revert to previous value
            }
        }
    });
