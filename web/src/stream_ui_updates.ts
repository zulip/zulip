...
export function update_history_public_to_subscribers_state(
    ...
    if (update_checked_state) {
        // Input needs to be checked if channel privacy is changed from private to public
        // or web-public.
        // Input needs to be unchecked if channel privacy is changed from public or web-public
        // to private because by default private streams have private history.
        $history_public_to_subscribers_container.find("input").prop("checked", !is_invite_only);
    }
}
...
export function handle_channel_privacy_update($container: JQuery): void {
    ...
    update_history_public_to_subscribers_state($container, true);

    if (!is_stream_creation) {
        // Default stream input is shown in a separate tab,
        // so it is updated once the changes are received
        // on receiving the update event from server.
        // And announce option is only available in the stream
        // creation form.
        return;
    }
