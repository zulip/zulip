...
function get_first_and_last_selected_elements(): {
    ...
    const first_element_html = fragment_children[0]?.querySelector(".message_content")?.innerHTML;
...
export function copy_handler(ev: ClipboardEvent): boolean {
    ...
    const first = $(
        `#message_feed_container .message-list.focused-message-list [data-message-id='${start_id}`,
    ).find(".messagebox-content")[0];
    const last = $(
        `#message_feed_container .message-list.focused-message-list [data-message-id='${end_id}`,
    ).find(".messagebox-content")[0];
    const range_count = document.getSelection()?.rangeCount;
