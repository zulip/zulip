...
function get_selected_message_content_html_at(
    ...
    if (selected_message_content_elements.length === 1 && idx < 0) {
        return "";
    }
...
function construct_copy_div($div: JQuery, start_id: number, end_id: number): void {
    ...
    if (range_count && range_count > 1) {
        ...
        if (selected_message_content_elements.length === copy_rows.length - 1) {
            // This is one of those cases, where analyze_selection sets the end_id as the
            // message with just the highlighted user name at the top as the last selected
            // message, even though there is no meaningful `.message_content` selected for
            // that message.
            // So the actually  last message of this selection is
