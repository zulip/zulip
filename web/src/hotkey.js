...
// A set of characters that, on many keyboard layouts, require modifier keys
// to type, or where pressing the key with a modifier is expected to produce
// a different character. We ignore all modifiers when processing these keys,
// and disallow assigning hotkeys that involve any modifier combined with them.
const KNOWN_IGNORE_MODIFIERS_KEYS = new Set(["*", "+", "=", "-", "/", ":", "<", ">", "?", "@"]);
...
export function get_keydown_hotkey(e) {
    ...
    if (!KNOWN_IGNORE_MODIFIERS_KEYS.has(key)) {
        if (e.shiftKey) {
            key = "Shift+" + key;
        }
        if (e.altKey) {
