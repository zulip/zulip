...
type BotInfo = {
    ...
    show_actions_for_this_row: boolean;
...
function bot_info(bot_user_id: number): BotInfo {
    ...
    const show_actions_for_this_row =
        (is_bot_owner && bot_user.bot_type === GENERIC_BOT_TYPE) ||
...
function get_show_actions_column_for_current_list(
    section: BotSettingsSection,
    fallback_ids: number[],
): boolean {
    const current_list = section.list_widget?.get_current_list();
    const filtered_ids = current_list ? current_list.map((bot) => bot.user_id) : fallback_ids;
    return should_show_actions_column(filtered_ids);
}
...
function create_all_bots_table(): void {
    ...
    all_bots_section.list_widget = ListWidget.create($all_bots_table, bot_user_ids, {
        ...
        get_item: bot_info,
        modifier_html(item) {
            const show_actions_column = get_show_actions_column_for_current_list(
                all_bots_section,
                bot_user_ids,
            );
            return render_settings_bot_list_row({...item, show_actions_column});
        },
