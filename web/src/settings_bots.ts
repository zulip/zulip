...
import {show_bot_trigger_options} from "./user_profile.ts";
...
export function add_a_new_bot(): void {
    ...
    function create_a_new_bot(): void {
        ...
        const selected_bot_triggers = $("#bot_triggers_checkbox_container .setting-widget:checked")
            .map(function () {
                return this.id;
            })
            .get();
        ...
        switch (bot_type) {
            case OUTGOING_WEBHOOK_BOT_TYPE: {
                ...
                formData.append("service_triggers", JSON.stringify(selected_bot_triggers));
                break;
            }
    ...
    function set_up_form_fields(): void {
        ...
        const $bot_triggers_field = $("#bot_triggers_field");
        $bot_triggers_field.hide();
        ...
        $("#create_bot_type").on("change", () => {
            ...
            switch (bot_type) {
                case OUTGOING_WEBHOOK_BOT_TYPE: {
                    ...
                    $bot_triggers_field.show();
                    show_bot_trigger_options($bot_triggers_container);
                    break;
                }
                case EMBEDDED_BOT_TYPE: {
