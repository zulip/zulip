...
export function suggest_delete_detached_attachments(attachments_list: ServerAttachment[]): void {
    ...
    // retry if it fails. A list is way too slow for the current use
    // case. By Build a map from the list of attachments we want to
    // delete, we can perform searches and deletion at O(1) time.
