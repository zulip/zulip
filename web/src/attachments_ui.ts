...
export function percentage_used_space(uploads_size: number): string | null {
    if (state_data.realm_upload_quota_mib === null) {
        return null;
    }
    return ((100 * uploads_size) / state_data.realm_upload_quota_mib).toFixed(1);
}
...
function set_upload_space_stats(): void {
    ...
    const args = {
        show_upgrade_message: state_data.realm_plan_type === 2,
        percent_used: percentage_used_space(upload_space_used),
        upload_quota: bytes_to_size(state_data.realm_upload_quota_mib, true),
    };
