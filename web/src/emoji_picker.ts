...
const generate_emoji_picker_content = function (
    id: number | null,
    add_frequently_used_section: boolean,
): string {
    ...
    if (!add_frequently_used_section) {
...
function get_default_emoji_popover_options(
    ...
    return {
        ...
        onCreate(instance: tippy.Instance) {
            ...
            instance.setContent(
                ui_util.parse_html(
                    generate_emoji_picker_content(current_message_id, add_frequently_used_section),
                ),
            );
            emoji_catalog_last_coordinates = {
                section: 0,
...
export function toggle_emoji_popover(
    ...
    add_frequently_used_section = true,
): void {
    if (id) {
    ...
    popover_menus.toggle_popover_menu(
        target,
        {
            ...get_default_emoji_popover_options(add_frequently_used_section),
...
function register_click_handlers(): void {
    ...
    $("body").on(
        ...
        function (this: HTMLElement, e): void {
            e.preventDefault();
            e.stopPropagation();
            toggle_emoji_popover(this, undefined, {placement: "bottom"}, false);
            if (is_open()) {
