...
export function rebuild_catalog(): void {
    ...
    for (const {emoji_code, emoji_name} of typeahead.frequently_used_emojis) {
        let emoji_dict;
        const name = emoji.get_emoji_name(emoji_code);
        if (name !== undefined) {
            emoji_dict = emoji.emojis_by_name.get(name);
        } else {
            // For custom emojis
            emoji_dict = emoji.emojis_by_name.get(emoji_name);
        }
...
function register_click_handlers(): void {
    ...
    $("body").on(
        ...
        function (this: HTMLElement, e): void {
            ...
            toggle_emoji_popover(this, undefined, {placement: "bottom", appendTo: micromodal}, false);
            if (is_open()) {
