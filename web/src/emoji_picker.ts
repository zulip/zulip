...
export function hide_emoji_popover(): void {
    ...
    if (user_status_ui.user_status_picker_open()) {
        ...
        $(".app, .header, .modal__overlay, #set-user-status-modal").removeClass("no-pointer-events");
    }
    ...
    $("body").on(
        ...
        function (this: HTMLElement, e): void {
            ...
            if (is_open()) {
                ...
                $(".app, .header, .modal__overlay, #set-user-status-modal").addClass("no-pointer-events");
            }
        },
    );
