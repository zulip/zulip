...
export function toggle_user_card_popover_from_hotkey() {
...
export function initialize() {
    ...
    function placement_on_mobile(instance) {
        if ($("body").outerWidth() < media_breakpoints_num.md) {
            const $overlay = $("<div>").addClass("user-card-overlay").css({
                position: "fixed",
                top: "0",
                left: "0",
                width: "100%",
    ...
    const user_card_options = {
        ...
        onShow(instance) {
            ...
            on_show_prep(instance);
            $(instance.popper).addClass("user_info_card_tippy " + template_class + "_tippy");
            ...
            // The preventOverflow option ensures that the user card remains at least 10px from the top of the viewport,
            // preventing it from going above that limit. It also prevents the user card from overlapping with the compose box.
            // The eventlistener option ensures that the user card does not disappear when there are zoom or resize events.
            instance.setProps({
                popperOptions: {
                    strategy: "fixed",
                    modifiers: [
