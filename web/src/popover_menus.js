/* Module for popovers that have been ported to the modern
   ...
   import render_compose_video_link_popover from "../templates/compose_video_link_popover.hbs";
   ...
   export function initialize() {
       ...
       delegate("body", {
           ...
           onShow(instance) {
               ...
               if (page_params.realm_video_chat_provider.length === 1) {
                   compose.video_call_handler(page_params.realm_video_chat_provider[0]);
                   // Returning false from an onShow handler cancels the show.
                   return false;
               }
               ...
               const should_display_jitsi_meet =
                   available_providers.jitsi_meet &&
                   page_params.realm_video_chat_provider.includes(available_providers.jitsi_meet.id);
   
               const should_display_zoom =
                   available_providers.zoom &&
                   page_params.realm_video_chat_provider.includes(available_providers.zoom.id);
   
               const should_display_bigbluebutton =
                   available_providers.big_blue_button &&
                   page_params.realm_video_chat_provider.includes(
                       available_providers.big_blue_button.id,
                   );
   
               instance.setContent(
                   parse_html(
                       render_compose_video_link_popover({
                           available_providers,
                           should_display_jitsi_meet,
                           should_display_zoom,
                           should_display_bigbluebutton,
                       }),
                   ),
               );
               popover_instances.compose_video_link_popover_displayed = instance;
   
               const $popper = $(instance.popper);
               $popper.one("click", ".video_chat_provider_jitsi_meet", (e) => {
                   compose.video_call_handler(
                       Number.parseInt($(e.target).attr("data-video-chat-id"), 10),
                   );
                   instance.hide();
               });
               $popper.one("click", ".video_chat_provider_zoom", (e) => {
                   compose.video_call_handler(
                       Number.parseInt($(e.target).attr("data-video-chat-id"), 10),
                   );
                   instance.hide();
               });
               $popper.one("click", ".video_chat_provider_bigbluebutton", (e) => {
                   compose.video_call_handler(
                       Number.parseInt($(e.target).attr("data-video-chat-id"), 10),
                   );
                   instance.hide();
           ...
           onHidden(instance) {
               instance.destroy();
               popover_instances.compose_video_link_popover_displayed = undefined;
           },
       });
