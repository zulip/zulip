/* Module for popovers that have been ported to the modern
   ...
   export function toggle_message_actions_menu(message) {
     ...
     const $popover_reference = $(
       ".selected_message .actions_hover .zulip-icon-ellipsis-v-solid"
     );
   ...
   export function initialize() {
     tippy_no_propagation("#streams_inline_icon", {
       onShow(instance) {
         ...
         instance.setContent(
           parse_html(render_left_sidebar_stream_setting_popover())
         );
         left_sidebar_stream_setting_popover_displayed = true;
   
         const addStreamsTooltip = [...$("#add_streams_tooltip")][0]?._tippy;
         const filterStreamsTooltip = [...$("#filter_streams_tooltip")][0]?._tippy;
   
         addStreamsTooltip?.setProps({ placement: "bottom" });
         filterStreamsTooltip?.setProps({ placement: "bottom" });
       ...
       onHidden(instance) {
         ...
         addStreamsTooltip?.setProps({ placement: "top" });
         filterStreamsTooltip?.setProps({ placement: "top" });
       },
     });
     ...
     tippy_no_propagation(".actions_hover .zulip-icon-ellipsis-v-solid", {
       ...
       onMount(instance) {
         ...
         $popper.one(
           "click",
           ".popover_edit_message, .popover_view_source",
           (e) => {
         ...
         $popper.one("click", ".rehide_muted_user_message", (e) => {
           ...
           const message_container =
             message_lists.current.view.message_containers.get(message.id);
