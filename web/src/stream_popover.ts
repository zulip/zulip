...
export async function build_move_topic_to_stream_popover(
    ...
    function warn_unsubscribed_participants(destination_stream_id: number): void {
        ...
        const conversation_messages = message_util.get_messages_in_topic(
            current_stream_id,
            topic_name,
        );

        const unsubscribed_participant_ids_set = new Set<number>();

        for (const message of conversation_messages) {
            if (
                !stream_data.is_user_subscribed(destination_stream_id, message.sender_id) &&
                !people.get_by_user_id(message.sender_id).is_bot
            ) {
                unsubscribed_participant_ids_set.add(message.sender_id);
            }
        }
        const unsubscribed_participant_ids = [...unsubscribed_participant_ids_set];
        const n_unsubscribed_participants = unsubscribed_participant_ids.length;

        if (n_unsubscribed_participants === 0) {
        ...
        const unsubscribed_participantn_names_list = util.format_array_as_list_with_conjuction(
            unsubscribed_participant_ids.map(
                (user_id) =>
                    `<strong>${people.get_user_by_id_assert_valid(user_id).full_name}</strong>`,
            ),
            "long",
        );
        ...
        const few_unsubscribed_participants = n_unsubscribed_participants <= 5;
        ...
        const context = {
            ...
            unsubscribed_participantn_names_list,
            n_unsubscribed_participants,
            few_unsubscribed_participants,
        };
