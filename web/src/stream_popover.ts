...
export async function build_move_topic_to_stream_popover(
    ...
    function move_topic_on_update(event: JQuery.ClickEvent, dropdown: {hide: () => void}): void {
        ...
        if (stream_data.can_only_use_empty_topic(selected_stream.stream_id)) {
            $topic_input.val("");
            $topic_input.prop("disabled", true);
            $topic_input.addClass("topics-disabled");
        } else {
            $topic_input.removeClass("topics-disabled");
        }
        ...
        if ($topic_input.prop("disabled")) {
            // This trigger is required to update the placeholder to "general chat".
    ...
    function move_topic_post_render(): void {
        ...
        const streams_list_options = (): dropdown_widget.Option[] =>
            stream_data.get_streams_for_move_messages_widget().filter(({stream}) => {
                ...
                //  1) messages from named topics to channels where topics are disabled.
                //  2) messages from empty topics to channels where empty topics are disabled.
        ...
        if (stream_data.can_only_use_empty_topic(current_stream_id)) {
            ...
            $topic_input.prop("disabled", true);
            $topic_input.addClass("topics-disabled");
            // This hack is needed to trigger the blur event.
            $topic_input.trigger("focus");
            $topic_input.trigger("blur");
        }
    }
