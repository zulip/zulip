...
export async function build_move_topic_to_stream_popover(
    ...
    function get_messages_to_be_moved(
        propagate_mode: string | undefined,
    ...
    async function warn_unsubscribed_participants(
        selected_propagate_mode: string | undefined,
    ): Promise<void> {
    ...
    function move_topic_on_update(event: JQuery.ClickEvent, dropdown: {hide: () => void}): void {
        ...
        const selected_propagate_mode = $("#message_move_select_options").val();
        void warn_unsubscribed_participants(
            selected_propagate_mode ? String(selected_propagate_mode) : undefined,
        );
