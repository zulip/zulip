    ...
    function edit_bot_post_render(): void {
        ...
        function update_service_data_ui(bot_type: number): void {
            if (bot_type === OUTGOING_WEBHOOK_BOT_TYPE) {
                ...
                const show_embedded_bot_config = (service_name: string): void => {
                    $("#config_edit_inputbox").children().hide();
                    $(`#config_edit_inputbox [name='${CSS.escape(service_name)}']`).show();
                    if (embedded_service?.config_data) {
                        for (const [key, value] of Object.entries(embedded_service.config_data)) {
                            $(
                                `#config_edit_inputbox [name='${CSS.escape(service_name)}'] input[name='${CSS.escape(key)}']`,
                            ).val(value);
                        }
                    }
                };

                show_embedded_bot_config(
                    $<HTMLSelectOneElement>("select:not([multiple])#select_service_name").val()!,
                );
