...
function create_initial_participant_tiles(room: Room): void {
    // ParticipantConnected only fires for participants who join after us,
    // so ensure a camera tile exists for every participant already in the
    // room. TrackSubscribed (registered before connect) attaches their media.
...
export async function start_in_call(
    ...
    create_initial_participant_tiles(room);
}
