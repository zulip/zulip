...
export function build_realm_logo_widget(upload_function: UploadFunction, is_night: boolean): void {
    ...
    let logo_source = state_data.realm_logo_source;

    if (is_night) {
        logo_section_id = "#realm-night-logo-upload-widget";
        logo_source = state_data.realm_night_logo_source;
    }
    ...
    upload_widget.build_direct_upload_widget(
        ...
        upload_function,
        state_data.max_logo_file_size_mib,
    );
}
...
export function render(): void {
    ...
    $("#realm-day-logo-upload-widget .image-block").attr("src", state_data.realm_logo_url);

    if (state_data.realm_night_logo_source === "D" && state_data.realm_logo_source !== "D") {
        ...
        $("#realm-night-logo-upload-widget .image-block").attr("src", state_data.realm_logo_url);
    } else {
        $("#realm-night-logo-upload-widget .image-block").attr(
            "src",
            state_data.realm_night_logo_url,
        );
    }

    if (settings_data.using_dark_theme() && state_data.realm_night_logo_source !== "D") {
        $("#realm-logo").attr("src", state_data.realm_night_logo_url);
    } else {
        $("#realm-logo").attr("src", state_data.realm_logo_url);
    }

    change_logo_delete_button(
        state_data.realm_logo_source,
        $("#realm-day-logo-upload-widget .image-delete-button"),
        $file_input,
    );
    change_logo_delete_button(
        state_data.realm_night_logo_source,
