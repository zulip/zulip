...
export function get_custom_stream_specific_notifications_table_row_data(): NotificationSettingCheckbox[] {
    return stream_specific_notification_settings.map((setting_name) => {
        ...
        if (setting_name === "push_notifications") {
            // Prevents the mobile push notifications disabled tooltip from showing
            // unnecessarily when the checkbox is initially disabled due to no stream
            // being selected, even though push notifications are enabled.
            checkbox.is_mobile_checkbox = !realm.realm_push_notifications_enabled;
        }
        return checkbox;
    });
}
