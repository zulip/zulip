...
const UPGRADE_TO_ZULIP_CLOUD_BANNER: Banner = {
    intent: "neutral",
    label: $t({defaultMessage: "Available on Zulip Cloud Standard."}),
    buttons: [],
    close_button: false,
...
export function set_up_upgrade_banners(): void {
    ...
    if (is_business_type_org) {
        banner = has_billing_access ? UPGRADE_ACCESS_BANNER : UPGRADE_TO_ZULIP_CLOUD_BANNER;
    } else {
        banner = has_billing_access ? UPGRADE_OR_SPONSORSHIP_BANNER : UPGRADE_TO_ZULIP_CLOUD_BANNER;
    }
