...
export const update_person = function update(event: UserUpdate): void {
    ...
    if ("role" in event) {
        ...
        settings_account.add_or_remove_owner_from_role_select();
        settings_account.update_user_own_role_select_state();
        ...
        if (people.is_my_user_id(event.user_id) && was_owner !== user.is_owner) {
            settings_account.add_or_remove_owner_from_role_select();
            settings_account.update_user_own_role_select_state();
        }
