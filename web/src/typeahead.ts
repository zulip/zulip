...
export function triage_raw<T>(
    ...
    /*
        We split objs into six groups:
            ...
            - prefix match case-insensitively (possibly via normalized fallback)
            - word-boundary prefix match case-insensitively (possibly via
              normalized fallback)
            - other
    ...
    */
    ...
    const normalized_word_boundary_regex = new RegExp(
        `[${word_boundary_chars}]${_.escapeRegExp(normalized_query)}`,
    );

    for (const obj of objs) {
        const items = get_items(obj);
        const lower_items = items.map((item) => item.toLowerCase());
        const normalized_items = lower_items.map((item) => remove_diacritics(item));
        const all_items_are_ascii = lower_items.every((item, i) => item === normalized_items[i]);
        // When the query is plain ASCII, skip normalized matching for items
        // that have diacritics, so "a" doesn't match "Ądam".
        const allow_normalized_fallback = query_has_diacritics || all_items_are_ascii;
        ...
        if (lower_items.includes(lower_query)) {
            ...
            normalized_items.some((item) => item.startsWith(normalized_query))
        ) {
            begins_with_case_insensitive_matches.push(obj);
            ...
            allow_normalized_fallback &&
            normalized_items.some((item) => normalized_word_boundary_regex.test(item))
        ) {
            word_boundary_matches.push(obj);
        } else {
            no_matches.push(obj);
