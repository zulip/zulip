...
function make_list_widget({
    ...
    return ListWidget.create($list_container, users, {
        ...
        $parent_container,
        sort_fields: {
            email: sort_group_member_email,
            name: sort_group_member_name,
        },
        modifier_html(item) {
            if ("user_id" in item) {
        ...
        filter: {
            ...
            predicate(person, value) {
                const matcher = build_group_member_matcher(value);
...
function remove_subgroup({
    ...
    function do_remove_subgroup_from_group(): void {
        edit_user_group_membership({
            group,
