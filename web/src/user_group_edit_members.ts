...
function generate_group_link(group: UserGroup): string {
    if (group.is_system_group) {
        let group_name = group.name.split(":")[1];
        if (group_name) {
            group_name = group_name?.charAt(0).toUpperCase() + group_name?.slice(1);
            return `<a data-user-group-id="${group.id}" class="view_user_group">${$t({defaultMessage: "Role"})}:${group_name}</a>`;
        }
    }
...
function generate_user_link(user: User): string {
    return `<a data-user-id="${user.user_id}" class="view_user_profile">${user.full_name}</a>`;
}
