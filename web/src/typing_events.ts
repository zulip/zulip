...
function apply_message_edit_notifications($row: JQuery | undefined, is_typing: boolean): void {
    const $editing_notifications = $row?.find("#editing_notifications");
    if (is_typing) {
        $row?.find(".message_edit_notice").hide();
    } else {
        $row?.find(".typing-animation").hide();
...
function render_message_editing_typing(message_id: number, is_typing: boolean): void {
    ...
    const $all_msgs_view_row = message_lists.home?.get_row(message_id);
    apply_message_edit_notifications($all_msgs_view_row, is_typing);
}
