...
export function send_flag_update_for_messages(
    ...
    void channel.post({
        ...
        success() {
            local_untracked_flag_changes(flag, (changes) => {
...
export function send_flag_update_for_untracked_messages(): void {
    all_local_untracked_flag_changes((entries) => {
...
export function local_untracked_flag_changes(
    flag: string,
    cb: (entries: UntrackedFlagEntry) => UntrackedFlagEntry | undefined,
): void {
    all_local_untracked_flag_changes((untracked_flag_entries) => {
...
export function all_local_untracked_flag_changes(
    ...
    if (localstorage.supported()) {
        ...
        let untracked_flag_entries: UntrackedFlagRecord;
        const result = untracked_flag_record_schema.safeParse(untracked_flag_entries_value);

        if (!result.success) {
            untracked_flag_entries = {};
        } else {
            untracked_flag_entries = result.data;
        }
