...
function get_supersets_containing_near_or_with_message(filter: Filter): MessageListData[] {
    ...
    // Restricting candidates to conversation views via
    // `is_conversation_view` ensures a candidate cannot have silently
    ...
    for (const cached_data of cache.values().toArray().toReversed()) {
