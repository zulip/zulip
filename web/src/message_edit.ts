...
export function handle_message_edit_update(
    ...
    if (!currently_editing_messages.has(message_id)) {
        // No edit form to preserve, but end_message_edit() also does row
        // cleanup unrelated to the form -- uploads, condense controls,
        // tooltips -- that update_messages has always run for every event.
        ...
        error(xhr) {
            if (msg_list === message_lists.current) {
                ...
                if (edit_locally_echoed) {
                    ...
                    // Seed this from the message, not from the failed edit we reopen
                    // the form with: content the server never had would make a later
                    // external edit look like there were no unsaved changes.
                    last_synced_raw_content.set(message_id, echoed_message.raw_content ?? "");
