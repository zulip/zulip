...
import render_reminder from "../templates/reminder.hbs";
...
export const keyboard_handling_context = {
    get_items_ids() {
        const reminders_ids = [];
        const sorted_messages = sort_reminders(message_reminder.reminders_data);
        for (const message of sorted_messages) {
            reminders_ids.push(message.scheduled_message_id.toString());
        }
...
function sort_reminders(reminders: Map<number, Reminder>): Reminder[] {
    const sorted_messages = [...reminders.values()].sort(
...
function format(reminders: Map<number, Reminder>): ReminderRenderContext[] {
    ...
    const sorted_messages = sort_reminders(reminders);
...
export function launch(): void {
    ...
    const rendered_list = render_reminder({
        reminders_data: format(message_reminder.reminders_data),
    });
