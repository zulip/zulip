...
/** Exported only for tests. */
export function stop_last_notification(worker, message_id) {
    if (state.idle_timer) {
        clearTimeout(state.idle_timer);
    }
    worker.notify_server_stop(state.current_recipient, message_id);
    initialize_state();
}

/** Exported only for tests. */
export function start_or_extend_idle_timer(worker, message_id) {
...
function actually_ping_server(worker, recipient, current_time, message_id) {
    worker.notify_server_start(recipient, message_id);
    set_next_start_time(current_time);
}

/** Exported only for tests. */
export function maybe_ping_server(worker, recipient, message_id) {
    const current_time = worker.get_current_time();
    if (current_time > state.next_send_start_time) {
        actually_ping_server(worker, recipient, current_time, message_id);
    }
}
 ...
 *   anymore.
 */
export function update(worker, new_recipient, message_id) {
    ...
    if (current_recipient !== null) {
        ...
        if (_.isEqual(new_recipient, current_recipient)) {
            ...
            maybe_ping_server(worker, new_recipient, message_id);

            // We can also extend out our idle time.
            start_or_extend_idle_timer(worker, message_id);
    ...
    const current_time = worker.get_current_time();
    actually_ping_server(worker, new_recipient, current_time, message_id);
    start_or_extend_idle_timer(worker, message_id);
}
