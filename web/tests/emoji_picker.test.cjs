...
run_test("initialize", () => {
    ...
    const make_emoji = (emoji, score) => ({
        ...emoji,
        score,
    });

    const popular_emojis = typeahead.popular_emojis.map((emoji_code) => make_emoji(emoji_code, 10));
    const non_popular_emojis = [
        {emoji_name: "stadium", emoji_code: "1f3df"},
        {emoji_name: "moneybag", emoji_code: "1f4b0"},
        {emoji_name: "japanese_post_office", emoji_code: "1f3e3"},
        {emoji_name: "panda_face", emoji_code: "1f43c"},
        {emoji_name: "see_no_evil", emoji_code: "1f648"},
        {emoji_name: "grinning_face", emoji_code: "1f600"},
        {emoji_name: "rocket", emoji_code: "1f680"},
    ];
    const non_popular_emojis_usage = [];
    for (const [i, non_popular_emoji] of non_popular_emojis.entries()) {
        non_popular_emojis_usage.push(make_emoji(non_popular_emoji, i));
    }
    ...
    non_popular_emojis.reverse();

    assert.equal(typeahead.frequently_used_emojis.length, 12);
    assert.deepEqual(typeahead.frequently_used_emojis, [
        ...typeahead.popular_emojis,
        ...non_popular_emojis.slice(0, 6),
    ]);
});
