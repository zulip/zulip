...
const {zform_widget_extra_data_schema} = zrequire("zform_data");

run_test("schema rejects", () => {
    const bogus_data = {bogus_field: 99};
    const parse_result = zform_widget_extra_data_schema.safeParse(bogus_data);
...
run_test("schema accepts choices", () => {
    ...
    const parse_result = zform_widget_extra_data_schema.safeParse(form_data);
    assert.ok(parse_result.success);
