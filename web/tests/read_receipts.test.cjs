...
const popovers = mock_esm("../src/popovers", {hide_all() {}});
const message_lists = mock_esm("../src/message_lists", {current: {}});
const popover_menus = mock_esm("../src/popover_menus", {
    popover_instances: {read_receipt_popover: null, message_actions: null},
    toggle_popover_menu() {},
...
run_test("toggling closes an open popover, without reopening it", ({override}) => {
    override(popover_menus.popover_instances, "read_receipt_popover", {});
    ...
    override(
        ...
        /* istanbul ignore next */
        () => {
            throw new Error("an already-open popover must not be reopened");
        },
        {unused: false},
    );

    read_receipts.toggle_read_receipts(message_id);

    assert.ok(hide_all_called, "an open popover is closed with hide_all");
});

run_test("toggling opens read receipts on the message's actions button", ({override}) => {
    override(popover_menus.popover_instances, "read_receipt_popover", null);

    const $button = $.create("actions-menu-button");
    const $row = $.create("message-row");
    $row.set_find_results(".actions_hover .message-actions-menu-button", $button);
    override(message_lists.current, "get_row", () => $row);
    override(popover_menus, "hide_current_popover_if_visible", noop);

    let opened_on;
    override(popover_menus, "toggle_popover_menu", (target) => {
        opened_on = target;
    });

    read_receipts.toggle_read_receipts(message_id);

    assert.equal(opened_on, $button[0], "opens anchored to that message's actions button");
});

run_test("toggling does nothing when the message has no actions button", ({override}) => {
    override(popover_menus.popover_instances, "read_receipt_popover", null);

    const $row = $.create("message-row-without-button");
    $row.set_find_results(".actions_hover .message-actions-menu-button", []);
    override(message_lists.current, "get_row", () => $row);
    ...
    override(
        ...
        /* istanbul ignore next */
        () => {
            throw new Error("should not open a popover when there is no actions button");
        },
        {unused: false},
    );
