...
const message_store_untested = zrequire("message_store_untested");
...
test("save_topic_links", ({override_rewire}) => {
    function set_message_links(links) {
        override_rewire(message_store_untested, "extract_message_links", (message_content) => {
            if (message_content === "ignore") {
                return [];
            }
            assert.equal(message_content, "rewired");
            return links;
        });
    }
...
test("get and update topic links to/from narrow", ({override_rewire}) => {
    function set_message_links(links) {
        override_rewire(message_store_untested, "extract_message_links", (message_content) => {
            if (message_content === "ignore") {
                return [];
