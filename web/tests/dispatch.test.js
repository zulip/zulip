...
state_data.realm_description = "already set description";
...
run_test("custom profile fields", ({override}) => {
    ...
    dispatch(event);
    assert_same(state_data.custom_profile_fields, event.fields);
});
...
run_test("onboarding_steps", ({override}) => {
    ...
    dispatch(event);
    assert_same(state_data.onboarding_steps, event.onboarding_steps);
});
...
run_test("realm settings", ({override}) => {
    state_data.is_admin = true;
    state_data.realm_date_created = new Date("2023-01-01Z");
    ...
    function test_realm_boolean(event, parameter_name) {
        state_data[parameter_name] = true;
        event = {...event};
        event.value = false;
        dispatch(event);
        assert.equal(state_data[parameter_name], false);
        event = {...event};
        event.value = true;
        dispatch(event);
        assert.equal(state_data[parameter_name], true);
    }

    function test_realm_integer(event, parameter_name) {
        state_data[parameter_name] = 1;
        event = {...event};
        event.value = 2;
        dispatch(event);
        assert.equal(state_data[parameter_name], 2);

        event = {...event};
        event.value = 3;
        dispatch(event);
        assert.equal(state_data[parameter_name], 3);

        event = {...event};
        event.value = 1;
        dispatch(event);
        assert.equal(state_data[parameter_name], 1);
    }
    ...
    dispatch(event);
    assert_same(state_data.realm_create_web_public_stream_policy, 2);
    ...
    assert_same(state_data.realm_name, "new_realm_name");

    event = event_fixtures.realm__update__org_type;
    dispatch(event);
    assert_same(state_data.realm_org_type, 50);
    ...
    dispatch(event);
    assert_same(state_data.realm_notifications_stream_id, 42);
    state_data.realm_notifications_stream_id = -1; // make sure to reset for future tests

    event = event_fixtures.realm__update__signup_notifications_stream_id;
    dispatch(event);
    assert_same(state_data.realm_signup_notifications_stream_id, 41);
    state_data.realm_signup_notifications_stream_id = -1; // make sure to reset for future tests

    event = event_fixtures.realm__update__default_code_block_language;
    dispatch(event);
    assert_same(state_data.realm_default_code_block_language, "javascript");
    ...
    dispatch(event);
    assert_same(state_data.realm_enable_spectator_access, true);
    assert_same(update_called, true);
    ...
    state_data.realm_message_content_edit_limit_seconds = 0;
    state_data.realm_edit_topic_policy = 3;
    override(settings_org, "populate_auth_methods", noop);
    dispatch(event);
    assert_same(state_data.realm_create_multiuse_invite_group, 3);
    assert_same(state_data.realm_allow_message_editing, true);
    assert_same(state_data.realm_message_content_edit_limit_seconds, 5);
    assert_same(state_data.realm_edit_topic_policy, 4);
    assert_same(state_data.realm_authentication_methods, {Google: true});
    ...
    assert_same(state_data.realm_icon_url, "icon.png");
    assert_same(state_data.realm_icon_source, "U");

    override(realm_logo, "render", noop);

    event = event_fixtures.realm__update_dict__logo;
    dispatch(event);
    assert_same(state_data.realm_logo_url, "logo.png");
    assert_same(state_data.realm_logo_source, "U");

    event = event_fixtures.realm__update_dict__night_logo;
    dispatch(event);
    assert_same(state_data.realm_night_logo_url, "night_logo.png");
    assert_same(state_data.realm_night_logo_source, "U");
...
run_test("realm_domains", ({override}) => {
    ...
    dispatch(event);
    assert_same(state_data.realm_domains, [event.realm_domain]);
    ...
    dispatch(event);
    assert_same(state_data.realm_domains, []);
});
...
run_test("typing", ({override}) => {
    ...
    state_data.user_id = typing_person1.user_id + 1;
    ...
    state_data.user_id = typing_person1.user_id;
    event = event_fixtures.typing__start;
    dispatch(event);
    state_data.user_id = undefined; // above change shouldn't effect stream_typing tests below
});
