...
test_people("get_users_full_names_that_matches_role_ids", () => {
    people.add_active_user(realm_admin);
    people.add_active_user(realm_owner);
    people.add_active_user(moderator);
    people.add_active_user(guest);
    ...
    let names = people.get_users_full_names_that_matches_role_ids(
        user_ids,
        new Set([settings_config.user_role_values.admin.code]),
    );
    assert.deepEqual(names.sort(), [realm_admin.full_name].sort());

    names = people.get_users_full_names_that_matches_role_ids(
        user_ids,
        new Set([settings_config.user_role_values.moderator.code]),
    );
    assert.deepEqual(names.sort(), [moderator.full_name].sort());
    ...
    assert.deepEqual(names.sort(), [realm_owner.full_name, realm_admin.full_name].sort());
});
