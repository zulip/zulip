...
test("predicate_basics", ({override}) => {
    ...
    assert.ok(
        !predicate({
            content: `<span class="user-mention" data-user-id="${steve.user_id}">@steve</span>`,
        }),
    );
    // Silent mentions should not match.
    assert.ok(
        !predicate({
            content: `<span class="user-mention silent" data-user-id="${joe.user_id}">joe</span>`,
        }),
    );
