...
test("alert_words", () => {
    ...
    // Alert words count even in muted streams and topics, just like
    // direct mentions, so these two muted messages are included.
    ...
    counts = unread.get_counts();
    assert.equal(counts.alert_word_message_count, 5);
    assert.deepEqual(unread.get_msg_ids_for_alert_words(), [
    ...
    unread.mark_as_read(muted_alert_word_message.id);
