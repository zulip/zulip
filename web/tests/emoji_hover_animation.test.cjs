...
function make_animated_emoji(name, {inside_hover_area = false} = {}) {
    ...
    $emoji.set_matches(":focus-within", false);
...
function make_hover_area(name, emojis) {
    ...
    $area.set_matches(":focus-within", false);
...
// `:focus-within` matches the trigger itself when it is focused, and
// also a trigger that merely contains the focused element.
function focus_in(handler, $trigger) {
    $trigger.set_matches(":focus-within", true);
    handler.call($trigger[0]);
}

function focus_out(handler, $trigger) {
    $trigger.set_matches(":focus-within", false);
    handler.call($trigger[0]);
}
...
run_test("keyboard focus animates too", () => {
    ...
    // An emoji picker cell is itself the keyboard target; a buddy list
    // row instead wraps a focusable link. `:focus-within` covers both.
    const $area = make_hover_area(".emoji-picker-cell", [...$emoji]);

    focus_in(handler, $area);
...
run_test("emoji with no animated URL is left alone", () => {
    ...
    $static_emoji.set_matches(":focus-within", false);
