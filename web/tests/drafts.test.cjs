...
const echo = mock_esm("../src/echo", {
    ...
    abort_messages_by_draft_ids: noop,
...
test("outbox cancel propagates to echo abort pipeline", ({override, mock_template}) => {
    ...
    assert.deepEqual(abort_called_with, ["outbox_id"]);
});

test("outbox cancel offers an undo that restores a plain draft", ({override, mock_template}) => {
    const outbox_draft = {
        stream_id,
        topic: "topic",
        type: "stream",
    ...
    override(echo, "abort_messages_by_draft_ids", noop);

    mock_template("draft_table_body.hbs", false, () => "<draft table stub>");
    let banner_label;
    mock_template("components/banner.hbs", false, (data) => {
        banner_label = data.label;
        return "<banner stub>";
    });
    ...
    drafts_overlay_ui.launch();
    drafts_overlay_ui.initialize();
    // Dismissing the banner discards any pending undo batch, so the count
    // below is this test's alone.
    $("body").get_on_handler("click", "#draft_overlay_banner_container .banner-close-button")();

    const $row = $.create("outbox undo row");
    $row.attr("data-draft-id", "outbox_id");
    const $cancel_btn = $("#drafts_table .outbox-cancel-message");
    $cancel_btn.set_closest_results(".overlay-message-row", $row);
    ...
    assert.equal(banner_label, "translated: 1 message was canceled.");

    // Undo brings the text back as an ordinary draft; the local echo is gone.
    const added_drafts = [];
    override(drafts.draft_model, "addDraft", (draft) => {
        added_drafts.push(draft);
        return "restored_id";
    });
    $("body").get_on_handler(
        "click",
    ...
    assert.deepEqual(
        added_drafts.map((draft) => [draft.content, draft.is_sending_saving]),
...
test("outbox tab", ({override, mock_template}) => {
    ...
    mock_template("draft_table_body.hbs", false, (data) => {
        ...
        // Rendering the table empties #drafts_table, which discards its
        // stubbed find results, so set them up here instead.
        $("#drafts_table").set_find_results(
            ".outbox-tab-pane .message_content.rendered_markdown",
            [],
        );
        return "<draft table stub>";
    });

    const $outbox_row = $.create("outbox row");
    $outbox_row.attr("data-draft-id", "outbox_id");

    $.set_results(".drafts-list", []);
    $.set_results(".drafts-tab-pane .overlay-message-row", []);
    $.set_results(".outbox-tab-pane .overlay-message-row", [$outbox_row]);
    $.set_results(".draft-selection-checkbox", []);
    drafts_overlay_ui.launch();
