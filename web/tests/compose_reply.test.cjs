...
run_test("get_multi_message_quote_status", () => {
    ...
    assert.equal(
        compose_reply.get_multi_message_quote_status(
            [channel_1_general, channel_1_general_2],
            false,
        ),
    ...
    assert.equal(
        compose_reply.get_multi_message_quote_status([channel_1_general, channel_1_design], false),
        "QUOTING_MESSAGES_FROM_SAME_CHANNEL_AND_MULTIPLE_TOPICS",
    );

    assert.equal(
        compose_reply.get_multi_message_quote_status([channel_1_general, channel_1_design], true),
    ...
    assert.equal(
        compose_reply.get_multi_message_quote_status([channel_1_general, channel_2_general], false),
        "MESSAGES_WITH_NOTHING_IN_COMMON",
    );

    // 2. Mixed channel and private messages
    assert.equal(
        compose_reply.get_multi_message_quote_status([channel_1_general, pm_1], false),
        "MESSAGES_WITH_NOTHING_IN_COMMON",
    );
...
run_test("build_and_process_quote_assets_for_messages", ({override}) => {
    ...
    assert.deepEqual(
        ...
        "Should use existing raw_content if already available",
    );
