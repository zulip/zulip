...
run_test("build_and_process_quote_assets_for_messages", ({override}) => {
    ...
    // Case: Partial success. Undefined slots use message_store raw_content
    // when present, otherwise paste_handler_converter on the message content.
    override(
        compose_paste,
        "paste_handler_converter",
        (content) => `converted_by_turndown: ${content}`,
    ...
    assert.deepEqual(
        ...
        "Undefined batch slot without message_store raw_content uses HTML conversion.",
    );
    assert.deepEqual(
        result_assets[2],
        {message: msg_from_message_store, quote_content: "From message_store"},
        "Undefined batch slot with message_store raw_content uses message_store.",
    );
    assert.deepEqual(
        result_assets[3],
        {
    ...
    // Case: All slots are undefined. Use message_store for id 3 and HTML for id 4.
    override(
        message_fetch_raw_content,
        "get_raw_content_for_messages",
        ({_message_ids, on_success, _on_error}) => {
    ...
    assert.deepEqual(
        ...
        ["From message_store", "converted_by_turndown: <p>html only body</p>"],
    );
});
