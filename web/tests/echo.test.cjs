...
function make_spinner_row() {
    // A minimal stand-in for the failed-message $row, tracking just the
    // "rotating" class that the retry-spinner show/hide helpers toggle.
    let rotating = false;
    const $spinner = {
        hasClass: (cls) => cls === "rotating" && rotating,
        toggleClass(cls, val) {
            if (cls === "rotating") {
                rotating = val;
            }
