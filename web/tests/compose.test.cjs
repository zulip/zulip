...
test_ui("split_message_send_multi_part", ({override, override_rewire, disallow_rewire}) => {
    ...
    override_rewire(drafts, "update_draft", () => {
        ...
        drafts.set_compose_draft_id(100);
        return 100;
    });
...
test_ui(
    "split_message_send_does_not_clear_a_switched_compose_session",
    ({override, override_rewire, disallow_rewire}) => {
        ...
        override_rewire(echo, "reify_message_id", noop);
        disallow_rewire(echo, "try_deliver_locally");
        override(sent_messages, "get_new_local_id", () => "loc-switch-1");
        override(compose_notifications, "get_muted_narrow", () => undefined);
        ...
        override(transmit, "send_message", (payload, success) => {
            sent_contents.push(payload.content);
            if (sent_contents.length === 2) {
                drafts.set_compose_draft_id(200);
                fake_compose_box.set_textarea_val("a fresh unrelated draft");
            }
            success({id: sent_contents.length * 100});
        });
        ...
        assert.deepEqual(sent_contents, ["part1", "part2"]);
        assert.equal(fake_compose_box.textarea_val(), "a fresh unrelated draft");
        ...
        assert.ok(
            banners.some(
                (banner) =>
                    ...
                    banner.text.includes("1 message was already sent"),
            ),
        );
        assert.ok(
            banners.some(
...
test_ui(
    ...
    ({disallow, disallow_rewire, mock_template}) => {
        ...
        fake_compose_box.set_textarea_val(parts.join(compose_split_messages.SPLIT_DELIMITER));
...
test_ui("message_edit_preview_is_not_split", ({override}) => {
    compose_split_messages.set_split_messages_enabled(true);
    ...
    override(markdown, "render", (raw_content) => {
        rendered_parts.push(raw_content);
        return {content: "R:" + raw_content};
    });
    override(channel, "post", () => {});

    const $edit_row = $.create("edit-row-stub");
    $edit_row.set_matches("#compose", false);

    compose_ui.render_and_show_preview(
        $edit_row,
        $.create("edit-spinner-stub"),
        $.create("edit-preview-content-stub"),
        "part1\n\n\npart2",
    );

    assert.deepEqual(rendered_parts, ["part1\n\n\npart2"]);
