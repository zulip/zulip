    ...
    (function test_livekit_video_and_audio_links_compose_clicked() {
        ...
        function call_success_callback() {
            ...
            success_callback({
                result: "success",
                msg: "",
                url: "/calls/livekit/join?call=signed-token",
            });
        }
        ...
        call_success_callback();
        const video_link_regex =
            /\[translated: Join video call\.]\(\/calls\/livekit\/join\?call=signed-token\)/;
        assert.ok(called);
        assert.match(syntax_to_insert, video_link_regex);
        ...
        call_success_callback();
        const audio_link_regex =
            /\[translated: Join voice call\.]\(\/calls\/livekit\/join\?call=signed-token\)/;
        assert.ok(called);
        assert.match(syntax_to_insert, audio_link_regex);
    })();
});
