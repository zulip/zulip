...
const empty_topic_only_channel = {
    stream_id: 302,
    name: "empty_topic_only_channel",
};

const sub_map = new Map([
    [social.name, social],
    [empty_topic_only_channel.name, empty_topic_only_channel],
]);
...
function is_empty_topic_only_channel(stream_id) {
    return stream_id === empty_topic_only_channel.stream_id;
}
...
run_test("empty-topic-only channel topic links", () => {
    ...
    assert_parse(
        "#**empty_topic_only_channel>lunch**",
        "<p>#**empty_topic_only_channel&gt;lunch**</p>",
    );
    assert_parse(
        "#**empty_topic_only_channel>lunch@12**",
        "<p>#**empty_topic_only_channel&gt;lunch@12**</p>",
    );
    // The empty topic is still linkified, since it's the one topic
    // such a channel can contain.
    assert_parse(
        "#**empty_topic_only_channel>**",
        '<p><a class="stream-topic" data-stream-id="302" href="stream-302-topic-">#empty_topic_only_channel &gt; <span class="empty-topic-display">translated: general chat</span></a></p>',
    );
    // The channel link itself is still linkified.
    assert_parse(
        "#**empty_topic_only_channel**",
        '<p><a class="stream" data-stream-id="302" href="/stream-302">#empty_topic_only_channel</a></p>',
    );
});
