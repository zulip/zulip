...
run_test("delete_message_private_cleanup", ({override}) => {
    // Server confirms empty → DM row is removed from the sidebar.
    // display_recipient items need .id (pm_with_user_ids reads .id, not .user_id).
    const user4 = {user_id: 4, id: 4, email: "user4@example.com", full_name: "User 4"};
    const user5 = {user_id: 5, id: 5, email: "user5@example.com", full_name: "User 5"};
    ...
    // Only clear the cache; skip the full remove_messages side effects.
    override(message_events, "remove_messages", (ids) => {
        message_store.remove(ids);
    });
    override(emoji_frequency, "update_emoji_frequency_on_messages_deletion", noop);
...
run_test("delete_message_private_local_optimization", ({override}) => {
    // display_recipient items need .id (pm_with_user_ids reads .id, not .user_id).
    const user4 = {user_id: 4, id: 4, email: "user4@example.com", full_name: "User 4"};
    const user5 = {user_id: 5, id: 5, email: "user5@example.com", full_name: "User 5"};
    people.add_active_user(user4);
    ...
    const remove_stub = make_stub();
    override(pm_conversations.recent, "remove", remove_stub.f, {unused: false});
    override(unread_ops, "process_read_messages_event", noop);
    // Deletes msg_old (500) from the cache; msg_new (501) survives, so
    // has_messages_in_dm() returns true and the server check is skipped.
    override(message_events, "remove_messages", (ids) => {
        message_store.remove(ids);
    });
    override(emoji_frequency, "update_emoji_frequency_on_messages_deletion", noop);
    ...
    // msg_new (501) was not part of the deletion; remove it so it doesn't
    // leak into subsequent tests.
    message_store.remove([501]);
});

run_test("delete_message_private_server_not_empty", ({override}) => {
    // display_recipient items need .id (pm_with_user_ids reads .id, not .user_id).
    const user4 = {user_id: 4, id: 4, email: "user4@example.com", full_name: "User 4"};
    const user5 = {user_id: 5, id: 5, email: "user5@example.com", full_name: "User 5"};
    people.add_active_user(user4);
...
run_test("delete_message_private_race_condition", ({override}) => {
    ...
    // First call returns the pre-deletion max; second call (in the success
    // callback) returns a higher ID, simulating a new message arriving.
    let get_max_call_count = 0;
    override(pm_conversations.recent, "get_max_message_id", () => {
        get_max_call_count += 1;
        return get_max_call_count === 1 ? 503 : 999;
    });
...
run_test("delete_message_private_already_removed", ({override}) => {
    const user6 = {user_id: 6, id: 6, email: "user6@example.com", full_name: "User 6"};
    const user7 = {user_id: 7, id: 7, email: "user7@example.com", full_name: "User 7"};
    people.add_active_user(user6);
    people.add_active_user(user7);

    const test_message_dm = {
        id: 506,
        type: "private",
        to_user_ids: "6,7",
        display_recipient: [user6, user7],
        sender_id: 6,
        content: "test",
        timestamp: 1234567890,
    };
    ...
    pm_conversations.recent.insert([6, 7], 506);

    const remove_stub2 = make_stub();
    override(pm_conversations.recent, "remove", remove_stub2.f, {unused: false});

    // First call (before server request) returns the pre-deletion max; second
    // call (in the success callback) returns undefined, simulating the
    // conversation having been removed by another event while in flight.
    let get_max_call_count = 0;
    override(pm_conversations.recent, "get_max_message_id", () => {
        get_max_call_count += 1;
        return get_max_call_count === 1 ? 506 : undefined;
    });
    ...
    assert.equal(remove_stub2.num_calls, 0);
});

run_test("delete_message_private_api_error", ({override}) => {
    const user8 = {user_id: 8, id: 8, email: "user8@example.com", full_name: "User 8"};
    const user9 = {user_id: 9, id: 9, email: "user9@example.com", full_name: "User 9"};
