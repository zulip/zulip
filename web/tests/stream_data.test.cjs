...
test("can_access_topics_in_stream", ({override}) => {
    ...
    // Being an organization administrator is not by itself enough: the server
    // decides purely on membership in can_access_stream_topics_group, so a
    // client-side bypass here would locally echo messages the server rejects.
    override(current_user, "is_admin", true);
    assert.equal(stream_data.can_access_topics_in_stream(sub), false);
    override(current_user, "is_admin", false);
