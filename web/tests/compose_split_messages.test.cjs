...
run_test("delimiter_index_outside_code_block", () => {
    compose_split_messages.set_split_messages_enabled(true);
    const first_delimiter_indices = [0, 0, -1, 32, 32];
    let i = 0;
...
run_test("whitespace_only_blank_lines_split", () => {
    compose_split_messages.set_split_messages_enabled(true);
    assert.deepEqual(compose_split_messages.split_message("first\n \n\t\nsecond"), [
        "first",
        "\n \n\t\nsecond",
    ]);
    assert.equal(
        compose_split_messages.will_split_into_multiple_messages("first\n \n \nsecond"),
        true,
    );
    compose_split_messages.set_split_messages_enabled(false);
});
