...
run_test("remove preview button - standard embed", () => {
    const $content = get_content_element();
...
run_test("remove preview button - image preview", () => {
    const $content = get_content_element();
...
run_test("remove preview button - YouTube video preview", () => {
    const $content = get_content_element();
...
run_test("remove preview button - skipped for user uploads", () => {
    ...
    const $content = get_content_element();
    const selector = ".message_embed, .message-media-preview-image, .message-media-preview-video";
    $content.set_find_results(selector, $uploads);
...
run_test("remove preview button - malformed href", () => {
    const $content = get_content_element();
    const $preview = make_preview_element("malformed-href", "http://");

    const selector = ".message_embed, .message-media-preview-image, .message-media-preview-video";
    $content.set_find_results(selector, [$preview]);

    set_message_for_message_content($content, {id: 106});

    // append must not be called; assert.fail will throw if it is.
    $preview[0].append = assert.fail;
...
run_test("remove preview button - external upload-like URL", () => {
    ...
    const $preview = make_preview_element(
...
run_test("remove preview button - skipped in compose/edit preview", () => {
    const $content = get_content_element();
    const $embed = make_preview_element("embed-in-preview", "https://example.com/article");
