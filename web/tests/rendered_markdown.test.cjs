...
run_test("stream-topic link alert words", ({mock_template}) => {
    const $content = get_content_element();
    const $stream_topic = $.create("a.stream-topic(alert)");
    $stream_topic.set_find_results(".highlight", false);
    $stream_topic.attr("href", `/#narrow/channel/${stream.stream_id}-random/topic/alert`);
    $stream_topic.hasClass = (class_name) => class_name === "stream-topic";
    let replaced_html;
