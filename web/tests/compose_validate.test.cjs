...
test_ui("check_overflow_text_does_not_re_enter_validate", ({override, disallow_rewire}) => {
    mock_banners();
    override(current_user, "user_id", me.user_id);
    override(realm, "realm_topics_policy", "allow_empty_topic");
    ...
    compose_state.topic("length topic");
    $("#send_message_form").set_find_results(".message-textarea", $("textarea#compose-textarea"));
    $("#send_message_form").set_find_results(
        ".message-limit-indicator",
        $.create("length-limit-indicator-stub"),
    );
    $("textarea#compose-textarea").set_closest_results(".message_row", []);

    compose_state.message_content("a".repeat(50));
    assert.ok(compose_validate.validate());

    disallow_rewire(compose_validate, "validate_and_update_send_button_status");
    compose_state.message_content("a".repeat(10005));
    assert.ok(!compose_validate.validate());
});
