...
people.add_active_user(non_active_user);
...
test("get_conversations", ({override}) => {
    ...
    people.deactivate(non_active_user);
    ...
    assert.ok(deactivated_user?.includes_deactivated_user);
    assert.ok(deactivated_group?.includes_deactivated_user);
