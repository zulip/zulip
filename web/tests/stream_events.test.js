...
test("update_property", ({override}) => {
    ...
    // Test stream can_access_stream_topics_group change event
    {
        const stub = make_stub();
        override(stream_settings_ui, "update_can_access_stream_topics_group_id", stub.f);
        stream_events.update_property(stream_id, "can_access_stream_topics_group", 4);
        assert.equal(stub.num_calls, 1);
        const args = stub.get_args("sub", "val");
        assert.equal(args.sub.stream_id, stream_id);
        assert.equal(args.val, 4);
    }
});
