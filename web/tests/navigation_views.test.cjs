...
const {built_in_views_values} = zrequire("navigation_views");
...
run_test("get_built_in_views", () => {
    ...
    assert.equal(inbox_view.is_pinned, built_in_views_values.inbox.is_pinned);
});
...
run_test("get_all_navigation_views", () => {
    ...
    assert.ok(starred_view);
    assert.equal(starred_view.is_pinned, true);
    assert.equal(starred_view.name, built_in_views_values.starred_messages.name);
