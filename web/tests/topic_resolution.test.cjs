...
run_test(
    "can_toggle_topic_resolution defers to can_resolve_topics for missing or archived sub",
    ({override}) => {
        ...
        assert.equal(
            topic_resolution.can_toggle_topic_resolution({is_archived: true}, "topic"),
            false,
        );

        override(stream_data, "can_resolve_topics", () => true);
        assert.equal(topic_resolution.can_toggle_topic_resolution(undefined, "topic"), true);
    },
);
