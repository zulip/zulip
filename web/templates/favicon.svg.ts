...
export default function render_favicon_svg(context) {
    const out = html`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
        <style>
            @font-face {
                font-family: "Source Sans 3";
                font-style: normal;
                font-weight: 700;
                src: url(${{__html: context.favicon_font_url_html}}) format("truetype");
            }
        </style>
        ...
        <path
            ...
            fill="url(#a)"
            transform="translate(8 8) scale(0.023769201057729446) translate(-386.56 -386.56)"
        />
        <text
            x="15"
            y="15"
            ...
            opacity=".5"
            ${to_bool(context.count_long)
                ? html`font-size="9" textLength="14" lengthAdjust="spacingAndGlyphs"`
                : html`font-size="11"`}
        >
            ${context.count}
        </text>
        ...
        ${to_bool(context.have_pm) ? html` <circle cx="14" cy="4" r="2" fill="#f00" /> ` : ""}
    </svg> `;
    return to_html(out);
}
