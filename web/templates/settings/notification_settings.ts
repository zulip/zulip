...
export default function render_notification_settings(context) {
    const out = html`<form class="notification-settings-form">
        ...
        >
            ...
            <table class="notification-table table table-bordered wrapped-table">
                        ...
                        (settings_section) => html`
                            <tr>
                                <td>
                                    ...
                                    ${to_bool(settings_section.help_link)
                                        ? html` ${{
                                              __html: render_help_link_widget({
                                                  link: settings_section.help_link,
                                              }),
                                          }}`
                                        : ""}
                                </td>
                                ${to_array(settings_section.notification_settings).map(
                                    (setting) =>
                                        html` ${{
                                            __html: render_notification_settings_checkboxes({
                                                prefix: context.prefix,
                                                push_notifications_disabled:
                                                    setting.push_notifications_disabled,
                ...
                ${!to_bool(context.for_realm_settings)
                    ? html`
                          <tbody id="stream-specific-notify-table"></tbody>
                          <tbody id="customizable_stream_notifications_table">
                              <tr>
                                  ...
                                  ${to_array(
                                      context.custom_stream_specific_notification_settings,
                                  ).map(
                                      (setting) =>
                                          html` ${{
                                              __html: render_notification_settings_checkboxes({
        ...
        >
            ...
            <div class="input-group">
                <label
                    for="${context.prefix}resolved_topic_notice_auto_read_policy"
                    class="settings-field-label"
                    >${$t({
                        defaultMessage: "Automatically mark resolved topic notices as read",
                    })}</label
                >
                <select
                    name="resolved_topic_notice_auto_read_policy"
                    class="setting_resolved_topic_notice_auto_read_policy prop-element settings_select bootstrap-focus-style"
                    id="${context.prefix}resolved_topic_notice_auto_read_policy"
                    data-setting-widget-type="string"
                >
                    ${{
                        __html: render_dropdown_options_widget({
                            option_values: context.resolved_topic_notice_auto_read_policy_values,
                        }),
                    }}
                </select>
            </div>
        ...
        >
            ...
            ${context.prefix === "user_"
                ? html` <div class="desktop-notification-settings-banners banner-wrapper"></div> `
                : ""}
            ${!to_bool(context.for_realm_settings)
                ? html` ${{
                      __html: render_action_button({
                          ...
                          intent: "neutral",
                          attention: "quiet",
                          label: $t({defaultMessage: "Send a test notification"}),
                      }),
                  }}`
                : ""}
        ...
        >
            ...
            ${!to_bool(context.realm_push_notifications_enabled)
                ? html`
                      <div class="mobile-push-notifications-banner-container banner-wrapper"></div>
                  `
                : ""}
