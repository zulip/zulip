...
export default function render_navbar_icon_and_title(context) {
    const out = html`${to_bool(context.zulip_icon)
        ...
        : to_bool(context.icon)
          ...
          : ""}${to_bool(context.title)
        ? html`<span class="message-header-navbar-title">${context.title}</span> `
        : ""}${to_bool(context.title_html)
        ? html`<span class="message-header-navbar-title">${{__html: context.title_html}}</span> `
