...
import render_message_reminders from "./message_reminders.ts";
    ...
    const out = html`${to_bool(context.include_sender)
            ...
            ${to_bool(context.include_sender)
                ? html` <span class="sender_info_hover sender_name" role="button" tabindex="0">
                      ...
                      ${!to_bool(context.is_hidden)
                          ? html`${to_bool(context.sender_is_bot)
                                ? html`
                                      <i
                                ...
                                : ""}${to_bool(context.status_message)
                                ? html` <span
                                          class="message_content rendered_markdown status-message"
                                          >${{
                                              __html: postprocess_content(context.status_message),
                                          }}</span
                                      >
        ...
        ${!to_bool(context.is_hidden) && to_bool(context.msg.message_reactions)
            ...
            : ""}
        ${to_bool(context.msg.reminders)
            ? html` ${{__html: render_message_reminders(context)}}`
            : ""}`;
    return to_html(out);
}
