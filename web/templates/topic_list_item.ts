...
export default function render_topic_list_item(context) {
    ...
    >
        <a href="${context.url}" class="topic-box" draggable="false">
            <span class="sidebar-topic-check"> ${context.topic_resolved_prefix} </span>
            <span class="sidebar-topic-name">
                <span
                    class="sidebar-topic-name-inner ${to_bool(context.is_empty_string_topic)
                        ? "empty-topic-display"
                        : ""}"
            ...
            >
                ${to_bool(context.contains_unread_mention)
                    ...
                    : to_bool(context.is_followed)
                      ...
                      : ""}
                <span
                    class="unread_count normal-count ${to_bool(context.is_zero)
                        ? "zero_count"
                        : ""}"
                >
                    ${context.unread}
