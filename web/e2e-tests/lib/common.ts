...
export async function get_rendered_messages(
    ...
    return Promise.all(
        recipient_rows.map(async (element): Promise<[string, string[]]> => {
            ...
            let key;
            if (channel_view && topic_name !== "") {
                // We use only topic_name when we are in stream narrow and
                // stream header button is visible.
                key = topic_name;
            } else if (topic_name !== "" && stream_name !== "") {
                // If topic_name is '', then this is direct messages, so only
                // append > topic_name if we are in combined feed and not in
                // 1:1 or group direct messages.
