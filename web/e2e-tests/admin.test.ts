...
async function test_add_emoji_without_cropping(page: Page): Promise<void> {
    ...
    await page.waitForSelector(".emoji-edit-button", {visible: true});
...
async function test_add_emoji_with_cropping(page: Page): Promise<void> {
    ...
    await page.waitForSelector(".emoji-preview-section", {visible: true});
    await page.click(".emoji-edit-button");
    ...
    // wait for transition back to preview state
...
async function test_cancel_emoji_cropping(page: Page): Promise<void> {
    ...
    await page.waitForSelector("tr#emoji_zulip_cancelled", {visible: true});
    assert.strictEqual(
        await common.get_text_from_selector(page, "tr#emoji_zulip_cancelled .emoji_name"),
        "zulip cancelled",
        "Cancelled emoji name incorrectly saved.",
    );
    await page.waitForSelector("tr#emoji_zulip_cancelled img", {visible: true});
}
...
async function test_custom_realm_emoji(page: Page): Promise<void> {
    ...
    await common.wait_for_micromodal_to_open(page);
    await test_add_emoji_without_cropping(page);
    await test_delete_emoji(page, "zulip_logo");
    ...
    await test_delete_emoji(page, "zulip_cancelled");
}
